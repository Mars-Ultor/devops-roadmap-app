...
describe("HintSystem", () => {
  ...
  it("shows locked state when hints are not unlocked", () => {
    ...
    expect(
      screen.getByText("Hints locked. Complete the struggle timer first."),
    ).toBeInTheDocument();
  ...
  it("enforces 5-minute cooldown between hints", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: /view hint 2/i }),
  ...
  it("allows next hint after cooldown expires", async () => {
    ...
    await waitFor(() => {
      const viewSecondHintButton = screen.getByRole("button", {
        name: /view hint 2/i,
      });
  ...
  it("shows all hints used message when no more hints available", async () => {
        ...
        await waitFor(() => {
          expect(
            screen.getByRole("button", {
              name: new RegExp(`view hint ${i + 1}`, "i"),
            }),
          ).toBeEnabled();
        });
      }
    ...
    expect(
      screen.getByText(/all available hints\. the full solution will unlock/i),
    ).toBeInTheDocument();
  });
