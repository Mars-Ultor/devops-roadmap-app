...
describe('StruggleTimer', () => {
  ...
  it.skip('shows struggle documentation form when button is clicked', async () => {
    ...
    expect(screen.getByText('What have you tried? (Minimum 3 things)')).toBeInTheDocument()
    expect(screen.getByText('Where are you stuck? (Be specific)')).toBeInTheDocument()
  ...
  it.skip('accepts valid struggle documentation', async () => {
    ...
    await user.type(stuckTextarea, 'Cannot navigate to the correct directory even though it exists')

    const hypothesisTextarea = screen.getByPlaceholderText(/your best guess/i)
    await user.type(hypothesisTextarea, 'The path might be incorrect or there might be permission issues')
    ...
    expect(mockOnStruggleLogged).toHaveBeenCalledWith({
      attemptedSolutions: ['Tried cd command', 'Checked ls -la', 'Used pwd to verify location'],
      stuckPoint: 'Cannot navigate to the correct directory even though it exists',
      hypothesis: 'The path might be incorrect or there might be permission issues',
  ...
  it.skip('shows struggles logged status after documentation', async () => {
    ...
    expect(screen.getByText(/hints will unlock when timer reaches 0:00/i)).toBeInTheDocument()
