...
describe("MandatoryAARModal", () => {
  ...
  it("renders modal with correct title and status", () => {
    ...
    expect(
      screen.getByText("After Action Review Required"),
    ).toBeInTheDocument();
  ...
  it("shows all required AAR questions", () => {
    ...
    expect(
      screen.getByText(/What was I trying to accomplish/),
    ).toBeInTheDocument();
  ...
  it.skip("validates minimum word counts", async () => {
    ...
    const submitButton = screen.getByRole("button", {
      name: /submit aar and continue/i,
    });
    ...
    expect(
      screen.getByText("Too brief. Need at least 20 words (currently 1)"),
    ).toBeInTheDocument();
  });
  ...
  it.skip("validates minimum word count for each question", async () => {
    ...
    expect(
      screen.getByText(/Too brief\. Need at least \d+ words/),
    ).toBeInTheDocument();
  });
  ...
  it.skip("shows submission progress", async () => {
    ...
    for (const textarea of textareas) {
      await user.type(
        textarea,
        "This is sufficient content for the minimum word count requirement",
      );
    }
  ...
  it.skip("handles submission errors gracefully", async () => {
    ...
    vi.mocked(addDoc).mockImplementationOnce(() =>
    ...
    await waitFor(
      () => {
        expect(window.alert).toHaveBeenCalledWith(
          "Error submitting AAR. Please try again.",
        );
      },
      { timeout: 10000 },
    );
  ...
  it("shows completion progress indicator", async () => {
    ...
    await user.type(
      firstTextarea,
      "This is sufficient content for the minimum word count requirement that meets the twenty word minimum requirement for this particular question in the after action review process",
    );
  ...
  it("enables submit button when all questions meet requirements", () => {
    ...
    for (let i = 0; i < textareas.length; i++) {
      fireEvent.change(textareas[i], {
        target: { value: sufficientContent[i] },
      });
    }
  ...
  it("shows warning about mandatory reflection", () => {
    ...
    expect(
      screen.getByText(/you must complete this aar before continuing/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/no navigation, no skipping, no shortcuts/i),
    ).toBeInTheDocument();
  });
