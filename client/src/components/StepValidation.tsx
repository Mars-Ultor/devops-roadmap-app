...
export default function StepValidation({ steps, onStepComplete, onValidationRun }: StepValidationProps) {
  ...
  const runValidation = async (stepNumber: number, rule: ValidationRule, ruleIndex: number) => {
    ...
    try {
      ...
      if (onValidationRun) { success = await onValidationRun(stepNumber, rule); message = success ? 'Validation passed' : 'Validation failed'; }
      else { const result = await ValidationService.runValidation(rule); success = result.success; message = result.message; }
      setValidationResults(prev => new Map(prev).set(validationKey, { success, message, timestamp: new Date() }));
      return success;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      setValidationResults(prev => new Map(prev).set(validationKey, { success: false, message: `❌ Error: ${errorMessage}`, timestamp: new Date() }));
      return false;
    } finally {
      setRunningValidations(prev => { const newSet = new Set(prev); newSet.delete(validationKey); return newSet; });
    }
  };

  const runAllValidationsForStep = async (step: LabStep) => {
    let passedCount = 0;
    for (let i = 0; i < step.validations.length; i++) { if (await runValidation(step.number, step.validations[i], i)) passedCount++; }
  ...
  return (
    <div className="space-y-6">
      {steps.map((step) => {
        ...
        return (
          <div key={step.number} className={`border-2 rounded-lg p-6 transition-all duration-300 ${getStepStatusColor(step.status)}`}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-3">
                <StepStatusIcon status={step.status} />
                <div>
                  <h3 className={`text-lg font-semibold ${isLocked ? 'text-gray-500' : 'text-white'}`}>Step {step.number}: {step.title}</h3>
                  {step.description && <p className={`text-sm ${isLocked ? 'text-gray-600' : 'text-gray-400'}`}>{step.description}</p>}
                </div>
              </div>
              {canRun && <RunValidationButton isRunning={runningValidations.size > 0} isCompleted={step.status === 'completed'} onClick={() => runAllValidationsForStep(step)} />}
            </div>
            <div className="space-y-3">
              <h4 className={`text-sm font-medium uppercase tracking-wide ${isLocked ? 'text-gray-600' : 'text-indigo-300'}`}>Validation Checks ({step.completedValidations}/{step.validations.length})</h4>
