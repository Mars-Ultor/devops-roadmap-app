...
function buildQuickAARData(props: UseAdaptiveAARSubmitProps, quickReflection: QuickReflection) {
...
export function useAdaptiveAARSubmit(props: UseAdaptiveAARSubmitProps) {
  ...
  const handleSkip = useCallback(async () => {
    ...
    try {
      const docRef = await addDoc(collection(db, 'afterActionReviews'), buildSkipAARData(props));
  ...
  const handleQuickSubmit = useCallback(async (quickReflection: QuickReflection) => {
    if (!quickReflection.mainTakeaway.trim() || !quickReflection.oneImprovement.trim()) return;
    setIsSubmitting(true);
    try {
      const docRef = await addDoc(collection(db, 'afterActionReviews'), buildQuickAARData(props, quickReflection));
      props.onComplete(docRef.id);
    } catch (error) {
