...
export const AAR_QUESTIONS: AARQuestion[] = [
  ...
  },
  {
    ...
    minWords: 30,
    placeholder:
      "List successful approaches, commands that worked, good decisions...",
  },
  {
    id: "didntWork",
    ...
    minWords: 20,
    placeholder:
      "What failed, what errors occurred, what took longer than expected...",
  },
  {
    id: "why",
 ...
 */
export const countCompletedQuestions = (
  responses: Record<string, string>,
): number => {
  return AAR_QUESTIONS.filter(
    (q) => countWords(responses[q.id] || "") >= q.minWords,
  ).length;
};
