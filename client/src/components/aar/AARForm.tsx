...
import {
  AARFormHeader,
  AARValidationSummary,
  AARFormActions,
} from "./AARFormComponents";
      ...
      <div className="bg-slate-800 rounded-lg shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-8">
          <AARTextField
            ...
            minWords={20}
            value={formData.whatWasAccomplished}
            onChange={(v) => handleTextChange("whatWasAccomplished", v)}
            showValidation={showValidation}
            validation={validation}
            rows={4}
          />
          <AARListField
            field="whatWorkedWell"
            label="2. What worked well?"
            placeholder="Specific thing that went well"
            minItems={3}
            items={formData.whatWorkedWell}
            onItemChange={(i, v) =>
              handleListItemChange("whatWorkedWell", i, v)
            }
            onAddItem={() => addListItem("whatWorkedWell")}
            onRemoveItem={(i) => removeListItem("whatWorkedWell", i)}
            showValidation={showValidation}
            validation={validation}
          />
          <AARListField
            field="whatDidNotWork"
            label="3. What didn't work?"
            placeholder="Specific problem or struggle encountered"
            minItems={2}
            items={formData.whatDidNotWork}
            onItemChange={(i, v) =>
              handleListItemChange("whatDidNotWork", i, v)
            }
            onAddItem={() => addListItem("whatDidNotWork")}
            onRemoveItem={(i) => removeListItem("whatDidNotWork", i)}
            showValidation={showValidation}
            validation={validation}
          />
          <AARTextField
            field="whyDidNotWork"
            label="4. Why didn't it work?"
            placeholder="Analyze the root causes, not just symptoms. What was the underlying reason for the failures?"
            minWords={15}
            value={formData.whyDidNotWork}
            onChange={(v) => handleTextChange("whyDidNotWork", v)}
            showValidation={showValidation}
            validation={validation}
            rows={4}
          />
          <AARTextField
            field="whatWouldIDoDifferently"
            ...
            onChange={(v) => handleTextChange("whatWouldIDoDifferently", v)}
            showValidation={showValidation}
            validation={validation}
            rows={4}
          />
          <AARTextField
            ...
            onChange={(v) => handleTextChange("whatDidILearn", v)}
            showValidation={showValidation}
            validation={validation}
            rows={4}
          />
          ...
          {submitError && (
            <div
              className="p-4 rounded-md bg-red-900/50 border border-red-700"
              role="alert"
            >
