...
const markdownComponents: Components = {
  h1: ({ children }) => (
    <h1 className="text-2xl font-bold text-purple-400 mb-4">{children}</h1>
  ),
  h2: ({ children }) => (
    <h2 className="text-xl font-semibold text-purple-300 mb-3">{children}</h2>
  ),
  h3: ({ children }) => (
    <h3 className="text-lg font-semibold text-purple-200 mb-2">{children}</h3>
  ),
  p: ({ children }) => (
  ...
  pre: ({ children }) => (
    <pre className="bg-slate-800 p-4 rounded-lg overflow-x-auto mb-3">
      {children}
    </pre>
  ),
  blockquote: ({ children }) => (
    <blockquote className="border-l-4 border-purple-500 pl-4 italic text-slate-300 mb-3">
      {children}
    </blockquote>
  ),
  strong: ({ children }) => (
  ...
  return (
    <div className="bg-slate-800 rounded-lg p-4 border border-slate-700 flex items-center justify-between">
      <div className="flex items-center space-x-6">
        <div
          className={`flex items-center space-x-2 ${isOverTime ? "text-red-400" : "text-yellow-400"}`}
        >
          <Clock className="w-5 h-5" />
          <span className="font-mono text-lg">{formatTime(elapsedTime)}</span>
          {timeTarget && (
            <span className="text-slate-500 text-sm">
              / {timeTarget}:00 target
            </span>
          )}
        </div>
        ...
        {successCriteria.map((criteria, i) => (
          ...
          >
            <label className="flex items-start space-x-3 cursor-pointer w-full">
              ...
              <span
                className={
                  checkedCriteria.has(i) ? "text-green-300" : "text-slate-300"
                }
              >
                {criteria}
              </span>
      ...
      <div className="flex items-start space-x-3">
        ...
        <div>
          ...
          <ul className="text-sm text-amber-200 space-y-1">
            ...
            <li>
              • Your response should demonstrate mastery, not just completion
            </li>
            <li>
              • This is your final test - no hints, no guidance, just your
              knowledge
            </li>
          </ul>
        </div>
