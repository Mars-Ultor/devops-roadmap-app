      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <div className="text-sm text-yellow-400">
            {strugglesLogged ? (
              <span className="flex items-center"><CheckCircle className="w-4 h-4 mr-1" />Struggles logged</span>
            ) : (
              <span className="flex items-center"><AlertCircle className="w-4 h-4 mr-1" />Documentation required</span>
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="p-4">
      <button onClick={onClick}
        ...
        <FileText className="w-5 h-5 mr-2" />Document Your Struggle (Required)
      </button>
      <p className="text-center text-slate-400 text-xs mt-2">Must document attempts before hints unlock</p>
    </div>
  );
}
  ...
  return (
    <div className="p-6 bg-slate-800/50 space-y-4">
      ...
      <div className="flex gap-3">
        <button onClick={onCancel} className="flex-1 px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition-all">Cancel</button>
        <button onClick={onSubmit} className="flex-1 px-4 py-2 bg-yellow-600 hover:bg-yellow-700 text-white font-medium rounded-lg transition-all">Submit Documentation</button>
      </div>
    </div>
  );
}
