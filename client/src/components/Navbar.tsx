...
import { DesktopNavLink, MobileNavMenu, UserStats } from './navbar/NavbarComponents';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-4">
            {NAV_LINKS.map((link) => <DesktopNavLink key={link.to} link={link} />)}
          </div>

          {/* User Info and Logout */}
          <div className="flex items-center space-x-4">
            <UserStats currentWeek={user?.currentWeek} totalXP={user?.totalXP} className="hidden sm:block" />
        ...
        <MobileNavMenu isOpen={isMobileMenuOpen} onClose={closeMobileMenu}
          currentWeek={user?.currentWeek} totalXP={user?.totalXP} />
      </div>
    </nav>
  );
}
