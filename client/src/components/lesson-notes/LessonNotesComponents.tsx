    ...
    <div className="min-h-[12rem] max-h-[32rem] overflow-y-auto px-4 py-3 bg-slate-900 text-white rounded border border-slate-600">
      <ReactMarkdown
        components={{
          ...
          h5: (props) => (
            <h5
              className="text-base font-semibold text-white mb-1 mt-2"
              {...props}
            />
          ),
          h6: (props) => (
          ...
          code: ({
            ...
            return isInline ? (
              <code
                className="text-indigo-300 bg-slate-800 px-2 py-1 rounded text-sm font-mono"
                {...props}
              >
                {children}
              </code>
              ...
              <code
                className="block text-indigo-300 bg-slate-800 p-4 rounded text-sm font-mono overflow-x-auto my-4"
                {...props}
              >
                {children}
              </code>
      ...
      >
        {content ||
          "*No content to preview. Switch to Edit mode to write notes.*"}
      </ReactMarkdown>
    </div>
  );
}
...
export function NotesFooter() {
  return (
    <div className="mt-3 text-xs text-slate-400">
      💾 Auto-saves every 2 seconds | 📝 Use Markdown syntax for formatting | 👁️
      Toggle preview to see rendered markdown | 📥 Export to share or blog
    </div>
  );
}
