...
import { DrillLoadingScreen, DrillBlockedHeader, TrainingBlockedWarning, WhyDrillsMatter, StatusCheckInfo, StartDrillButton } from './daily-drill/DailyDrillBlockerComponents';
...
export default function DailyDrillBlocker({ children }: DailyDrillBlockerProps) {
  ...
  const checkDrillStatus = useCallback(async () => {
    if (!user || isExemptRoute()) { setIsBlocked(false); setLoading(false); return; }
    try {
  ...
  useEffect(() => { checkDrillStatus(); }, [checkDrillStatus]);
