    ...
    <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-white font-semibold flex items-center">
          <Lightbulb className="w-5 h-5 mr-2 text-yellow-400" />Hints ({viewedHints.length}/{hints.length})
        </h3>
        {solutionAvailable && (
          <span className="text-green-400 text-sm font-medium flex items-center">
            <CheckCircle2 className="w-4 h-4 mr-1" />Solution Available
          </span>
        )}
      </div>
      <div className="w-full bg-slate-700 rounded-full h-2 mb-4">
        <div data-testid="hint-progress-bar" className="bg-yellow-500 h-2 rounded-full transition-all duration-300"
          style={{ width: `${(viewedHints.length / hints.length) * 100}%` }} />
      </div>
      <div className="grid grid-cols-3 gap-2">
        {hints.map(hint => (
          <div key={hint.id} className={`text-center py-2 rounded text-sm ${
            viewedHints.includes(hint.id) ? 'bg-green-900/30 text-green-400 border border-green-700' : 'bg-slate-700 text-slate-500 border border-slate-600'
          }`}>
            {viewedHints.includes(hint.id) ? <CheckCircle2 className="w-4 h-4 mx-auto" /> : <Lock className="w-4 h-4 mx-auto" />}
  ...
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-lg p-6">
      <div className="flex items-start justify-between mb-4">
        <div>
          <h4 className="text-white font-semibold mb-1">Hint {hint.id} of {totalHints}</h4>
          <p className="text-sm text-slate-400">Difficulty: <span className={`font-medium ${difficultyClass}`}>{hint.difficulty}</span></p>
        </div>
      ...
      {canViewNextHint ? (
        <div>
          <button onClick={() => onViewHint(hint.id)}
            ...
            <Lightbulb className="w-5 h-5 mr-2" />View Hint {hint.id}
          </button>
          <p className="text-center text-slate-400 text-xs mt-2">Next hint will be available in 5 minutes</p>
        </div>
      ) : (
        <div className="bg-slate-700/50 border border-slate-600 rounded-lg p-4 text-center">
          <Clock className="w-8 h-8 text-slate-500 mx-auto mb-2" />
          <p className="text-slate-400 text-sm font-medium">{nextHintAvailableIn > 0 ? `Next hint available in ${formatTime(nextHintAvailableIn)}` : 'Hint locked'}</p>
          <p className="text-slate-500 text-xs mt-1">Use this time to try different approaches</p>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-lg p-6">
      <h4 className="text-white font-semibold mb-4 flex items-center">
        <CheckCircle2 className="w-5 h-5 mr-2 text-green-400" />Previously Viewed Hints
      </h4>
          ...
          return (
            <div key={hint.id} className="bg-slate-700/50 border border-slate-600 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`text-xs px-2 py-1 rounded ${bgClass}`}>{hint.difficulty}</span>
              </div>
    ...
    <div className="bg-yellow-900/20 border border-yellow-700 rounded-lg p-6">
      <div className="flex items-start">
        ...
        <div>
          ...
          <p className="text-yellow-300 text-sm mb-3">You've viewed all available hints. The full solution will unlock after 90 minutes total lab time.</p>
          <p className="text-yellow-400 text-sm font-medium">Time remaining: {formatTime(getTimeUntilSolution())}</p>
        </div>
      </div>
    </div>
  );
...
export function SolutionAvailableBanner() {
  return (
    <div className="bg-green-900/20 border border-green-700 rounded-lg p-6">
      ...
      <p className="text-green-300 text-sm mb-4">You've struggled for 90 minutes. The complete solution is now available.</p>
      <button className="w-full px-6 py-3 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg transition-all">View Full Solution</button>
    </div>
  );
}
