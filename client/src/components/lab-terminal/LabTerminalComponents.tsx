...
export function writeLabWelcome(term: { writeln: (s: string) => void }, tasks: string[]) {
  term.writeln('Welcome to DevOps Learning Lab!');
  term.writeln('Complete the tasks below to earn XP.\n');
  term.writeln('Tasks:');
  tasks.forEach((task, idx) => { term.writeln(`  ${idx + 1}. ${task}`); });
  term.writeln('');
}

export function writeLabPrompt(term: { write: (s: string) => void }, currentDir: string) {
