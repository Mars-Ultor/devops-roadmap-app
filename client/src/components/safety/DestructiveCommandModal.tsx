...
export const DestructiveCommandModal: FC<DestructiveCommandModalProps> = ({
  ...
  const getSeverityColor = () => {
    switch (command.severity) {
      case "high":
        return "from-red-900 to-orange-900 border-red-500";
      case "medium":
        return "from-amber-900 to-yellow-900 border-amber-500";
      case "low":
        return "from-yellow-900 to-amber-900 border-yellow-500";
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
      <div className="bg-slate-800 rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div
          className={`sticky top-0 bg-gradient-to-r ${getSeverityColor()} p-6 flex items-start justify-between`}
        >
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <p className="text-red-100 text-sm mt-1">
                  Safety checklist required before execution
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 mt-3">
              ...
              <span
                className={`px-3 py-1 rounded-full text-xs font-semibold uppercase ${getSeverityBadge()}`}
              >
            ...
            <div className="bg-blue-900/20 border border-blue-500/50 rounded-lg p-4">
              ...
              <ul className="space-y-2">
                {command.alternatives.map((alt) => (
                  <li
                    key={alt}
                    className="flex items-start gap-2 text-blue-200"
                  >
            ...
            <div className="space-y-3">
              {command.checklistItems.map((item, index) => (
                ...
                >
                  ...
                  />
                  <span
                    className={`flex-1 ${checkedItems[index] ? "text-emerald-300" : "text-slate-300"}`}
                  >
                    {item}
                  </span>
                  {checkedItems[index] && (
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-lg p-4">
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <span className="flex-1 text-white font-semibold">
                I understand the risks and have completed all safety checks. I
                accept full responsibility for executing this command.
              </span>
            </label>
          </div>
        </div>
