...
export function getDifficultyStyle(difficulty: Challenge['difficulty']): string {
  const styles = {
...
const CHALLENGE_TEMPLATES: Challenge[] = [
  {
    ...
    hints: ['Check container logs with docker logs', 'Inspect file permissions inside the container', 'Review the Dockerfile or docker run command'],
    successCriteria: ['Identified the root cause of the crash', 'Container runs without restarting for 2+ minutes', 'Documented the fix in a comment or file'],
    timeLimit: 300, difficulty: 'easy'
  },
  {
    id: 'docker-network-1',
    ...
    hints: ['Check if containers are on the same network', 'Verify exposed ports match connection attempts', 'Test connectivity with docker exec and curl'],
    successCriteria: ['Created or used a Docker network', 'Both containers connected to the network', 'Frontend successfully calls backend API'],
    timeLimit: 300, difficulty: 'medium'
  },
  {
    ...
    hints: ['Check pod logs and events', 'Review ConfigMaps and Secrets', 'Check resource limits and requests'],
    successCriteria: ['Found the root cause of crashes', 'All pods running successfully', 'Service is accessible'],
    timeLimit: 300, difficulty: 'hard'
  },
  {
    ...
    hints: ['Use git status to see conflicting files', 'Look for conflict markers <<< === >>>', 'Test the application after resolving'],
    successCriteria: ['All merge conflicts resolved', 'Code compiles/runs successfully', 'Merge completed to main branch'],
    timeLimit: 300, difficulty: 'easy'
  },
  {
    ...
    hints: ['Check for missing dependencies', 'Review pipeline configuration file', 'Verify test environment setup'],
    successCriteria: ['Identified the failing test', 'Fixed the dependency issue', 'Pipeline runs green end-to-end'],
    timeLimit: 300, difficulty: 'medium'
  }
];

/** Adjust difficulty based on week */
function adjustDifficulty(week: number, baseDifficulty: Challenge['difficulty']): Challenge['difficulty'] {
...
export function generateRandomChallenge(week: number): Challenge {
  ...
  return { ...template, difficulty: adjustDifficulty(week, template.difficulty) };
}
