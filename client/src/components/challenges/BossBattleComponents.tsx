        ...
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h2 className="text-3xl font-bold text-white">
                Week {week} Boss Battle
              </h2>
              <p className="text-slate-300 text-sm mt-1">
                {battle?.title || "Unknown Battle"}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-4 mt-4">
            ...
            <div className="bg-slate-900/50 rounded-lg px-4 py-2">
              ...
              <div
                className={`text-2xl font-bold ${currentScore >= (battle?.minimumPassScore || 0) ? "text-emerald-400" : "text-amber-400"}`}
              >
                {currentScore}%
              </div>
            </div>
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 className="text-red-300 font-semibold mb-1">
            WARNING: This is a Boss Battle
          </h3>
          <ul className="text-sm text-red-200 space-y-1">
            <li>
              • You have {(battle?.timeLimit || 0) / 3600} hours to complete
              this challenge
            </li>
            <li>
              • You must score at least {battle?.minimumPassScore || 0}% to pass
            </li>
            <li>
              • You CANNOT advance to Week {week + 1} until you pass this battle
            </li>
    ...
    <div>
      ...
      <div className="bg-slate-900 rounded-lg p-4">
        <p className="text-slate-300 whitespace-pre-line mb-4">
          {battle?.scenario || "No scenario available"}
        </p>
        <div className="bg-blue-900/20 border border-blue-500/50 rounded-lg p-3">
          <p className="text-blue-200 font-semibold">
            Objective: {battle?.objective || "No objective available"}
          </p>
        </div>
      </div>
    </div>
      ...
      <div className="space-y-2">
        {(phase?.tasks || []).map((task, taskIndex) => (
          ...
          >
            ...
            />
            <span
              className={`text-sm ${phaseCompletion[phaseIndex]?.[taskIndex] ? "text-emerald-300 line-through" : "text-slate-300"}`}
            >
              {task}
            </span>
          </label>
...
export function BattleFooter({
  ...
  const totalTasks =
    battle?.phases?.reduce((sum, p) => sum + (p?.tasks?.length || 0), 0) || 0;
      ...
      {isActive ? (
            ...
            <div className="text-xs text-slate-500 mt-1">
              {isPassing ? (
                ...
                <span className="text-amber-400 flex items-center gap-1">
                  <XCircle className="w-4 h-4" /> Need{" "}
                  {battle.minimumPassScore - currentScore}% more to pass
                </span>
              )}
            </div>
          </div>
        ...
        <div className="flex justify-between items-center">
          <p className="text-sm text-slate-400">
            Ready to begin? Make sure you have {battle.timeLimit / 3600} hours
            available.
          </p>
          <button
            onClick={onStart}
