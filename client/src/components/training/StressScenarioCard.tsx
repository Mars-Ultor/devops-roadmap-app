    ...
    >
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        {scenario.conditions.map((condition) => (
          ...
          >
            {condition.type === "time-pressure" && (
              <Clock className="w-3 h-3" />
            )}
            {condition.type === "multi-tasking" && (
              <Users className="w-3 h-3" />
            )}
            {condition.type === "production-incident" && (
              <AlertTriangle className="w-3 h-3" />
            )}
            {condition.type === "interruption" && <Zap className="w-3 h-3" />}
            {condition.type === "resource-constraint" && (
      ...
      {locked && (
        <div className="mt-3 pt-3 border-t border-gray-700">
          <p className="text-xs text-gray-400">
            🔒 Complete {getUnlockRequirement(String(scenario.stressLevel))}{" "}
            stress scenarios to unlock
          </p>
        </div>
      )}
    </button>
