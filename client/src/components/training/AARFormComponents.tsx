    ...
    <div className="p-6 bg-indigo-900/30 border-b border-indigo-700">
      <div className="flex items-start gap-3">
        ...
        <div className="text-sm text-indigo-100">
          ...
          <p>
            After Action Reviews are critical for deep learning. This structured
            reflection helps you internalize lessons, identify patterns, and
            build transferable expertise. Complete all 6 questions to proceed.
          </p>
        </div>
      </div>
    </div>
...
export function TextFieldWithCount({
  ...
  return (
    <div>
      ...
      />
      <div
        className={`text-xs mt-1 ${isValid ? "text-green-400" : "text-slate-500"}`}
      >
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="text-sm text-red-200">
          ...
          <ul className="list-disc ml-4 space-y-1">
            {objWords < MIN_OBJECTIVE_WORDS && (
              <li>
                Objective needs {MIN_OBJECTIVE_WORDS - objWords} more words
              </li>
            )}
            {validWorks < MIN_ITEMS && (
              <li>Add {MIN_ITEMS - validWorks} more success items</li>
            )}
            {validNotWorks < MIN_ROOT_CAUSES && (
              <li>
                Add {MIN_ROOT_CAUSES - validNotWorks} more challenge items
              </li>
            )}
            {validRoots < MIN_ROOT_CAUSES && (
              <li>Add {MIN_ROOT_CAUSES - validRoots} more root causes</li>
            )}
            {validImprovs < MIN_IMPROVEMENTS && (
              <li>Add {MIN_IMPROVEMENTS - validImprovs} more improvements</li>
            )}
            {knowledgeWords < MIN_KNOWLEDGE_WORDS && (
              <li>
                Transferable knowledge needs{" "}
                {MIN_KNOWLEDGE_WORDS - knowledgeWords} more words
              </li>
            )}
          </ul>
        </div>
