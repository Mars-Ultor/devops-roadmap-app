    ...
    <>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-red-100 text-sm">
              You must complete this AAR before proceeding
            </p>
          </div>
        </div>
      </div>
      <div className="bg-slate-900 border border-slate-700 rounded-lg p-4 mb-6 mx-6 mt-6">
        ...
        <p className="text-gray-400 text-sm mt-2">
          Reflection is critical for learning. Answer each question thoroughly -
          minimum word counts are enforced.
        </p>
      </div>
    </>
  );
      ...
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-amber-200 text-xs mt-2">
            Be specific! Include technical details like error messages,
            commands, root causes, and actionable steps.
          </p>
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      <label className="block text-white font-semibold mb-2">
        {label} <span className="text-red-400">*</span>
        <span className="text-gray-400 text-sm font-normal ml-2">
          (min {minItems} items, {minWords}+ words each)
        </span>
      </label>
      {items.map((item, idx) => (
  ...
  return (
    <div className="flex items-center justify-between mt-8 pt-6 border-t border-slate-700">
      <p className="text-gray-400 text-sm">
        <span className="text-red-400">*</span> You must complete this AAR to
        proceed
      </p>
      <button
        onClick={onSubmit}
