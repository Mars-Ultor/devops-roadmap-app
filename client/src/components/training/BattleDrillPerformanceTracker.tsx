...
const PerformanceAnalytics = memo(
  ({
    trendAnalysis,
    formatTime,
  }: {
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-green-900/20 border border-green-600 p-3 rounded">
              ...
              <div className="text-white font-bold">
                {formatTime(trendAnalysis.bestTime)}
              </div>
            </div>
            <div className="bg-green-900/20 border border-green-600 p-3 rounded">
              <div className="flex items-center gap-2 text-green-400 text-sm mb-1">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`px-3 py-1 rounded-full text-sm font-semibold ${getProficiencyBadge(trendAnalysis.proficiencyLevel).color}`}
                >
                  {getProficiencyBadge(trendAnalysis.proficiencyLevel).icon}{" "}
                  {trendAnalysis.proficiencyLevel.toUpperCase()}
                </span>
              </div>
            </div>
...
function BattleDrillPerformanceTracker({
  ...
  const analyzeTrendsFromHistory = useCallback(
    (history: PerformanceData[]) => {
      ...
      const avgTime =
        recentAttempts.reduce((sum, p) => sum + p.timeSeconds, 0) /
        recentAttempts.length;
      const avgScore =
        recentAttempts.reduce((sum, p) => sum + p.score, 0) /
        recentAttempts.length;
      ...
      if (recentAttempts.length >= 3) {
        ...
        if (older3.length >= 3) {
          const recentAvg =
            recent3.reduce((sum, p) => sum + p.score, 0) / recent3.length;
          const olderAvg =
            older3.reduce((sum, p) => sum + p.score, 0) / older3.length;
      ...
      const recertificationDays: Record<
        TrendAnalysis["proficiencyLevel"],
        number
      > = {
        novice: 7,
        competent: 14,
      ...
      const daysUntilRecertification =
        recertificationDays[proficiencyLevel] - daysSinceLastAttempt;
      ...
      setTrendAnalysis((prev) => {
        ...
        if (
          ...
          prev.daysUntilRecertification ===
            newAnalysis.daysUntilRecertification &&
