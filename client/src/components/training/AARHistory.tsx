...
const AARSection: FC<{ title: string; color: string; items: string[] }> = ({
  title,
  color,
  items,
}) => (
...
export default function AARHistory({
  ...
  useEffect(() => {
    async function loadAARs() {
      setLoading(true);
      try {
        setAars(await getAARs(contentId, contentType));
      } catch (error) {
    ...
    <div className="bg-slate-800 rounded-lg border border-slate-700 p-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-lg font-bold text-white">
          Past After Action Reviews ({aars.length})
        </h3>
      </div>
      <div className="space-y-3">
        {aars.map((aar) => (
          <AARCard
            key={aar.id}
            aar={aar}
            isExpanded={expandedId === aar.id}
            onToggle={() =>
              setExpandedId(expandedId === aar.id ? null : aar.id)
            }
          />
        ))}
      </div>
