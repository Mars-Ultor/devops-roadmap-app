...
import {
  ...
  getTypeLabel,
  getPriorityColor,
} from "./EnhancedAICoachPanelUtils";
  ...
  return (
    <div className="mb-4 space-y-2">
      {alerts.map((alert) => (
        <div
          key={alert}
          className="bg-red-900/50 border border-red-700 rounded p-3"
        >
...
export function FeedbackHeader({ feedback }: FeedbackHeaderProps) {
  return (
    <div className="flex items-center gap-2 mb-2">
      <span className={`font-bold text-lg ${getIconColor(feedback)}`}>
        {getTypeLabel(feedback)}
      </span>
      {feedback?.priority && feedback.priority !== "low" && (
        <span
          className={`px-2 py-1 text-xs font-bold rounded ${getPriorityColor(feedback.priority)} text-white uppercase`}
        >
          {feedback.priority}
        </span>
      )}
      ...
      {analysis.issues.length > 0 && (
        <div className="space-y-2">
          {analysis.issues.map((issue) => (
            <div
              key={issue.message}
              className={`p-3 rounded text-sm ${getIssueSeverityStyles(issue.severity)}`}
            >
        ...
        <div className="space-y-2">
          ...
          {analysis.suggestions.map((suggestion) => (
            <div
              ...
              className="p-2 bg-blue-900/20 border border-blue-700/50 rounded text-sm"
            >
