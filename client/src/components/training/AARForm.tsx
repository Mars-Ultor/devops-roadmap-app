      ...
      <div className="bg-slate-800 rounded-xl border-2 border-indigo-500 max-w-4xl w-full my-8">
        ...
        >
          ...
          <ArrayInputField
            ...
            items={form.whatWorked}
            minItems={MIN_ITEMS}
            onItemChange={form.updateWhatWorked}
            onAddItem={form.addWhatWorked}
            placeholder={(i) =>
              `Success #${i + 1}: What approach, tool, or technique worked?`
            }
            addLabel="Add another success"
          />
          <ArrayInputField
            label={`3. What didn't work? (minimum ${MIN_ROOT_CAUSES} items)`}
            items={form.whatDidntWork}
            minItems={MIN_ROOT_CAUSES}
            onItemChange={form.updateWhatDidntWork}
            onAddItem={form.addWhatDidntWork}
            placeholder={(i) =>
              `Challenge #${i + 1}: What failed or didn't go as expected?`
            }
            addLabel="Add another challenge"
          />
          <ArrayInputField
            label={`4. Why didn't it work? Root causes (minimum ${MIN_ROOT_CAUSES} items)`}
            items={form.rootCauses}
            minItems={MIN_ROOT_CAUSES}
            maxItems={5}
            onItemChange={form.updateRootCauses}
            onAddItem={form.addRootCauses}
            placeholder={(i) =>
              `Root cause #${i + 1}: Explain the underlying reason for the failure...`
            }
            addLabel="Add another root cause"
            isTextarea
          />
          <ArrayInputField
            label={`5. What would you do differently next time? (minimum ${MIN_IMPROVEMENTS} items)`}
            items={form.improvements}
            minItems={MIN_IMPROVEMENTS}
            maxItems={5}
            onItemChange={form.updateImprovements}
            onAddItem={form.addImprovements}
            placeholder={(i) =>
              `Improvement #${i + 1}: Specific action you'll take differently...`
            }
            addLabel="Add another improvement"
            isTextarea
          />
          <TextFieldWithCount
            label={`6. What knowledge can you use in future situations? (minimum ${MIN_KNOWLEDGE_WORDS} words)`}
            value={form.transferableKnowledge}
            onChange={form.setTransferableKnowledge}
            placeholder="Describe the principles, patterns, or insights you can apply to other problems..."
            minWords={MIN_KNOWLEDGE_WORDS}
            rows={4}
          />
          <ValidationMessages
            ...
            transferableKnowledge={form.transferableKnowledge}
          />
        </form>
        <AARFormActions
          isValid={form.isValid}
