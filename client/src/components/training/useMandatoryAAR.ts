...
import { type AARData, INITIAL_FORM_DATA, validateAARForm } from './MandatoryAARUtils';
...
export function useMandatoryAAR({ onSubmit, onClose }: UseMandatoryAARProps) {
  ...
  const addWorkItem = () => updateField('whatWorked', [...formData.whatWorked, '']);
  const addDidntWorkItem = () => updateField('whatDidntWork', [...formData.whatDidntWork, '']);
