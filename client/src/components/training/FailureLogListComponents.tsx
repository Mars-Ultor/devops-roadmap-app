      ...
      <div className="bg-slate-800 rounded-lg p-4 border border-slate-700">
        <div className="text-2xl font-bold text-indigo-400">
          {stats.avgResolutionTime > 0
            ? Math.round(stats.avgResolutionTime)
            : "-"}
        </div>
        ...
        <div className="flex-1 grid grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                onCategoryChange(e.target.value as FailureCategory | "all")
              }
...
export function FailureItem({ failure, onResolve }: FailureItemProps) {
  ...
  return (
    <div
      className={`bg-slate-800 rounded-lg p-5 border transition-all ${borderClass}`}
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          ...
          <div className="flex items-center gap-3 text-sm mb-2">
            <span
              className={`px-2 py-1 rounded ${getSeverityColor(failure.severity)}`}
            >
              {failure.severity}
            </span>
