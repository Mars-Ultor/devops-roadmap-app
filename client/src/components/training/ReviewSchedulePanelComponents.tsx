...
export function DailyLoadSummary({
  dailyLoad,
  dueCount,
}: DailyLoadSummaryProps) {
  return (
...
function ReviewItemHeader({ review }: { readonly review: ReviewSchedule }) {
  return (
    <div className="flex items-center gap-2 mb-2">
      ...
      <span
        className={`px-2 py-0.5 rounded text-xs font-semibold ${getPriorityBadge(review.priority)}`}
      >
...
export function ReviewsDueToday({
  reviews,
  onReviewClick,
}: ReviewsDueTodayProps) {
...
export function UpcomingWeek({ reviews }: UpcomingWeekProps) {
  const upcomingReviews = reviews.filter(
    (r) => r.daysUntilReview > 0 && r.daysUntilReview <= 7,
  );
...
function DayColumn({ day, reviews }: DayColumnProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`text-lg font-bold ${getDayCountColor(dayReviews.length)}`}
      >
        {dayReviews.length}
      </div>
    </div>
