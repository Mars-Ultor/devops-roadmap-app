...
import { TrendingUp, TrendingDown, Target, Clock, Zap, Shield, Award, AlertTriangle, CheckCircle, BarChart3 } from 'lucide-react';
import type { PerformanceAnalytics, LearningPath } from '../../types/aiCoach';
import { getMetricColor, formatTime, getTrendStatus } from './PerformanceDashboardUtils';
...
export function HeaderSection({ analytics }: HeaderSectionProps) {
  ...
  const TrendIcon = analytics.trends.improving.length > 0 ? TrendingUp : TrendingDown;
  return (
    <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <TrendIcon className={`w-5 h-5 ${trendStatus.improving ? 'text-green-400' : 'text-amber-400'}`} />
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      <MetricCard icon={<Target className="w-4 h-4 text-blue-400" />} label="Attempts"
        ...
        color={getMetricColor(analytics.metrics.averageAttempts, { good: 2, warning: 4 })} />
      <MetricCard icon={<Clock className="w-4 h-4 text-green-400" />} label="Avg Time"
        value={formatTime(analytics.metrics.averageTime)}
        color={getMetricColor(analytics.metrics.averageTime, { good: 600, warning: 1800 })} />
      <MetricCard icon={<Zap className="w-4 h-4 text-purple-400" />} label="Hint Usage"
        value={`${(analytics.metrics.hintDependency * 100).toFixed(0)}%`}
        color={getMetricColor(1 - analytics.metrics.hintDependency, { good: 0.7, warning: 0.4 })} />
      <MetricCard icon={<Shield className="w-4 h-4 text-red-400" />} label="Persistence"
        value={`${(analytics.metrics.persistenceScore * 100).toFixed(0)}%`}
        color={getMetricColor(analytics.metrics.persistenceScore, { good: 0.7, warning: 0.4 })} />
    </div>
  );
}
  ...
  return (
    <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
      <h4 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
        <TrendingUp className="w-5 h-5 text-blue-400" />Performance Trends
      </h4>
      <div className="space-y-3">
        {analytics.trends.improving.length > 0 && (
          <TrendItem icon={<CheckCircle className="w-5 h-5 text-green-400" />} label="Improving:" color="text-green-400" items={analytics.trends.improving} />
        )}
        {analytics.trends.declining.length > 0 && (
          <TrendItem icon={<AlertTriangle className="w-5 h-5 text-amber-400" />} label="Needs Attention:" color="text-amber-400" items={analytics.trends.declining} />
        )}
        {analytics.trends.plateaued.length > 0 && (
          <TrendItem icon={<Target className="w-5 h-5 text-blue-400" />} label="Plateaued:" color="text-blue-400" items={analytics.trends.plateaued} />
        )}
      </div>
    </div>
  );
...
function TrendItem({ icon, label, color, items }: { readonly icon: React.ReactNode; readonly label: string; readonly color: string; readonly items: string[] }) {
  return (
    <div className="flex items-start gap-3">
      ...
      <div><div className={`${color} font-medium`}>{label}</div><div className="text-slate-300 text-sm">{items.join(', ')}</div></div>
    </div>
  );
}
...
export function LearningPathSection({ learningPath }: { readonly learningPath: LearningPath }) {
  return (
    <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
      <h4 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
        <Award className="w-5 h-5 text-purple-400" />Learning Path
      </h4>
