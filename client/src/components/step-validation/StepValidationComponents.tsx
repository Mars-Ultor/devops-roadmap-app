...
export function RunValidationButton({ isRunning, isCompleted, onClick }: RunValidationButtonProps) {
  return (
    <button onClick={onClick} disabled={isRunning || isCompleted}
      ...
      {isRunning ? 'Running...' : isCompleted ? 'Completed ✅' : 'Run Validation'}
    </button>
  );
}
...
export function ValidationRuleItem({ rule, isLocked, isRunning, result, statusText }: ValidationRuleItemProps) {
  const borderClass = isLocked ? 'border-gray-700 bg-gray-900/50' 
    : result?.success ? 'border-green-600 bg-green-900/20' 
    : result?.success === false ? 'border-red-600 bg-red-900/20' 
    : 'border-slate-600 bg-slate-900/50';

  const statusClass = isLocked ? 'text-gray-500' : result?.success ? 'text-green-400' : result?.success === false ? 'text-red-400' : 'text-gray-400';
    ...
    <div className={`flex items-start justify-between p-3 rounded-lg border ${borderClass}`}>
      <div className="flex items-start space-x-3 flex-1">
        ...
        <div className="flex-1">
          <div className={`font-medium ${isLocked ? 'text-gray-500' : 'text-white'}`}>{rule.type.replace(/_/g, ' ').toUpperCase()}</div>
