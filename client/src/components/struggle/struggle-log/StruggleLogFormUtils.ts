...
export function validateStruggleLogForm(
  formData: StruggleLogFormData,
): string[] {
  const errors: string[] = [];
  const { MIN_APPROACHES_REQUIRED, MIN_STRUGGLE_TIME_MINUTES } =
    STRUGGLE_SESSION_CONFIG;
  ...
  if (validApproaches.length < MIN_APPROACHES_REQUIRED) {
    errors.push(
      `At least ${MIN_APPROACHES_REQUIRED} different approaches must be documented`,
    );
  }
  ...
  if (formData.timeSpentMinutes < MIN_STRUGGLE_TIME_MINUTES) {
    errors.push(
      `Minimum ${MIN_STRUGGLE_TIME_MINUTES} minutes of struggle required`,
    );
  }
...
export function isStruggleLogFormValid(formData: StruggleLogFormData): boolean {
  ...
  return (
    ...
    formData.approachesTried.filter((a) => a.trim()).length >=
      MIN_APPROACHES_REQUIRED &&
