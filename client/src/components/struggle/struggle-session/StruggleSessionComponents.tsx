...
export type SessionPhase = 'struggling' | 'logging' | 'hint_available' | 'completed';
...
export function SessionCompleteDisplay({ session }: SessionCompleteDisplayProps) {
  return (
...
export function PhaseIndicator({ currentPhase, session }: PhaseIndicatorProps) {
  const getPhaseLabel = () => {
    switch (currentPhase) {
      case 'struggling': return 'Initial Struggle Period';
      case 'logging': return 'Documenting Struggle';
      case 'hint_available': return 'Hint Access Available';
      default: return '';
    }
  };
...
export function CurrentHintDisplay({ hint, hintNumber }: CurrentHintDisplayProps) {
  return (
    <div className="bg-blue-900/50 border border-blue-700 rounded-lg p-4">
      <div className="flex items-start space-x-3">
        <div className="w-2 h-2 bg-blue-400 rounded-full mt-2"></div>
        <div className="flex-1">
          <h4 className="text-sm font-medium text-blue-400 mb-1">Hint #{hintNumber}</h4>
