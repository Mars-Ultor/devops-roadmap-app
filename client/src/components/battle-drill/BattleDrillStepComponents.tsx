...
export function StepNumber({ index, isCompleted }: StepNumberProps) {
  return (
    <div
      className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${isCompleted ? "bg-green-500" : "bg-slate-700"}`}
    >
      {isCompleted ? (
...
export function ValidationCriteria({
  criteria,
  validationResult,
...
export function ValidationFeedback({ result }: ValidationFeedbackProps) {
  return (
    <div
      className={`rounded-lg p-4 border-2 ${result.passed ? "bg-green-900/20 border-green-500/30" : "bg-red-900/20 border-red-500/30"}`}
    >
      <div
        className={`flex items-center gap-2 mb-2 ${result.passed ? "text-green-400" : "text-red-400"}`}
      >
        {result.passed ? (
          <>
      ...
      {result.specificErrors.length > 0 && (
        <div className="mt-3 space-y-1">
          ...
          {result.specificErrors.map((error: string) => (
            <div
              key={error}
