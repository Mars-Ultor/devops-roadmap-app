...
export const StepValidator: FC<StepValidatorProps> = ({
  ...
  const [validationState, setValidationState] = useState<
    Record<string, "pending" | "checking" | "passed" | "failed">
  >({});
    ...
    for (const criterion of criteria) {
      ...
      try {
        ...
        setValidationState((prev) => ({
          ...prev,
          [criterion.id]: passed ? "passed" : "failed",
        }));
  ...
  return (
    <div className="bg-slate-800 rounded-lg border border-slate-700 p-6 space-y-4">
      ...
      <div className="flex items-start justify-between">
        ...
        {attemptNumber > 1 && (
          <div className="text-sm text-slate-400">Attempt #{attemptNumber}</div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-slate-700">
        <div className="text-sm text-slate-400">
          ...
          {anyFailed &&
            !isValidating &&
            "✗ Some checks failed - review and retry"}
      ...
      {!allPassed && attemptNumber > 1 && (
        <div className="bg-amber-900/20 border border-amber-500/30 rounded-lg p-4">
          <p className="text-sm text-amber-300">
            <strong>Next step is locked.</strong> Fix the failed checks above
            before proceeding.
          </p>
        </div>
      )}
