...
export const CopyPasteBlocker: React.FC<CopyPasteBlockerProps> = ({
  ...
  const [blockedActions, setBlockedActions] = useState<
    Array<{ action: "copy" | "paste"; timestamp: Date }>
  >([]);
  ...
  useEffect(() => {
    ...
    const handleCopy = (event: ClipboardEvent) => {
      ...
      setBlockedActions((prev) => [
        ...prev,
        { action: "copy", timestamp: new Date() },
      ]);
    ...
    const handlePaste = (event: ClipboardEvent) => {
      ...
      setBlockedActions((prev) => [
        ...prev,
        { action: "paste", timestamp: new Date() },
      ]);
  ...
  if (!isBlocked) {
    return (
      <div
        className={`flex items-center gap-2 px-3 py-2 rounded-lg bg-green-900/20 border border-green-500 text-green-400 ${className}`}
      >
  ...
  return (
    <div
      className={`p-4 rounded-lg border-2 border-red-500 bg-red-900/20 ${className}`}
    >
      ...
      {blockedActions.length > 0 && (
        <div className="mb-3">
          ...
          >
            ...
            {blockedActions.length} blocked attempt
          ...
          {showBlocked && (
            <div className="mt-2 max-h-32 overflow-y-auto">
              {blockedActions.slice(-10).map((attempt) => (
                <div
                  key={`${attempt.action}-${attempt.timestamp.getTime()}`}
                  className="flex items-center gap-2 text-xs text-red-300 py-1"
                >
