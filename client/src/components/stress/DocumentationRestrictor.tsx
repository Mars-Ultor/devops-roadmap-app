...
export const DocumentationRestrictor: React.FC<
  DocumentationRestrictorProps
> = ({ sessionId, onAccessBlocked, className = "" }) => {
    ...
    <div className={`p-4 rounded-lg border-2 ${getLevelColor()} ${className}`}>
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="font-semibold">
          Documentation Access: {getDocumentationLevel().toUpperCase()}
        </span>
      </div>
      ...
      {blockedAttempts.length > 0 && (
        <div className="mb-3">
          ...
          >
            ...
            {blockedAttempts.length} blocked access attempt
            {blockedAttempts.length === 1 ? "" : "s"}
          </button>
