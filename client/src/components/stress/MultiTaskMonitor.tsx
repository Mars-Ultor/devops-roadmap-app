...
export const MultiTaskMonitor: React.FC<MultiTaskMonitorProps> = ({
  ...
  const initializeTasks = (taskCount: number) => {
    ...
    for (let i = 0; i < taskCount; i++) {
      const scenario =
        STRESS_CONFIG.multiTaskScenarios[
  ...
  const completeTask = (taskId: string) => {
    setTasks((prevTasks) => {
      ...
      const nextPendingTask = updatedTasks.find(
  ...
  return (
    <div
      className={`p-4 rounded-lg border-2 border-orange-500 bg-orange-900/20 ${className}`}
    >
      ...
      <div className="mt-4 p-3 bg-gray-800/50 rounded border border-gray-600">
        ...
        <p className="text-xs text-gray-400">
          ...
          This simulates real-world DevOps scenarios where multiple issues
