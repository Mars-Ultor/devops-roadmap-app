    ...
    <div className="bg-gray-800 border border-gray-700 rounded-lg p-6 mb-8">
      ...
      <div className="bg-blue-900/20 border border-blue-700 rounded-lg p-4">
        <p className="text-sm text-blue-300">
          <span className="font-semibold">AAR Completion Rate:</span>{" "}
          {analytics.totalFailures > 0
            ? Math.round(
                (analytics.aarCompleted / analytics.totalFailures) * 100,
              )
            : 0}
          %
          {analytics.aarCompleted < analytics.totalFailures && (
            <span className="ml-2 text-yellow-300">
              ({analytics.totalFailures - analytics.aarCompleted} failures
              without AAR review)
            </span>
          )}
        </p>
