  ...
  return (
    <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div
          className={`px-3 py-1 rounded-full text-xs font-semibold ${getConfidenceBadge(completionPrediction.confidence)}`}
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div className="bg-slate-900/50 rounded-lg p-4">
            ...
            <div className="text-xs text-slate-500">
              {Math.round(
                (completionPrediction.estimatedCompletionDate.getTime() -
                  Date.now()) /
                  (1000 * 60 * 60 * 24),
              )}{" "}
              days
        ...
        <div className="bg-slate-900/50 rounded-lg p-4 mb-4">
          ...
          <div className="w-full bg-slate-700 rounded-full h-2">
            <div
              ...
              style={{
                width: `${Math.min((completionPrediction.currentPace / completionPrediction.requiredPace) * 100, 100)}%`,
              }}
            />
          </div>
        </div>
              ...
              <div>
                ...
                <ul className="space-y-1">
                  {prediction.preventiveActions.map((action) => (
                    <li
                      key={action}
                      className="text-sm flex items-start gap-2 opacity-85"
                    >
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span className="text-white font-semibold">
                  {performanceForecast.nextWeekPrediction.expectedItems}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400 text-sm">
                ...
                <span className="text-white font-semibold">
                  {performanceForecast.nextWeekPrediction.expectedMastery}%
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400 text-sm">Confidence:</span>
                <span
                  className={`font-semibold ${getConfidenceBadge(performanceForecast.nextWeekPrediction.confidence).split(" ")[1]}`}
                >
          ...
          <div className="bg-slate-900/50 rounded-lg p-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span className="text-white font-semibold">
                  {performanceForecast.monthProjection.totalItems}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400 text-sm">Mastery Rate:</span>
                <span className="text-white font-semibold">
                  {performanceForecast.monthProjection.masteryRate}%
                </span>
              </div>
              <div className="text-sm text-slate-400 mt-2">
                Key improvements:{" "}
                {performanceForecast.monthProjection.skillImprovements.join(
                  ", ",
                )}
              </div>
            </div>
      ...
      <div className="bg-gradient-to-r from-indigo-900/30 to-purple-900/30 border border-indigo-700 rounded-lg p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              className={`font-semibold capitalize ${getTrajectoryColor(learningTrajectory.currentTrajectory)}`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-slate-300 text-sm capitalize">
              {learningTrajectory.optimalTrajectory}
            </p>
          </div>
