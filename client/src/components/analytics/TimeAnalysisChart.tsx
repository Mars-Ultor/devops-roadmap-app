...
export const TimeAnalysisChart: FC<TimeAnalysisChartProps> = ({
  data,
  formatHour,
}) => {
  ...
  const maxSessions = Math.max(
    ...hourlyPerformance.map((h) => h.totalSessions),
    1,
  );
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Based on {totalDataPoints} completed sessions
          </p>
        </div>
        <span
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-slate-300">
                  ...
                  <strong className="text-emerald-300">
                    {recommendation.peakPerformanceWindow}
                  </strong>
                </span>
              </div>
              ...
              {hourData.totalSessions > 0 ? (
                <>
                  <div
                    ...
                    style={{
                      width: `${(hourData.averageScore / maxScore) * 100}%`,
                    }}
                  />
                  <div className="absolute inset-0 flex items-center justify-between px-3">
                    ...
                    <span className="text-xs text-slate-300">
                      {hourData.totalSessions} session
                      {hourData.totalSessions === 1 ? "" : "s"}
                    </span>
                  </div>
                </>
              ) : (
            ...
            <div className="w-20">
              <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${(hourData.totalSessions / maxSessions) * 100}%`,
                  }}
                />
              </div>
            </div>
