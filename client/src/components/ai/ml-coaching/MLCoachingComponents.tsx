          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              {learningStyle.recommendations.slice(0, 2).map((rec) => (
                <p key={rec} className="text-sm text-gray-400">
                  • {rec}
                </p>
              ))}
            </div>
          ...
          <div className="space-y-3">
            ...
            {prediction.riskFactors.length > 0 && (
              <div className="flex items-center gap-2 text-sm text-red-400">
                ...
                {prediction.riskFactors.length} risk factor
                {prediction.riskFactors.length === 1 ? "" : "s"}
              </div>
            )}
          </div>
        ) : (
        ...
        {optimizedPath ? (
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-indigo-400">
                  {Math.round(optimizedPath.confidence * 100)}%
                </p>
              </div>
              <div className="text-right">
                <p className="text-sm text-gray-400">Estimated Time</p>
                <p className="text-2xl font-bold text-white">
                  {optimizedPath.estimatedTime}h
                </p>
              </div>
            </div>
          ...
          <div className="text-center py-8">
            ...
            <p className="text-gray-400">
              Analyzing your optimal learning path...
            </p>
          </div>
        )}
      </div>
    ...
    <div className="bg-gray-800 border border-gray-700 rounded-lg shadow-sm">
      <div className="p-6 border-b border-gray-700">
        <div className="flex items-center justify-between">
          ...
          <span
            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getPriorityBadgeClass(gap.priority)}`}
          >
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-gray-400">
            {Math.ceil(
              (prediction.estimatedCompletionDate.getTime() - Date.now()) /
                (1000 * 60 * 60 * 24),
            )}{" "}
            days remaining
          </p>
        ...
        <div>
          ...
          <div className="space-y-2">
            {prediction.riskFactors.map((risk) => (
              <div
                key={risk}
                className="flex items-start gap-2 p-3 bg-red-900/20 rounded-lg"
              >
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {prediction.interventions.map((intervention) => (
            <div
              key={intervention}
              className="flex items-start gap-2 p-3 bg-green-900/20 rounded-lg"
            >
              <CheckCircle className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" />
...
function LearningStyleContent({
  learningStyle,
}: {
  readonly learningStyle: LearningStyleResult;
}) {
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-400">
          Your learning style has been analyzed based on your interaction
          patterns, performance data, and engagement metrics.
        </p>
      </div>
      ...
      <div className="bg-slate-700 p-4 rounded-lg">
        ...
        <p className="text-sm text-gray-400">
          Tailoring your learning approach to your preferred style can
          significantly improve comprehension, retention, and overall learning
          effectiveness. The AI coach will adapt its recommendations based on
          this analysis.
        </p>
      </div>
    </div>
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-lg shadow-sm">
      <div className="p-6 text-center">
        ...
        <p className="text-gray-400">
          {message ||
            "Please provide coaching context to enable ML-powered insights."}
        </p>
      </div>
    </div>
  );
