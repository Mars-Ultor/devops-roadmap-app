...
import type { RecertificationDrill, RecertificationAttempt, RecertificationAnswer } from '../types/training';
...
export default function RecertificationDrillComponent({ drillId, onComplete, onCancel }: RecertificationDrillProps) {
  ...
  const handleSubmit = useCallback((autoSubmit = false) => {
    ...
    const score = Math.round((correctAnswers / currentDrill.questions.length) * 100);
    const passed = score >= currentDrill.passingScore;
    const attemptResult: RecertificationAttempt = {
      id: `attempt-${Date.now()}`, drillId: currentDrill.id, userId: user.uid,
      startedAt: startTime, completedAt: endTime, score, passed, timeSpentMinutes, answers,
  ...
  const handleAnswer = (answer: string | number | boolean) => {
    ...
    const answerObj: RecertificationAnswer = {
      questionId: question.id, answer, correct: answer === question.correctAnswer,
  ...
  if (showResults && attempt) {
    return <DrillResultsView drill={currentDrill} attempt={attempt} answers={answers}
      onRetry={() => window.location.reload()} onCancel={onCancel} />;
  }

  return <DrillQuizView drill={currentDrill} currentIndex={currentQuestionIndex}
    timeRemaining={timeRemaining} answers={answers} onAnswer={handleAnswer}
    onPrevious={() => setCurrentQuestionIndex(i => Math.max(0, i - 1))}
    onNext={() => setCurrentQuestionIndex(i => Math.min(currentDrill.questions.length - 1, i + 1))}
