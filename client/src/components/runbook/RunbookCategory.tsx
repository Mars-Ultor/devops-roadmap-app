...
import type { RunbookCategory as RunbookCategoryType, RunbookEntry } from '../../services/runbookGenerator';
...
const RunbookEntryCard: FC<RunbookEntryCardProps> = ({ entry, index }) => (
  <div className="p-5 bg-slate-900/30">
    <div className="flex items-start justify-between mb-4">
      ...
      {entry.occurrences > 3 && (
        <span className="px-3 py-1 bg-red-900/30 text-red-400 text-xs font-semibold rounded-full">Frequent Issue</span>
      )}
    </div>
    <div className="mb-4">
      <h5 className="text-sm font-semibold text-red-400 mb-2 flex items-center gap-2"><AlertCircle className="w-4 h-4" />Root Cause</h5>
      <p className="text-slate-300 text-sm bg-red-900/10 border border-red-800/30 rounded-lg p-3">{entry.rootCause}</p>
    </div>
    <div className="mb-4">
      <h5 className="text-sm font-semibold text-green-400 mb-2 flex items-center gap-2"><CheckCircle className="w-4 h-4" />Solution</h5>
      <p className="text-slate-300 text-sm bg-green-900/10 border border-green-800/30 rounded-lg p-3">{entry.solution}</p>
    </div>
    <div className="mb-4">
      <h5 className="text-sm font-semibold text-blue-400 mb-2">Prevention Strategy</h5>
      <p className="text-slate-300 text-sm bg-blue-900/10 border border-blue-800/30 rounded-lg p-3">{entry.prevention}</p>
    </div>
    <div>
      <h5 className="text-sm font-semibold text-purple-400 mb-2">Quick Check Command</h5>
      <div className="bg-slate-950 border border-purple-800/30 rounded-lg p-3">
        <code className="text-purple-300 text-sm font-mono">{entry.quickCheck}</code>
      </div>
    </div>
...
export const RunbookCategory: FC<RunbookCategoryProps> = ({ category, defaultExpanded = false }) => {
