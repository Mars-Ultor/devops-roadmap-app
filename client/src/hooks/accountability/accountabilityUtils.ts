...
export function getCurrentWeekInfo(): { weekNumber: number; weekStart: Date; weekEnd: Date } {
  const now = new Date();
...
export function updateCommitmentWithProgress(
  ...
  const newStatus: CommitmentStatus = newCurrent >= commitment.target ? 'completed' : 'in-progress';
...
export function calculateStreaks(
  docs: Array<{ overallStatus?: string }>
...
): { totalCommitments: number; completedCommitments: number; failedCommitments: number } {
...
export function calculateAccountabilityStats(
  commitmentDocs: Array<{ overallStatus?: string; commitments: Array<{ status: string }> }>,
  checkInsCount: number,
  publicDocs: Array<{ status?: string }>
): AccountabilityStats {
  const allCommitments = commitmentDocs.map(d => d.commitments);
  const { totalCommitments, completedCommitments, failedCommitments } = countCommitmentsByStatus(allCommitments);
  
  const { currentStreak, longestStreak } = calculateStreaks([...commitmentDocs].reverse());
  
  const publicKept = publicDocs.filter(d => d.status === 'completed').length;
  const numWeeks = commitmentDocs.length;

  return {
    weeklyCompletionRate: numWeeks > 0 ? completedCommitments / totalCommitments : 0,
    currentStreak,
    longestStreak,
    totalCommitments,
