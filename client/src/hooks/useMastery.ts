...
import {
  getLevelKey,
  getLevelProgressFromMastery,
  checkCanAccessLevel,
  checkIsLevelMastered,
  updateMasteryAfterAttempt,
} from "./mastery/masteryUtils";
...
export function useMastery(lessonId: string) {
  ...
  const recordAttempt = async (
    ...
    try {
      const { updatedMastery, levelMastered } = updateMasteryAfterAttempt(
        mastery,
        level,
        perfect,
        timeSpent,
      ...
      await setDoc(
        doc(db, "masteryProgress", `${user.uid}_${lessonId}`),
        updatedMastery,
      );
      ...
      return {
        levelMastered,
        nextLevelUnlocked: levelMastered && level !== "run-independent",
  ...
  const getLevelProgress = (level: MasteryLevel): MasteryProgress | null =>
    getLevelProgressFromMastery(mastery, level);
  const canAccessLevel = (level: MasteryLevel): boolean =>
    checkCanAccessLevel(mastery, level);
  const isLevelMastered = (level: MasteryLevel): boolean =>
    checkIsLevelMastered(mastery, level);
