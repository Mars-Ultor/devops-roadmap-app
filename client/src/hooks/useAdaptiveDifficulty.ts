  ...
  const loadDifficultySettings = useCallback(async () => {
    ...
    try {
      ...
      if (settingsDoc.exists()) {
        setCurrentLevel(settingsDoc.data().currentLevel);
        setSettings(settingsDoc.data() as DifficultySettings);
      } else {
        const init = createSettingsForLevel("recruit");
        await setDoc(doc(db, "difficultySettings", user.uid), init);
        setSettings(init);
      }
    } catch {
