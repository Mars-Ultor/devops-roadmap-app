...
import type { StressMetrics, StressTrainingSession } from '../../types/training';
...
export function getInitialStressMetrics(userId: string): StressMetrics {
  return {
    ...
    performanceDegradation: { normalAccuracy: 0, stressedAccuracy: 0, degradationRate: 0 },
    lastUpdated: new Date()
  };
}

/** Calculate running average */
export function calculateRunningAverage(currentAvg: number, newValue: number, count: number): number {
  return ((currentAvg * count) + newValue) / (count + 1);
}

/** Calculate stress tolerance level based on completed sessions */
export function calculateStressTolerance(metrics: StressMetrics, latestSession: StressTrainingSession): string {
...
export function canAttemptLevel(stressMetrics: StressMetrics | null, level: string): boolean {
...
export function calculateSessionAccuracy(tasksCompleted: number, errorsCount: number, totalTasks: number): number {
  return totalTasks > 0 ? ((tasksCompleted - errorsCount) / totalTasks) * 100 : 0;
}

/** Calculate performance degradation rate */
export function calculateDegradationRate(normalAccuracy: number, stressedAccuracy: number): number {
  return normalAccuracy > 0 ? ((normalAccuracy - stressedAccuracy) / normalAccuracy) * 100 : 0;
}
...
export function calculateUpdatedMetrics(
  ...
  return {
    ...
    sessionsByStressLevel: updateSessionsByLevel(metrics.sessionsByStressLevel, session.scenario.stressLevel),
    averageStressScore: calculateRunningAverage(metrics.averageStressScore, session.stressScore, metrics.totalSessions),
    averageAdaptabilityScore: calculateRunningAverage(metrics.averageAdaptabilityScore, session.adaptabilityScore, metrics.totalSessions),
...
export function updateMetricsPerformanceDegradation(
  ...
  return {
    ...
    performanceDegradation: {
      normalAccuracy,
      stressedAccuracy,
      degradationRate: calculateDegradationRate(normalAccuracy, sessionAccuracy)
    }
  };
}
