...
export function calculateMasteryLevel(
  ...
  if (successRate >= 0.75 && bestTime <= targetTimeSeconds * 1.25) return 'proficient';
...
export function calculateUpdatedPerformance(
  ...
  const newBestTime = passed && durationSeconds < current.bestTime 
    ? durationSeconds 
    : current.bestTime;
  ...
  if (passed) {
    const totalTime = current.averageTime * (current.attempts || 0) + durationSeconds;
  ...
  const masteryLevel = calculateMasteryLevel(newSuccessRate, newBestTime, targetTimeSeconds);
...
export function createAttemptResult(
  ...
  return {
    ...
    personalBest: currentBestTime !== undefined && durationSeconds < currentBestTime
  };
}
...
export function getTimeColor(elapsedSeconds: number, targetTimeSeconds: number | undefined): string {
