...
export function analyzeStudyConsistency(
  ...
  const consistencyScore = Math.min(
    ...
    Math.max(
      0,
      avgSessionsPerDay * 20 +
        (totalDays > 7 ? 30 : totalDays * 4) +
        (totalSessions > 10 ? 20 : totalSessions * 2),
    ),
  );
...
function getConsistencyFeedback(score: number): {
  description: string;
  ...
  if (score >= 60)
    return {
      ...
      recommendation: "Try studying at same times daily.",
    };
  if (score >= 40)
    return {
      description: "Inconsistent study patterns detected.",
      recommendation: "Establish a regular schedule.",
    };
  return {
    description: "Study consistency needs improvement.",
    recommendation: "Create a daily routine with reminders.",
  };
}
...
export function analyzeTimeOptimization(
  sessions: { data: () => { startTime?: { toDate: () => Date } } }[],
  progress: {
    data: () => { score?: number; completedAt?: { toDate: () => Date } };
  }[],
): TimeOptimization {
  const hourlyPerformance: Record<
    number,
    { sessions: number; scores: number[] }
  > = {};
  for (let i = 0; i < 24; i++)
  ...
  sessions.forEach((session) => {
    ...
    const progressData = progress.find(
      (p) =>
        p.data().completedAt?.toDate().getTime() ===
        data.startTime?.toDate().getTime(),
    );
    if (progressData)
      hourlyPerformance[hour].scores.push(progressData.data().score || 0);
  });

  const hourlyAvgScores = Object.entries(hourlyPerformance).map(
    ([hour, data]) => ({
      hour: parseInt(hour),
      avgScore:
  ...
  const optimalHours = sortedHours
    .slice(0, 3)
    .filter((h) => h.avgScore > 0)
    .map((h) => h.hour);
  const wastedHours = sortedHours
    .slice(-3)
    .filter((h) => h.sessions > 0 && h.avgScore < 50)
    .map((h) => h.hour);

  const totalSessions = sessions.length;
  const optimalSessions = optimalHours.reduce(
    (sum, h) => sum + (hourlyPerformance[h]?.sessions || 0),
    0,
  );
  const efficiency =
    totalSessions > 0 ? (optimalSessions / totalSessions) * 100 : 0;
...
export function analyzeTopicStruggles(
  ...
  const topicFailures: Record<
    string,
    { count: number; patterns: string[]; lastFailure: Date }
  > = {};
...
export function generateStruggleSuggestion(
  patterns: string[],
  frequency: number,
): string {
  if (patterns.includes("time-pressure"))
    return "Practice time management and break topics into smaller chunks.";
  if (patterns.includes("conceptual"))
    return "Review fundamental concepts and seek additional resources.";
  if (patterns.includes("application"))
    return "Focus on hands-on practice and real-world scenarios.";
  if (frequency > 3)
    return "Consider seeking help from mentors or taking a break.";
...
type PerformanceCorrelations =
  LearningPatternData["patterns"]["performanceCorrelations"];
...
export function analyzePerformanceCorrelations(
  ...
  // Time spent vs score
  const timeScoreData = progress
    .map((p) => ({
      time: p.data().timeSpentMinutes || 0,
  ...
  if (timeScoreData.length > 5) {
    ...
    correlations.push({
      ...
      insight:
        corr > 0.5
          ? "More time correlates with higher scores."
          : corr > 0.2
            ? "Some time-performance correlation."
  ...
  const sessionMasteryData = sessions.map((session) => {
    ...
    const dayProgress = progress.filter(
      (p) =>
        p.data().completedAt?.toDate().toISOString().split("T")[0] ===
        sessionDate,
    );
    const avgMastery =
      dayProgress.length > 0
        ? dayProgress.reduce((s, p) => s + (p.data().score || 0), 0) /
          dayProgress.length
        : 0;
  ...
  if (sessionMasteryData.length > 5) {
    ...
    correlations.push({
      ...
      insight:
        corr > 0.3
          ? "Regular sessions correlate with better mastery."
          : "Focus on quality per session.",
    });
  }
...
export function generateAdaptiveRecommendations(
  ...
  if (timeOpt.optimalHours.length > 0)
    recommendations.push(
      `Study during optimal hours: ${timeOpt.optimalHours.join(", ")}`,
    );
  if (struggles.length > 0)
    recommendations.push(
      "Focus on struggling topics with additional practice.",
    );
  const timeCorr = correlations.find((c) => c.factor === "Time Spent vs Score");
  if (timeCorr && timeCorr.correlation < 0.3)
    recommendations.push("Focus on study quality over quantity.");
  if (recommendations.length === 0)
    recommendations.push("Continue current habits while monitoring progress.");
...
export function generateInsights(
  ...
  if (consistency.score >= 80)
    insights.push({
      ...
      description:
        "Your consistent routine is contributing to excellent outcomes.",
      actionable: false,
    });
    ...
    insights.push({
      ...
      description: "Irregular sessions may impact retention.",
      actionable: true,
    });
  ...
  if (timeOpt.efficiency > 70)
    insights.push({
      ...
      description: `${timeOpt.efficiency}% of sessions at peak hours.`,
      actionable: false,
    });
    ...
    insights.push({
      ...
      description: `Avoid studying during ${timeOpt.wastedHours.join(", ")}.`,
      actionable: true,
    });
  ...
  if (struggles.length > 2)
    insights.push({
      ...
      description: `${struggles.length} topics showing struggles.`,
      actionable: true,
    });
  ...
  if (timeCorr && timeCorr.correlation < 0.2)
    insights.push({
      ...
      description: "Focus on quality study techniques.",
      actionable: true,
    });
