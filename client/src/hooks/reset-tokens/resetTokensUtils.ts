...
import type { TokenType, TokenAllocation, TokenUsageStats, ResetToken, TokenConfig } from '../types/tokens';
...
export function getRemainingForType(allocation: TokenAllocation, type: TokenType): number {
  switch (type) {
    case 'quiz-reset': return allocation.quizResets - allocation.quizResetsUsed;
    case 'lab-reset': return allocation.labResets - allocation.labResetsUsed;
    case 'battle-drill-reset': return allocation.battleDrillResets - allocation.battleDrillResetsUsed;
  }
}
...
export function checkCanReset(
  ...
  if (!allocation) return { allowed: false, reason: 'Loading token allocation...' };

  const remaining = getRemainingForType(allocation, type);
  if (remaining <= 0) {
    const typeLabel = type.replace('-reset', '').replace('-', ' ');
    return { allowed: false, reason: `No ${typeLabel} resets remaining this week` };
  }

  // Check cooldown
  if (recentResets.length > 0) {
    const lastReset = recentResets[0];
    const minutesSinceLastReset = (Date.now() - lastReset.usedAt.getTime()) / (1000 * 60);
    if (minutesSinceLastReset < cooldownMinutes) {
      const remainingMinutes = Math.ceil(cooldownMinutes - minutesSinceLastReset);
      return { allowed: false, reason: `Cooldown active. Wait ${remainingMinutes} more minute(s)` };
    }
  }
...
export function updateAllocationForType(allocation: TokenAllocation, type: TokenType): TokenAllocation {
  const updated = { ...allocation };
  switch (type) {
    case 'quiz-reset': updated.quizResetsUsed++; break;
    case 'lab-reset': updated.labResetsUsed++; break;
    case 'battle-drill-reset': updated.battleDrillResetsUsed++; break;
  }
  return updated;
}
...
export function calculateUsageStats(allResets: ResetToken[]): TokenUsageStats {
  ...
  const firstReset = allResets.length > 0 
    ...
    : new Date();
  const weeksSinceFirst = Math.max(1, Math.ceil((Date.now() - firstReset.getTime()) / (7 * 24 * 60 * 60 * 1000)));
  ...
  allResets.forEach(reset => {
    ...
    else itemCounts.set(reset.itemId, { title: reset.itemTitle, type: reset.type, count: 1 });
  });

  const itemsMostReset = Array.from(itemCounts.entries())
    .map(([itemId, data]) => ({ itemId, itemTitle: data.title, type: data.type, resetCount: data.count }))
...
export function createNewAllocation(userId: string, config: TokenConfig): Omit<TokenAllocation, 'id'> {
