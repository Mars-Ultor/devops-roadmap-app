...
export function parseHintRequests(
  hints:
    | Array<{
        hintLevel: number;
  ...
  return hints.map((hr) => ({
    ...
    requestedAt: hr.requestedAt?.toDate() || new Date(),
    hint: hr.hint,
  }));
}

/** Parse struggle session from Firestore data */
export function parseStruggleSession(
  data: Record<string, unknown>,
): StruggleSession {
  const hintsRequested = data.hintsRequested as
    | Array<{
        hintLevel: number;
        requestedAt?: { toDate: () => Date };
        hint: string;
    ...
    | undefined;
  const struggleLog = data.struggleLog as
    | {
  ...
  return {
    ...
    solutionUnlockedAt: (
      data.solutionUnlockedAt as { toDate: () => Date }
    )?.toDate(),
    hintsRequested: parseHintRequests(hintsRequested),
    struggleLog: struggleLog
      ? {
          ...struggleLog,
          createdAt: struggleLog.createdAt?.toDate() || new Date(),
        }
      : undefined,
  } as StruggleSession;
}
...
export function prepareHintForFirestore(
  hints: HintRequest[],
): Array<{ hintLevel: number; requestedAt: Date; hint: string }> {
  return hints.map((hr) => ({
    hintLevel: hr.hintLevel,
...
export function prepareStruggleLogForFirestore(log: StruggleLog): {
  attemptedSolutions: string[];
  stuckLocation: string;
  hypothesis: string;
  createdAt: Date;
...
export async function startStruggleSession(
  ...
  const now = new Date();
  const hintsUnlockAt = new Date(
    now.getTime() + STRUGGLE_LOCKOUT_SECONDS * 1000,
  );
