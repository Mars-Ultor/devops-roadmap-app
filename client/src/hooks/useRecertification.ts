...
export function useRecertification() {
  ...
  const checkRecertificationStatus = useCallback(async () => {
    ...
    try {
      ...
      setStatus({ lastRecertDate, nextRecertDue, daysUntilDue, isOverdue: daysUntilDue < 0, skillsNeedingRecert });
    } catch (error) { console.error('Error checking recertification status:', error); }
    finally { setLoading(false); }
  }, [user, analyzeSkillDecay]);

  useEffect(() => { if (user) checkRecertificationStatus(); }, [user, checkRecertificationStatus]);
  ...
  return { status, loading, checkRecertificationStatus, completeRecertification };
}
