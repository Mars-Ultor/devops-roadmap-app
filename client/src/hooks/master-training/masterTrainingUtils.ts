...
export interface MasterTrainingState {
  ...
  adaptiveMetrics: { performanceScore: number; difficultyAdjustment: number; timeEfficiency: number; learningVelocity: number };
  loading: boolean;
}
...
export interface ChallengeResult { challengeId: string; correct: boolean; selectedOptionId: string }
...
export function getDefaultAdaptiveMetrics() {
  return { performanceScore: 0, difficultyAdjustment: 0, timeEfficiency: 100, learningVelocity: 1.0 };
}
...
export function findNextScenarioId(path: LearningPath, pathScenarioMap: Record<string, string[]>): string | null {
  const pathScenarioIds = pathScenarioMap[path.id] || ['adaptive-incident-response'];
  return pathScenarioIds.find(id => !path.completedScenarios.includes(id)) || null;
}
...
export function createTrainingSession(
  ...
  return {
    ...
    performanceData: { decisions: [], communications: [], timeSpent: 0, score: 0, difficultyAdjustment: 0 },
    aiRecommendations: { nextDifficulty: scenario.currentDifficulty, focusAreas: [], suggestedScenarios: [], learningInsights: [] }
  };
}
...
export function calculateSessionScore(challengeResults: ChallengeResult[], totalChallenges: number): number {
