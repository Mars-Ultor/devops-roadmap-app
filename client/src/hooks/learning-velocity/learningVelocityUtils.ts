...
export function calculateVelocityTrend(
  ...
  const sumXY = weeklyData.reduce(
    (sum, w, idx) => sum + idx * w.itemsCompleted,
    0,
  );
...
export function convertToWeeklyProgress(
  weeklyData: Record<string, WeeklyDataItem>,
): WeeklyProgress[] {
  return Object.entries(weeklyData)
    ...
    .map(([weekKey, data]) => {
      const weekNumber =
        Math.floor(
          (new Date(weekKey).getTime() - new Date("2024-01-01").getTime()) /
            (1000 * 60 * 60 * 24 * 7),
        ) + 1;
      return {
        week: weekNumber,
        ...
        avgTimePerItem:
          data.items.length > 0 ? data.totalTime / data.items.length : 0,
        masteryRate:
          data.items.length > 0
            ? (data.masteredItems / data.items.length) * 100
            : 0,
        date: data.weekStart,
      };
    });
...
export function calculateCurrentPace(weeklyProgress: WeeklyProgress[]): number {
  ...
  return (
    recentWeeks.reduce((sum, w) => sum + w.itemsCompleted, 0) /
    recentWeeks.length
  );
}

/** Calculate optimal pace based on remaining work */
export function calculateOptimalPace(
  weeklyProgress: WeeklyProgress[],
  totalWeeks: number = 12,
  remainingItems: number = 100,
): number {
