...
export function getLevelKey(level: MasteryLevel): keyof Pick<LessonMastery, 'crawl' | 'walk' | 'runGuided' | 'runIndependent'> {
...
export function getLevelProgressFromMastery(mastery: LessonMastery | null, level: MasteryLevel): MasteryProgress | null {
  if (!mastery) return null;
  return mastery[getLevelKey(level)];
}

/** Check if user can access a level */
export function checkCanAccessLevel(mastery: LessonMastery | null, level: MasteryLevel): boolean {
  if (!mastery) return false;
  return mastery[getLevelKey(level)].unlocked;
}

/** Check if a level is mastered */
export function checkIsLevelMastered(mastery: LessonMastery | null, level: MasteryLevel): boolean {
  if (!mastery) return false;
...
export const LEVEL_ORDER: MasteryLevel[] = ['crawl', 'walk', RUN_GUIDED_LEVEL, RUN_INDEPENDENT_LEVEL];

/** Calculate new average time */
export function calculateNewAverageTime(currentAverage: number, attempts: number, newTime: number): number {
...
export function updateMasteryAfterAttempt(
  ...
  const newPerfectCompletions = perfect ? levelData.perfectCompletions + 1 : levelData.perfectCompletions;
  const newAverageTime = calculateNewAverageTime(levelData.averageTime || 0, levelData.attempts, timeSpent);
  ...
  updatedMastery[levelKey] = { ...levelData, attempts: newAttempts, perfectCompletions: newPerfectCompletions, lastAttemptDate: new Date(), averageTime: newAverageTime };

  // Unlock next level if current is mastered
  if (isLevelMastered) {
    if (level === 'crawl' && !mastery.walk.unlocked) { updatedMastery.walk = { ...mastery.walk, unlocked: true }; updatedMastery.currentLevel = 'walk'; }
    else if (level === 'walk' && !mastery.runGuided.unlocked) { updatedMastery.runGuided = { ...mastery.runGuided, unlocked: true }; updatedMastery.currentLevel = RUN_GUIDED_LEVEL; }
    else if (level === RUN_GUIDED_LEVEL && !mastery.runIndependent.unlocked) { updatedMastery.runIndependent = { ...mastery.runIndependent, unlocked: true }; updatedMastery.currentLevel = RUN_INDEPENDENT_LEVEL; }
    else if (level === RUN_INDEPENDENT_LEVEL) { updatedMastery.fullyMastered = true; updatedMastery.currentLevel = RUN_INDEPENDENT_LEVEL; }
  }
