...
export async function findSimilarFailures(
  ...
  try {
    ...
    const failures = snapshot.docs.map(
      (doc) =>
        ({
          id: doc.id,
          ...doc.data(),
    ...
    return failures.filter(
      (f) =>
        calculateSimilarity(f.title.toLowerCase(), title.toLowerCase()) >
        similarityThreshold,
    );
  } catch (error) {
...
export async function updateFailurePattern(
  ...
  try {
    ...
    const patternDoc = snapshot.docs.find(
      (d) =>
        calculateSimilarity(
          d.data().pattern.toLowerCase(),
          title.toLowerCase(),
        ) > 0.5,
    );
    ...
    if (patternDoc) {
      ...
      await updateDoc(doc(db, "failurePatterns", patternDoc.id), {
...
export function buildFailureQuery(
  userId: string,
  contentId?: string,
  category?: FailureCategory,
) {
