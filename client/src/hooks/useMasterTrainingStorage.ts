...
export const useMasterTrainingStorage = (userId: string | undefined) => {
  ...
  const loadProgress = useCallback(async (): Promise<StorageResult> => {
    ...
    try {
      ...
      const progressRef = doc(db, 'users', userId, 'masterTraining', 'progress');
  ...
  const saveProgress = useCallback(async (updatedPaths: LearningPath[]): Promise<boolean> => {
    ...
    try {
      ...
      localStorage.setItem(`masterTraining_${userId}`, JSON.stringify(updatedPaths));
