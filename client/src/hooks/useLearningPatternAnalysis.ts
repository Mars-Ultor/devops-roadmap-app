...
import {
  analyzeStudyConsistency,
  analyzeTimeOptimization,
  analyzeTopicStruggles,
...
export function useLearningPatternAnalysis() {
  ...
  const [patternData, setPatternData] = useState<LearningPatternData | null>(
    null,
  );
  ...
  const analyzeLearningPatterns = async () => {
    ...
    try {
      const [sessionsSnap, progressSnap, failuresSnap] = await Promise.all([
        ...
        getDocs(
          query(collection(db, "failureLogs"), where("userId", "==", user.uid)),
        ),
      ]);

      const studyConsistency = analyzeStudyConsistency(sessionsSnap.docs);
      const timeOptimization = analyzeTimeOptimization(
        sessionsSnap.docs,
        progressSnap.docs,
      );
      const topicStruggles = analyzeTopicStruggles(failuresSnap.docs);
      const performanceCorrelations = analyzePerformanceCorrelations(
        sessionsSnap.docs,
        progressSnap.docs,
      );
