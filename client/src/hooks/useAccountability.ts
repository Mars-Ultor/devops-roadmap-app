...
export function useAccountability() {
  ...
  const loadPublicCommitments = useCallback(async () => {
    ...
    try {
      ...
      console.error("Error loading public commitments:", e);
    }
  }, [user?.uid]);
  ...
  const createWeeklyCommitment = async (
  ...
  const updateCommitmentProgress = async (
    commitmentId: string,
    progress: number,
  ): Promise<void> => {
  ...
  const deleteIndividualCommitment = async (
    commitmentId: string,
  ): Promise<void> => {
    if (!currentWeekCommitment) return;
  ...
  const completeWeeklyCheckIn = async (
    reflection: string,
