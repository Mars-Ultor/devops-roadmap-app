...
import {
  ...
  executeGit,
  executeSystemctl,
} from "./terminal-commands/terminalCommandsUtils";
...
type CommandHandler = (
  ...
  ctx: CommandContext,
  setCtx: React.Dispatch<React.SetStateAction<CommandContext>>,
) => void;

const simpleCommands: Record<
  string,
  (term: Terminal, entry: CommandHistoryEntry, ctx: CommandContext) => void
> = {
  help: (term, entry) => {
    displayHelp(term);
    entry.success = true;
  ...
  pwd: (term, entry, ctx) => {
    term.writeln(ctx.currentDir);
    entry.success = true;
    entry.output = [ctx.currentDir];
  },
  whoami: (term, entry, ctx) => {
    term.writeln(ctx.environment.USER);
    entry.success = true;
    entry.output = [ctx.environment.USER];
  },
  env: (term, entry, ctx) => {
    Object.entries(ctx.environment).forEach(([k, v]) =>
      term.writeln(`${k}=${v}`),
    );
    entry.success = true;
  },
...
export function useTerminalCommands({
  ...
  const executeCommand = useCallback(
    (command: string, term: Terminal): CommandHistoryEntry => {
      ...
      if (simpleHandler) {
        simpleHandler(term, entry, context);
        return entry;
      }

      const argHandler = argCommands[cmd];
      if (argHandler) {
        argHandler(args, term, entry, context, setContext);
        return entry;
      }
