...
import type {
  HourlyPerformance,
  TimeRecommendation,
} from "./time-analysis/timeAnalysisUtils";
import {
  formatHour,
...
export function useTimeAnalysis(progressSnap?: QuerySnapshot) {
  ...
  const [analysisData, setAnalysisData] = useState<TimeAnalysisData | null>(
    null,
  );
  const [loading, setLoading] = useState(true);

  const analyzeStudyTimes = useCallback(
    async (data?: QuerySnapshot) => {
      ...
      try {
        ...
        setAnalysisData({
          hourlyPerformance,
          recommendation,
          totalDataPoints: snapToUse.size,
        });
      } catch (error) {
