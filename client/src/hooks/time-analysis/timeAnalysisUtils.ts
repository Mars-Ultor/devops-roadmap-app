...
export function calculateHourlyPerformance(hourlyData: Record<number, { scores: number[]; completions: number; attempts: number; totalMinutes: number }>): HourlyPerformance[] {
  return Object.entries(hourlyData).map(([hour, data]) => ({
    ...
    completionRate: data.attempts > 0 ? (data.completions / data.attempts) * 100 : 0
  }));
}

/** Generate recommendation from hourly performance */
export function generateRecommendation(hourlyData: HourlyPerformance[]): TimeRecommendation {
  const significantHours = hourlyData.filter(h => h.totalSessions >= 3);
  if (significantHours.length < 3) return getDefaultRecommendation();

  const sortedByPerformance = [...significantHours].sort((a, b) => ((b.averageScore * 0.6) + (b.completionRate * 0.4)) - ((a.averageScore * 0.6) + (a.completionRate * 0.4)));
  ...
  return {
    ...
    confidenceLevel: significantHours.length >= 10 ? 'high' : significantHours.length >= 5 ? 'medium' : 'low'
  };
}
