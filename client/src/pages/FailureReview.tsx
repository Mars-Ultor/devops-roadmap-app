...
export default function FailureReview() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const avgResolutionTime =
        resolvedFailures.length > 0
          ? resolvedFailures.reduce(
              (acc, f) => acc + (f.timeToResolveMinutes || 0),
              0,
      ...
      const failuresLast7Days = allFailures.filter(
        (f) => f.timestamp >= sevenDaysAgo,
      ).length;
      const failuresLast30Days = allFailures.filter(
        (f) => f.timestamp >= thirtyDaysAgo,
      ).length;

      // Improvement rate: reduction in failures over time
      const failuresPrevious30Days = allFailures.filter(
        (f) =>
      ...
      const improvementRate =
        failuresPrevious30Days > 0
          ? ((failuresPrevious30Days - failuresLast30Days) /
              failuresPrevious30Days) *
            100
          : 0;
      ...
      allFailures.forEach((f) => {
        failuresByCategory[f.category] =
          (failuresByCategory[f.category] || 0) + 1;
      });
      ...
      allFailures.forEach((f) => {
        failuresBySeverity[f.severity] =
          (failuresBySeverity[f.severity] || 0) + 1;
      });
                        ...
                        {pattern.recommendedActions.length > 0 && (
                          <div className="mt-3 pt-3 border-t border-slate-700">
                            <p className="text-sm text-slate-300">
                              ...
                              {pattern.recommendedActions.join(", ")}
                            </p>
                          </div>
                        )}
                      </div>
