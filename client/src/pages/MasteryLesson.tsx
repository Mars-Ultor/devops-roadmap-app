...
export default function MasteryLesson() {
  ...
  const rawContent = detailedContent
    ? detailedContent[getContentKey(level)]
    : null;
  ...
  const renderLevelContent = () => {
      ...
      return (
        <div className="bg-yellow-900/20 border border-yellow-600 rounded-lg p-6">
          ...
          <p className="text-yellow-200">
            This lesson content is being developed. Please check back soon for
            detailed mastery-level content.
          </p>
        </div>
      );
    }
                      ...
                      <div className="mb-3">
                        ...
                        <ul className="space-y-1">
                          {step.validationCriteria.map((criteria) => (
                            <li
                              key={`vc-${step.stepNumber}-${criteria}`}
                    ...
                    <div>
                      ...
                      <ul className="space-y-1">
                        {step.commonMistakes.map((mistake) => (
                          <li
                            key={`cm-${step.stepNumber}-${mistake}`}
                            className="text-sm text-red-200 flex items-start"
                          >
    ...
    if (level === "run-guided" && "checkpoints" in rawContent) {
      ...
      const handleRunGuidedSaveDraft = (
        responses: Record<string, string>,
        notes: string,
      ) => {
      ...
      return (
        <div className="space-y-6">
          ...
          {/* Conceptual Guidance */}
          {"conceptualGuidance" in rawContent &&
            rawContent.conceptualGuidance && (
              <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
          ...
          {/* Key Concepts */}
          {"keyConceptsToApply" in rawContent &&
            rawContent.keyConceptsToApply && (
              <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
    ...
    if (level === "run-independent" && "objective" in rawContent) {
      ...
      const savedDraft =
        runIndependentDraft ||
        localStorage.getItem(`run-independent-draft-${lessonId}`) ||
        "";
    ...
    return (
      <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
        <p className="text-slate-400">
          Content format not recognized for this level.
        </p>
      </div>
    );
  };
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {detailedContent?.baseLesson && (
                <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
                  ...
                  <ul className="space-y-2">
                    {detailedContent.baseLesson.learningObjectives.map(
                      (objective) => (
                        <li
                          key={`lo-${objective}`}
                  ...
                  {detailedContent.baseLesson.prerequisites.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-slate-700">
                      ...
                      <ul className="space-y-1">
                        {detailedContent.baseLesson.prerequisites.map(
                          (prereq) => (
                            <li
                              key={`pr-${prereq}`}
            ...
            <div className="space-y-6">
              ...
              {detailedContent?.baseLesson && (
                <div className="bg-slate-800 rounded-lg p-4 border border-slate-700">
                  ...
                  <div className="flex items-center space-x-2 text-slate-300">
                    ...
                    <span>
                      {level === "crawl" &&
                        detailedContent.baseLesson.estimatedTimePerLevel.crawl}
                      {level === "walk" &&
                        detailedContent.baseLesson.estimatedTimePerLevel.walk}
                      {level === "run-guided" &&
                        detailedContent.baseLesson.estimatedTimePerLevel
                          .runGuided}
                      {level === "run-independent" &&
                        detailedContent.baseLesson.estimatedTimePerLevel
                          .runIndependent}
                      {" minutes"}
                    </span>
              ...
              {mastery && getLevelProgress(level) && (
                <>
                  ...
                  {console.log(
                    "🟣 MasteryGate Render - Progress:",
                    getLevelProgress(level),
                  )}
                  {console.log(
                    "🟣 MasteryGate Render - Full Mastery:",
                    mastery,
                  )}
                  <MasteryGate
                    level={level}
              ...
              <button
                onClick={async () => {
                  ...
                  const finalMetrics: StruggleMetrics = {
                    ...
                    timeSpentSeconds: timeSpent,
                    isPerfectCompletion:
                      struggleMetrics.hintsUsed === 0 &&
                      struggleMetrics.validationErrors === 0 &&
                      struggleMetrics.retryCount === 0,
                  };
                  ...
                  console.log("🔵 Recording attempt:", {
                    ...
                    isPerfect,
                    timeSpent,
                    struggleMetrics: finalMetrics,
                  });
                  console.log(
                    "🔵 Mastery state BEFORE recordAttempt:",
                ...
                <div className="bg-yellow-900/20 border border-yellow-700 rounded-lg p-3">
                  <div className="flex items-start space-x-2">
                    ...
                    <div className="text-xs text-yellow-300">
                      ...
                      {struggleMetrics.hintsUsed > 0 && (
                        <p>• {struggleMetrics.hintsUsed} hint(s) used</p>
                      )}
                      {struggleMetrics.validationErrors > 0 && (
                        <p>• {struggleMetrics.validationErrors} error(s)</p>
                      )}
                    </div>
                  </div>
                </div>
