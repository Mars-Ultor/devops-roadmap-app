...
const LEARNING_PATHS: LearningPath[] = [
  ...
  },
  {
    ...
    description: 'Specialized training in security operations, compliance, and threat response',
...
const ADAPTIVE_SCENARIOS: AdaptiveScenario[] = [
  {
  ...
  },
  {
    ...
    challenges: [
      ...
      },
      {
        ...
        situation: 'Logs show attacker used compromised API key. How deep is the breach?',
        correctOptionId: 'opt-1',
  ...
  },
  {
    ...
    scenario: 'Your company is growing from 50 to 500 engineers. Current ad-hoc deployment process won\'t scale. You need to design a self-service platform that maintains developer velocity while ensuring reliability.',
    challenges: [
      {
        id: 'platform-vision',
  ...
  },
  {
    ...
    scenario: 'Your API latency has degraded from p95 200ms to 2000ms over the past week. No recent deployments. Users are complaining. Find and fix the root cause.',
    challenges: [
      {
      ...
      },
      {
        ...
        options: [
          ...
          },
          {
            ...
            outcome: 'suboptimal',
            explanation: 'Reactive. Want real-time alerts, not periodic reviews.',
            skillsAssessed: ['Process Improvement']
          }
        ]
      }
  ...
  },
  {
    ...
    scenario: 'Your CI/CD pipeline has been failing for 3 hours. 15 teams are blocked from deploying. Tests are timing out, builds are inconsistent, and the infrastructure team says nothing changed.',
    challenges: [
      {
  ...
  },
  {
    ...
    scenario: 'Marketing just announced a viral campaign starting in 2 hours. Expected traffic: 10x normal. Current infrastructure: barely handling current load. Budget approved for emergency scaling.',
    challenges: [
      {
  ...
  },
  {
    ...
    scenario: 'You\'re hiring 5 engineers over the next month. Current onboarding is ad-hoc: senior devs are pulled away from work for days, new hires feel lost, and time-to-productivity is 8 weeks.',
    challenges: [
      {
  ...
  },
  {
    ...
    scenario: 'Your primary AWS region went down (regional outage). All services offline. Customers can\'t access the product. You have backups in another region but never tested failover. Board is watching.',
    challenges: [
      {
        id: 'initial-response',
  ...
  },
  {
    ...
    scenario: 'Teams can\'t debug production issues. Logs scattered across services. No distributed tracing. Metrics dashboard is a mess. You\'re tasked with implementing unified observability.',
    challenges: [
      {
  ...
  },
  {
    ...
    scenario: 'Your company is migrating a 10-year-old monolithic application from on-premise to AWS. Constraints: zero downtime, data compliance (GDPR), $500k budget, 6-month timeline.',
    challenges: [
      {
  ...
  },
  {
    ...
    scenario: 'Your company runs 100+ microservices on Kubernetes. Cluster has stability issues: pods randomly evicted, nodes hitting resource limits, network policies causing connectivity issues.',
    challenges: [
      {
  ...
  },
  {
    ...
    scenario: 'Company pursuing SOC 2 Type 2 certification. Manual compliance checks are error-prone and slow. Auditors need evidence of continuous compliance, not point-in-time checks.',
    challenges: [
      {
        id: 'policy-as-code',
  ...
  },
  {
    ...
    scenario: 'Major outage lasted 4 hours, impacted $500k revenue. Teams are defensive, finger-pointing at each other. You need to facilitate a productive post-mortem that drives improvements.',
    challenges: [
      {
  ...
  },
  {
    ...
    scenario: 'Engineering team velocity decreasing. Tech debt piling up: legacy code, outdated dependencies, missing tests. Product wants features, engineering wants refactoring time. You need to build consensus.',
    challenges: [
      {
...
// Map learning paths to their recommended starting scenarios
const PATH_SCENARIO_MAP: Record<string, string[]> = {
  'devops-generalist': [
...
export default function MasterTraining() {
  ...
  const [currentSession, setCurrentSession] = useState<MasterTrainingSession | null>(null);
  const [phase, setPhase] = useState<'selection' | 'path-overview' | 'training' | 'progress' | 'insights'>('selection');
  const [trainingActive, setTrainingActive] = useState(false);
  const [sessionStartTime, setSessionStartTime] = useState<Date | null>(null);
  const [currentChallengeIndex, setCurrentChallengeIndex] = useState(0);
  const [selectedAnswer, setSelectedAnswer] = useState<string | null>(null);
  const [challengeResults, setChallengeResults] = useState<{challengeId: string; correct: boolean; selectedOptionId: string}[]>([]);
  ...
  const loadProgress = useCallback(async () => {
    ...
    try {
      console.log('[MasterTraining] Loading progress for user:', user.uid);
      
      // Try localStorage first
      const localData = localStorage.getItem(`masterTraining_${user.uid}`);
      if (localData) {
      ...
      // Fallback to Firestore - using users collection pattern
      const progressRef = doc(db, 'users', user.uid, 'masterTraining', 'progress');
  ...
  const saveProgress = async (updatedPaths: LearningPath[]) => {
    ...
    try {
      console.log('[MasterTraining] Saving progress for user:', user.uid);
      
      // Save to localStorage (immediate, reliable)
      localStorage.setItem(`masterTraining_${user.uid}`, JSON.stringify(updatedPaths));
      console.log('Saved master training progress to localStorage');
      
      // Try Firestore as backup (non-blocking) - using users collection pattern
      const progressRef = doc(db, 'users', user.uid, 'masterTraining', 'progress');
      console.log('[MasterTraining] Progress ref path:', progressRef.path);
  ...
  const beginTrainingSession = () => {
    if (!selectedPath) return;

    // Get the appropriate scenarios for this learning path
    const pathScenarioIds = PATH_SCENARIO_MAP[selectedPath.id] || ['adaptive-incident-response'];
    ...
    if (!nextScenarioId) {
      ...
      alert('Congratulations! You\'ve completed all scenarios in this learning path!');
      return;
    }
    
    const startingScenario = ADAPTIVE_SCENARIOS.find(s => s.id === nextScenarioId) || ADAPTIVE_SCENARIOS[0];

    console.log('Beginning Training Session:', {
      scenarioId: startingScenario.id,
      scenarioTitle: startingScenario.title,
      totalChallenges: startingScenario.challenges.length,
      challengeIds: startingScenario.challenges.map(c => c.id)
    });

    const session: MasterTrainingSession = {
      ...
      aiRecommendations: {
        ...
        focusAreas: startingScenario.skills,
        suggestedScenarios: pathScenarioIds,
        learningInsights: [
          `Tailored for ${selectedPath.title}`,
          `Focus areas: ${startingScenario.skills.join(', ')}`,
          `Estimated time: ${startingScenario.estimatedTime} minutes`
        ]
      }
    };
  ...
  const backToPathOverview = () => {
    // Reset all training session state
  ...
  const submitAnswer = () => {
    ...
    console.log('Submit Answer Debug:', {
      ...
      totalChallenges: currentSession.currentScenario.challenges.length,
      challengeId: currentChallenge.id,
      selectedAnswer,
      correctAnswer: currentChallenge.correctOptionId,
      isCorrect
    });
    ...
    const totalAttempts = updatedResults.length;
    const newScore = Math.round((correctCount / totalAttempts) * 100);
    
    setAdaptiveMetrics({
      performanceScore: newScore,
      difficultyAdjustment: isCorrect ? 1 : -1,
      timeEfficiency: Math.max(0, 100 - (totalAttempts * 5)),
      learningVelocity: isCorrect ? 1.2 : 0.8
    });

    // Move to next challenge or complete
    if (currentChallengeIndex < currentSession.currentScenario.challenges.length - 1) {
      setTimeout(() => {
  ...
  const completeSession = (finalResults?: typeof challengeResults) => {
    if (!currentSession || !selectedPath) return;

    const endTime = new Date();
    const timeSpent = sessionStartTime ? (endTime.getTime() - sessionStartTime.getTime()) / 1000 : 0;

    // Use passed results or fallback to state (for edge cases)
    const results = finalResults || challengeResults;

    // Calculate session performance
    const correctCount = results.filter(r => r.correct).length;
    const totalChallenges = results.length;
    const sessionScore = totalChallenges > 0 ? Math.round((correctCount / totalChallenges) * 100) : 0;
    ...
    const updatedPath = {
      ...selectedPath,
      progress: Math.min(100, selectedPath.progress + 10), // Increment by 10% per session
      completedScenarios: [...new Set([...selectedPath.completedScenarios, currentSession.currentScenario.id])], // Use Set to prevent duplicates
      currentLevel: sessionScore >= 80 ? Math.min(selectedPath.totalLevels, selectedPath.currentLevel + 1) : selectedPath.currentLevel,
      strengths: sessionScore >= 80 ? 
        [...new Set([...selectedPath.strengths, ...results.filter(r => r.correct).slice(0, 2).map(() => currentSession.currentScenario.skills[0])])] :
        selectedPath.strengths,
      areasForImprovement: sessionScore < 70 ?
        [...new Set([...selectedPath.areasForImprovement, ...results.filter(r => !r.correct).slice(0, 2).map(() => currentSession.currentScenario.skills[0])])] :
        selectedPath.areasForImprovement
    };

    // Update both selectedPath and the path in learningPaths array
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-400">Please log in to access master training.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-900 p-6">
      <div className="max-w-7xl mx-auto">
                  ...
                  {path.progress > 0 ? (
                    ...
                    <div className="bg-slate-900 rounded-lg p-4">
                      ...
                      <div className="mt-3 flex items-center text-xs text-slate-500">
                        ...
                        {ADAPTIVE_SCENARIOS.filter(s => 
                          path.careerFocus === 'generalist' ? true :
                          path.careerFocus === 'leadership' ? s.category === 'leadership' :
                          path.careerFocus === 'technical' ? s.category === 'specialized' || s.category === 'integration' :
                          s.category === 'specialized'
                        ).length} adaptive scenarios available
                      </div>
                    </div>
                  )}

                  <div className="mt-4 flex items-center justify-between">
                    <span className={`text-sm font-medium ${getDifficultyColor(path.currentLevel)}`}>
                      {getDifficultyLabel(path.currentLevel)} Level
                    </span>
                    <span className={`text-xs px-2 py-1 rounded ${
                      path.recommendedPace === 'accelerated' ? 'bg-green-500/20 text-green-400' :
                      path.recommendedPace === 'intensive' ? 'bg-red-500/20 text-red-400' :
                      'bg-blue-500/20 text-blue-400'
                    }`}>
                      {path.recommendedPace} pace
                    </span>
                  </div>
                </div>
              ))}
        ...
        {phase === 'path-overview' && selectedPath && (
            ...
            <div className="bg-slate-800 rounded-lg border border-slate-700 p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-slate-900 rounded-lg p-6">
                  ...
                  <div className="text-center">
                    <div className="text-3xl font-bold text-indigo-400 mb-2">
                      {selectedPath.currentLevel}/{selectedPath.totalLevels}
                    </div>
                    <div className="text-sm text-slate-400">Level {selectedPath.currentLevel}</div>
                    ...
                    <div className="text-xs text-slate-500 mt-1">{selectedPath.progress}% Complete</div>
                  </div>
                </div>

                <div className="bg-slate-900 rounded-lg p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold text-white">Time to Complete</h3>
                ...
                <div className="bg-slate-900 rounded-lg p-6">
                  ...
                  <div className="text-center">
                    <div className="text-2xl font-bold text-yellow-400 mb-2 capitalize">{selectedPath.recommendedPace}</div>
                    <div className="text-sm text-slate-400">AI-recommended pace</div>
                  </div>
                </div>
              </div>

              {/* Skills Development */}
              {selectedPath.progress > 0 && selectedPath.strengths.length > 0 && (
                <div className="mb-8">
                  <h3 className="text-lg font-semibold text-white mb-4">Skills Development Focus</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
              ...
              {/* Available Scenarios or AI Insights */}
              {selectedPath.progress === 0 ? (
                <div className="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 rounded-lg border border-indigo-500/20 p-6 mb-8">
                  ...
                  <div className="space-y-3">
                    {(() => {
                      const pathScenarioIds = PATH_SCENARIO_MAP[selectedPath.id] || [];
                      return pathScenarioIds.map((scenarioId) => {
                        const scenario = ADAPTIVE_SCENARIOS.find(s => s.id === scenarioId);
                ...
                <div className="bg-slate-800 rounded-lg border border-slate-700 p-8">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                    <div className="bg-slate-900 rounded-lg p-4 text-center">
                      <div className="text-2xl font-bold text-indigo-400">
                        {challengeResults.length > 0 ? `${adaptiveMetrics.performanceScore}%` : '—'}
                      </div>
                    ...
                    <div className="bg-slate-900 rounded-lg p-4 text-center">
                      <div className="text-2xl font-bold text-green-400">
                        {challengeResults.length > 0 ? `${adaptiveMetrics.timeEfficiency}%` : '—'}
                      </div>
                    ...
                    <div className="bg-slate-900 rounded-lg p-4 text-center">
                      <div className="text-2xl font-bold text-blue-400">
                        {challengeResults.length > 0 ? adaptiveMetrics.learningVelocity.toFixed(1) : '—'}
                      </div>
                    ...
                    <div className="bg-slate-900 rounded-lg p-4 text-center">
                      <div className={`text-2xl font-bold ${
                        challengeResults.length === 0 ? 'text-slate-600' :
                        adaptiveMetrics.difficultyAdjustment > 0 ? 'text-green-400' : 'text-red-400'
                      }`}>
                        {challengeResults.length > 0 ? 
                      ...
                      <div className="text-xs text-slate-400">Difficulty Adj.</div>
                      {challengeResults.length === 0 && (
                        <div className="text-xs text-slate-500 mt-1">Answer to start</div>
                      )}
                    </div>
                  ...
                  <div className="bg-slate-900 rounded-lg p-6 mb-6">
                    ...
                    {currentSession.currentScenario.challenges[currentChallengeIndex] && (() => {
                      const currentChallenge = currentSession.currentScenario.challenges[currentChallengeIndex];
                      const hasAnswered = challengeResults.some(r => r.challengeId === currentChallenge.id);
                      const userResult = challengeResults.find(r => r.challengeId === currentChallenge.id);

                      return (
                        <div key={currentChallenge.id} className="space-y-4">
                          <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-lg p-4">
                            <div className="flex items-center text-indigo-400 mb-2">
                              <Target className="w-4 h-4 mr-2" />
                              <span className="text-sm font-medium">{currentChallenge.phase}</span>
                            </div>
                            <p className="text-white font-medium">{currentChallenge.situation}</p>
                          </div>
                                  ...
                                  >
                                    <div className="flex items-start">
                                      <div className={`w-5 h-5 rounded-full border-2 mr-3 mt-0.5 flex items-center justify-center ${
                                        ...
                                        {(hasAnswered && showFeedback) && (
                                          isCorrect ? <CheckCircle className="w-3 h-3 text-white" /> : <XCircle className="w-3 h-3 text-white" />
                                        )}
                                      </div>
                                        ...
                                        {showFeedback && (
                                          <div className="mt-2 pt-2 border-t border-slate-700">
                                            ...
                                            <div className="flex flex-wrap gap-1 mt-2">
                                              {option.skillsAssessed.map((skill, idx) => (
                                                <span key={idx} className="text-xs px-2 py-1 bg-slate-900 text-slate-400 rounded">
                                                  {skill}
                                                </span>
                                              ))}
                                            </div>
                          ...
                          {hasAnswered && (
                            <div className="bg-slate-800 rounded-lg p-4 border border-slate-700">
                              ...
                              <p className="text-slate-300 text-sm">{currentChallenge.learningObjective}</p>
                            </div>
                          )}
                        </div>
                      );
                  ...
                  <div className="space-y-4">
                    <div className="bg-slate-900 rounded-lg p-4">
                      ...
                      <p className="text-slate-300 text-sm">
                        Building decision-making speed while maintaining accuracy.
                        Current performance shows strong technical skills.
                      </p>
                    </div>

                    <div className="bg-slate-900 rounded-lg p-4">
                      <h4 className="text-white font-medium mb-2">Next Challenge</h4>
                      <p className="text-slate-300 text-sm">
                        Increasing stakeholder complexity to improve communication
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="w-full bg-slate-700 rounded-full h-2">
                        <div 
                          ...
                          style={{ 
                            width: (currentSession?.scenario?.challenges?.length) 
                              ? `${Math.min(100, (challengeResults.length / currentSession.scenario.challenges.length) * 100)}%`
                              : '0%'
                          }}
                        ></div>
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-white">
                          {challengeResults.length}/{currentSession?.scenario?.challenges?.length || 0}
                        </span>
                      </div>
                      <div className="w-full bg-slate-700 rounded-full h-2">
                        <div 
                          ...
                          style={{ 
                            width: (currentSession?.scenario?.challenges?.length) 
                              ? `${(challengeResults.length / currentSession.scenario.challenges.length) * 100}%`
                              : '0%'
                          }}
                        ></div>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="w-full bg-slate-700 rounded-full h-2">
                        <div 
                          ...
                          style={{ 
                            width: challengeResults.length > 0 
                              ? `${(challengeResults.filter(r => r.correct).length / challengeResults.length) * 100}%`
                              : '0%'
                          }}
                        ></div>
                      </div>
        ...
        {phase === 'insights' && currentSession && (
          <div className="max-w-4xl mx-auto">
            <div className="bg-slate-800 rounded-lg border border-slate-700 p-8">
                  ...
                  <div className="space-y-4">
                    <div className="bg-slate-900 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-indigo-400 font-semibold">{adaptiveMetrics.performanceScore}%</span>
                      </div>
                      <div className="w-full bg-slate-700 rounded-full h-2">
                        <div className="bg-indigo-500 h-2 rounded-full" style={{ width: `${adaptiveMetrics.performanceScore}%` }}></div>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-900 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-green-400 font-semibold">{adaptiveMetrics.timeEfficiency}%</span>
                      </div>
                      <div className="w-full bg-slate-700 rounded-full h-2">
                        <div className="bg-green-500 h-2 rounded-full" style={{ width: `${adaptiveMetrics.timeEfficiency}%` }}></div>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-900 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-blue-400 font-semibold">{adaptiveMetrics.learningVelocity}</span>
                      </div>
                      <div className="w-full bg-slate-700 rounded-full h-2">
                        <div className="bg-blue-500 h-2 rounded-full" style={{ width: `${Math.min(adaptiveMetrics.learningVelocity * 10, 100)}%` }}></div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4">
                      <h4 className="text-green-400 font-medium mb-2">Strengths Demonstrated</h4>
                      <ul className="space-y-1 text-sm text-slate-300">
                        <li>• Excellent technical problem-solving</li>
                        <li>• Strong systematic approach</li>
                        <li>• Good prioritization skills</li>
                      </ul>
                    </div>

                    <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
                      <h4 className="text-blue-400 font-medium mb-2">Improvement Opportunities</h4>
                      <ul className="space-y-1 text-sm text-slate-300">
                        <li>• Increase decision speed</li>
                        <li>• Enhance stakeholder communication</li>
                        <li>• Practice risk assessment</li>
                      </ul>
                    </div>

                    <div className="bg-purple-500/10 border border-purple-500/20 rounded-lg p-4">
                      <h4 className="text-purple-400 font-medium mb-2">Next Session Focus</h4>
                      <p className="text-sm text-slate-300">
                        The AI will increase time pressure while maintaining technical complexity
              ...
              {/* Recommendations */}
              <div className="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 rounded-lg border border-indigo-500/20 p-6 mb-8">
                <h3 className="text-lg font-semibold text-white mb-4">AI Recommendations for Next Session</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-indigo-400 mb-2">7.2</div>
                    <div className="text-sm text-slate-400">Recommended Difficulty</div>
                  </div>
                  <div className="text-center">
                  ...
                  <div className="text-center">
                    <div className="text-2xl font-bold text-blue-400 mb-2">32 min</div>
                    <div className="text-sm text-slate-400">Optimal Session Length</div>
                  </div>
                </div>
              </div>
