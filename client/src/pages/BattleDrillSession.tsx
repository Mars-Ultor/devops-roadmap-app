...
export default function BattleDrillSession() {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      alert(
        "❌ Error starting drill. Please check your connection and try again.",
      );
    }
  };
  ...
  const getTimeColor = () => {
    ...
    if (elapsedSeconds <= drill.targetTimeSeconds * 1.5)
    ...
    <div className="min-h-screen bg-slate-900 py-8 px-4">
      <div className="max-w-5xl mx-auto">
                ...
                {!aarSubmitted && (
                  <div className="mt-6 bg-indigo-900/30 border border-indigo-600/30 rounded-lg p-4">
                    <p className="text-indigo-200 text-sm">
                      ⚠️ AAR Required: Complete your After Action Review to
                      proceed
                    </p>
                  </div>
                )}
          ...
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 text-sm">
                <span
                  className={`px-3 py-1 rounded ${getDifficultyStyles(drill.difficulty)}`}
                >
                  {drill.difficulty}
                </span>
            ...
            {sessionStarted && (
              <div className="text-right">
                ...
                <div
                  className={`text-4xl font-bold font-mono ${getTimeColor()}`}
                >
          ...
          <div className="mb-6">
            <BattleDrillPerformanceTracker
              ...
              onRecertificationNeeded={() => {
                alert(
                  "⚠️ Recertification Required: Your proficiency has degraded. Complete this drill to recertify.",
                );
              }}
            />
          </div>
        ...
        {user && sessionStarted && coachContext && (
          <div className="mb-6">
            <PerformanceDashboard userId={user.uid} context={coachContext} />
          </div>
        )}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="space-y-1">
                          ...
                          {step.validationCriteria.map((criteria) => {
                            ...
                            const isPassed =
                              validation?.passedCriteria.includes(criteria);
                            const isFailed =
                              validation?.failedCriteria.includes(criteria);

                            return (
                              <div
                                key={criteria}
                                className={`flex items-center gap-2 text-sm ${getValidationTextColor(isPassed, isFailed)}`}
                              >
                                {isPassed && (
                    ...
                    <div className="mt-4 space-y-3">
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            handleStepInput(index, e.target.value)
                          }
                      ...
                      {validationResults[index] && (
                        ...
                        >
                          ...
                          {/* Specific Errors */}
                          {validationResults[index].specificErrors.length >
                            0 && (
                            <div className="mt-3 space-y-1">
                              <div className="text-sm text-red-300 font-semibold">
                                Issues Found:
                              </div>
                              {validationResults[index].specificErrors.map(
                                (error) => (
                                  <div
                                    key={error}
                          ...
                          {validationResults[index].suggestions.length > 0 && (
                            <div className="mt-3 space-y-1">
                              ...
                              {validationResults[index].suggestions.map(
                                (suggestion) => (
                                  <div
                                    key={suggestion}
