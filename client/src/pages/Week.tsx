...
import LessonMasteryDisplay, {
  type LessonMastery,
} from "../components/LessonMasteryDisplay";
...
export default function Week() {
  ...
  >(new Map());
  const [labProgressMap, setLabProgressMap] = useState<Map<string, boolean>>(
    new Map(),
  );
  const [masteryMap, setMasteryMap] = useState<Map<string, LessonMastery>>(
    new Map(),
  );

  const { getLessonProgress, isLessonDueForReview, getLabProgress } =
    useProgress();
    ...
    async function fetchWeek() {
      ...
      try {
        ...
        if (weekNum > 1) {
          ...
          if (user?.uid) {
            try {
              const diagnosticDoc = await getDoc(
                doc(db, "diagnostics", user.uid),
              );
              if (diagnosticDoc.exists()) {
                const diagnosticData = diagnosticDoc.data();
                const suggestedStartWeek =
                  diagnosticData.suggestedStartWeek || 1;
                ...
                if (weekNum <= suggestedStartWeek) {
                  ...
                  console.log(
                    `🎯 Diagnostic allows access to Week ${weekNum} (recommended start: Week ${suggestedStartWeek})`,
                  );
                }
              }
            } catch (error) {
          ...
          if (!diagnosticAllowsAccess) {
            try {
              ...
              const prevTotalItems =
                prevWeekData.lessons.length + prevWeekData.labs.length;
              const prevCompletedItems =
                prevWeekData.lessons.filter((l: { id: string }) =>
                  completedLessons.has(l.id),
                ).length +
                prevWeekData.labs.filter((l: Lab) => completedLabs.has(l.id))
                  .length;
              ...
              if (prevPercentage < 80) {
                console.log(
                  `🔒 Week ${weekNum} locked. Previous week only ${prevPercentage}% complete (need 80%)`,
                );
                navigate("/curriculum");
                return;
              }
        ...
        try {
          ...
          if (weekData) {
            ...
            for (const lesson of weekData.lessons) {
              ...
              if (user?.uid) {
                const masteryRef = doc(
                  db,
                  "masteryProgress",
                  `${user.uid}_${lesson.lessonId}`,
                );
                const masterySnap = await getDoc(masteryRef);

                if (masterySnap.exists()) {
                  const firestoreMastery =
                    masterySnap.data() as FirestoreMastery;

                  // Convert Firestore mastery to display format
                  const getCrawlStatus = () => {
                    if (
                      firestoreMastery.crawl.perfectCompletions >=
                      firestoreMastery.crawl.requiredPerfectCompletions
                    )
                      return "completed";
                  ...
                  const getWalkStatus = () => {
                    if (
                      firestoreMastery.walk.perfectCompletions >=
                      firestoreMastery.walk.requiredPerfectCompletions
                    )
                      return "completed";
                  ...
                  const getRunGuidedStatus = () => {
                    if (
                      firestoreMastery.runGuided.perfectCompletions >=
                      firestoreMastery.runGuided.requiredPerfectCompletions
                    )
                      return "completed";
                    if (firestoreMastery.runGuided.unlocked)
                      return "in_progress";
                    return "locked";
                  };
                  ...
                  const getRunIndependentStatus = () => {
                    if (
                      firestoreMastery.runIndependent.perfectCompletions >=
                      firestoreMastery.runIndependent.requiredPerfectCompletions
                    )
                      return "completed";
                    if (firestoreMastery.runIndependent.unlocked)
                      return "in_progress";
                    return "locked";
                  };
                  ...
                  const displayMastery: LessonMastery = {
                    levels: {
                      ...
                      runIndependent: {
                        ...
                        requiredPerfect:
                          firestoreMastery.runIndependent
              ...
              {week
                ? Math.round(
                    ((lessonProgressMap.size + labProgressMap.size) /
                      (week.lessons.length + week.labs.length)) *
                      100,
                  )
                : 0}
              %
        ...
        <div className="mb-8">
          <div className="bg-gradient-to-br from-blue-900 to-cyan-900 rounded-xl p-8 border-2 border-blue-500">
            <h2 className="text-2xl font-bold text-white mb-3 flex items-center">
              <span className="bg-blue-400 rounded-full p-2 mr-3">📊</span>{" "}
              Skill Assessment
            </h2>
            <p className="text-blue-100 mb-6 text-lg">
              Measure your knowledge growth before and after completing this
              week's content.
            </p>
            <div className="grid md:grid-cols-2 gap-4">
              <div className="bg-blue-800/30 rounded-lg p-5 border border-blue-600/50">
                <h3 className="text-white font-semibold mb-2 flex items-center gap-2">
                  <span className="text-2xl">📝</span> Pre-Assessment
                </h3>
                <p className="text-blue-200 text-sm mb-4">
                  Take this before starting the week to establish your baseline
                  knowledge.
                </p>
                <Link
              ...
              <div className="bg-cyan-800/30 rounded-lg p-5 border border-cyan-600/50">
                <h3 className="text-white font-semibold mb-2 flex items-center gap-2">
                  <span className="text-2xl">🎓</span> Post-Assessment
                </h3>
                <p className="text-cyan-200 text-sm mb-4">
                  Complete this after finishing the week to see how much you've
                  learned!
                </p>
                <Link
      ...
      {week.weekNumber <= 5 && (
        <div className="mb-8">
          <div className="bg-gradient-to-br from-purple-900 to-pink-900 rounded-xl p-8 border-2 border-purple-500">
            <h2 className="text-2xl font-bold text-white mb-3 flex items-center">
              <span className="bg-purple-400 rounded-full p-2 mr-3">🚀</span>
              Week {week.weekNumber} Project
            </h2>
            <p className="text-purple-100 mb-4 text-lg">
              Apply what you've learned in a real-world hands-on project. Build
              your portfolio and earn XP!
            </p>
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-amber-100 mb-4 text-lg">
                  Test your understanding of this week's concepts. Score 70% or
                  higher to earn 150 XP!
                </p>
