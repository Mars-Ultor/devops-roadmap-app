...
export default function FailureLog() {
  ...
  const analyzePatterns = useCallback(() => {
    const categoryMap = new Map<
      string,
      { count: number; solutions: Set<string> }
    >();
    ...
    categoryMap.forEach((value, category) => {
      if (value.count >= 3) {
        // Pattern detected if 3+ failures in same category
  ...
  const addEntry = async () => {
    ...
    try {
      const nextEntryNumber =
        entries.length > 0
          ? Math.max(...entries.map((e) => e.entryNumber)) + 1
          : 1;
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              ...
              <div className="bg-slate-800 rounded-lg border border-slate-700 p-4">
                ...
                <div className="text-3xl font-bold text-orange-400">
                  {Math.round(totalTimeWasted / 60)}h
                </div>
              </div>
              <div className="bg-slate-800 rounded-lg border border-slate-700 p-4">
                <div className="text-sm text-slate-400 mb-1">
                ...
                <div className="space-y-2">
                  {patterns.map((pattern) => (
                    <div key={pattern.category} className="text-slate-300">
                      ...
                      {pattern.count} failures detected. Common solution:{" "}
                      {pattern.commonSolutions[0]}
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, whatBroke: e.target.value })
                      }
                      className="w-full px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white"
                      rows={3}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            timeWasted: parseInt(e.target.value) || 0,
                          })
                        }
                        className="w-full px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white"
                      />
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, keyLesson: e.target.value })
                      }
                      className="w-full px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white"
                      rows={2}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, quickCheck: e.target.value })
                      }
              ...
              {filteredEntries.length === 0 ? (
                <div className="bg-slate-800 rounded-lg border border-slate-700 p-12 text-center">
                  ...
                  <p className="text-slate-500">
                    Start documenting your mistakes to build your personal
                    runbook
                  </p>
                </div>
              ) : (
