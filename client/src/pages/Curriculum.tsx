...
function calculateWeekProgress(
  ...
  const completedLessonsCount = (week.lessons as LeveledLessonContent[]).filter(
    (lesson) => lesson.lessonId && completedLessons.has(lesson.lessonId),
  ).length;
...
function getWeekCardClassName(
  isLocked: boolean,
  isRecommended: boolean,
  weekNumber: number,
  recommendedStartWeek: number | null,
): string {
  if (isLocked) {
...
export default function Curriculum() {
  ...
  const [weekProgress, setWeekProgress] = useState<Map<number, WeekProgress>>(
    new Map(),
  );
  const [loading, setLoading] = useState(true);
  const [recommendedStartWeek, setRecommendedStartWeek] = useState<
    number | null
  >(null);
  const [diagnosticStartWeek, setDiagnosticStartWeek] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    async function fetchCurriculum() {
      try {
        ...
        console.log(
          "Curriculum: Received weeks data:",
          weeksData.length,
          "weeks",
        );
        console.log(
          "Curriculum: Week numbers:",
    ...
    async function fetchProgress(weeksData: Week[]) {
      ...
      try {
        ...
        weeksData.forEach((week) => {
          progressMap.set(
            week.weekNumber,
            calculateWeekProgress(week, completedLessons, completedLabs),
          );
        });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-12">
          ...
          <p className="text-xl text-gray-300 max-w-3xl">
            From zero to job-ready in 3 months. Complete 33 hands-on labs, build
            real-world projects, and earn 6000+ XP on your journey to a DevOps
            career.
          </p>
        </div>
        ...
        <div className="relative bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 rounded-2xl p-8 mb-12 overflow-hidden shadow-2xl">
          ...
          <div className="relative">
            ...
            <p className="text-indigo-50 mb-6 text-lg leading-relaxed max-w-3xl">
              Build a cloud-native resume that showcases your AWS, Terraform,
              and CI/CD skills. This capstone project spans Weeks 7-9 and is
              your ticket to standing out to employers. Earn{" "}
              <span className="font-bold text-yellow-300">1800 XP</span> upon
              completion!
            </p>
            <Link
              to="/projects"
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-green-100 text-sm">
                          Based on your diagnostic quiz results, start here!
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex items-start justify-between mb-6">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-3">
                      <div
                        className={`w-12 h-12 rounded-lg flex items-center justify-center font-bold text-lg shadow-lg ${
                          isLocked
                      ...
                      <div>
                        <h3
                          className={`text-2xl font-bold ${isLocked ? "text-gray-500" : "text-white"}`}
                        >
                          {week.title}
                        </h3>
                        ...
                        {isLocked && (
                          <span className="inline-block mt-1 bg-slate-700 text-gray-400 px-3 py-1 rounded-full text-xs font-bold">
                            🔒 LOCKED - Complete Week {week.weekNumber - 1}{" "}
                            (80%+)
                          </span>
                        )}
                      </div>
                    ...
                    <p
                      className={`mb-6 text-lg ${isLocked ? "text-gray-500" : "text-gray-300"}`}
                    >
                      {week.description}
                    </p>
                    ...
                    <div className="mb-6 bg-slate-900 rounded-lg p-4 border border-slate-700">
                      <h5 className="text-sm font-bold text-indigo-300 mb-3 uppercase tracking-wide">
                        Learning Objectives
                      </h5>
                      <ul className="space-y-2">
                        {week.objectives.map((obj) => (
                          ...
                          >
                            <span className="text-indigo-400 mr-3 mt-1 text-lg">
                              ✓
                            </span>
                            <span>{obj}</span>
                          </li>
                  ...
                  <div className="flex flex-col items-end space-y-3 ml-6">
                    <div className="text-right">
                      ...
                      <div
                        className={`text-2xl font-bold ${getProgressTextColor(percentage)}`}
                      >
                        {percentage}%
                      </div>
                      {progress && (
                        <div className="text-xs text-gray-500 mt-1">
                          ...
                          lessons · {progress.completedLabs}/
                          {progress.totalLabs} labs
                        </div>
                      )}
                    </div>
                    {isLocked ? (
