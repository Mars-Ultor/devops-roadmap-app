...
export default function Analytics() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "velocity" | "mastery" | "predictions"
  >("overview");
  ...
  const loadStudySessionsData = useCallback(
    async (dateFilter: Date, data: Partial<AnalyticsData>) => {
      ...
      data.avgSessionDuration =
        sessionsSnap.size > 0 ? totalTime / sessionsSnap.size : 0;
  ...
  const loadBattleDrillData = useCallback(
    async (data: Partial<AnalyticsData>) => {
      ...
      data.battleDrillAvgTime =
        drillCount > 0 ? totalDrillTime / drillCount : 0;
      data.battleDrillSuccessRate =
        drillPerfSnap.size > 0 ? successfulDrills / drillPerfSnap.size : 0;
    },
    [user?.uid],
  );
  ...
  const loadProgressData = useCallback(
    async (data: Partial<AnalyticsData>) => {
      ...
      data.masteryRate =
        progressSnap.size > 0 ? runIndependent / progressSnap.size : 0;
  ...
  const loadFailureData = useCallback(
    async (data: Partial<AnalyticsData>) => {
