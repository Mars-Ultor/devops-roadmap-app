  ...
  return (
    <div className="mb-8">
      ...
      <p className="text-gray-400">
        Limited resets encourage learning from failures. Tokens refresh every
        Monday.
      </p>
    </div>
  );
      ...
      {allocation && (
        <div className="bg-gray-800 border border-gray-700 rounded-lg p-4 mb-4">
          <div className="flex items-center justify-between text-sm text-gray-400">
            ...
            <span>
              Resets refresh: {allocation.weekEnd.toLocaleDateString()}
            </span>
          </div>
        </div>
      )}
    ...
    <div className="mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <StatCard
          ...
          label="Total Resets"
          value={stats.totalResetsUsed}
          color="text-white"
        />
        <StatCard
          icon={<Target className="w-8 h-8 text-blue-600" />}
          label="Quiz Resets"
          value={stats.quizResetsUsed}
          color="text-blue-400"
        />
        <StatCard
          icon={<Target className="w-8 h-8 text-purple-600" />}
          label="Lab Resets"
          value={stats.labResetsUsed}
          color="text-purple-400"
        />
        <StatCard
          icon={<TrendingDown className="w-8 h-8 text-yellow-600" />}
          label="Avg/Week"
          value={stats.averageResetsPerWeek.toFixed(1)}
          color="text-yellow-400"
        />
      </div>
      {stats.itemsMostReset.length > 0 && (
        <MostResetItems items={stats.itemsMostReset} />
      )}
    </div>
  );
}
...
function MostResetItems({
  items,
}: {
  readonly items: TokenUsageStats["itemsMostReset"];
}) {
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
      ...
      <div className="mt-4 p-3 bg-orange-900/20 border border-orange-800 rounded-lg">
        <p className="text-sm text-orange-300">
          💡 <span className="font-semibold">Tip:</span> Items requiring
          multiple resets may need additional study.
        </p>
      </div>
    </div>
  );
                  ...
                  <td className="px-4 py-3">
                    ...
                    {reset.weekNumber && (
                      <div className="text-xs text-gray-500">
                        Week {reset.weekNumber}
                      </div>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`text-sm font-medium ${getTypeColor(reset.type)}`}
                    >
            ...
            <li className="flex items-start gap-2">
              ...
              <span>
                <span className="font-semibold">
                  Limited resets simulate real-world consequences
                </span>{" "}
                - In production, you can't just restart without cost.
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-yellow-400 mt-1">→</span>
              <span>
                ...
                - Review your After Action Reports and Failure Log before
                resetting.
              </span>
            </li>
            <li className="flex items-start gap-2">
              ...
              <span>
                <span className="font-semibold">
                  30-minute cooldown encourages reflection
                </span>{" "}
                - Don't immediately retry. Analyze what went wrong first.
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-yellow-400 mt-1">→</span>
              <span>
                <span className="font-semibold">Discipline builds mastery</span>{" "}
                - The best operators rarely need resets because they learn from
                each attempt.
              </span>
            </li>
          </ul>
