...
function determineEnvironment(title: string, description: string): string {
  ...
  if (content.includes('docker')) return 'Linux system with Docker Engine installed';
  if (content.includes('kubernetes')) return 'Kubernetes cluster with kubectl access';
  if (content.includes('terraform')) return 'System with Terraform CLI and cloud provider access';
  if (content.includes('ansible')) return 'Control node with Ansible installed and SSH access to targets';
  ...
  if (content.includes('aws') || content.includes('cloud')) return 'AWS account with appropriate IAM permissions';
