...
function validateDockerfileBaseImage(input: string): CriterionValidation {
  ...
  return {
    ...
    error: hasBaseImage
      ? undefined
      : "FROM instruction not found in Dockerfile",
...
function validateDockerfileCopyRun(input: string): CriterionValidation {
  ...
  return {
    ...
    error: !hasCopy
      ? "Missing COPY/ADD instruction"
      : !hasRun
        ? "Missing RUN instruction"
        : undefined,
    suggestion:
      "Include both COPY (to add files) and RUN (to execute commands)",
  };
}
...
function validateBuildSuccess(input: string): CriterionValidation {
  const buildSuccess =
    input.includes("successfully built") ||
  ...
  return {
    ...
    error: buildSuccess
      ? undefined
      : "Build failed or did not complete successfully",
    suggestion:
      "Check build output for errors. Common issues: missing files, syntax errors in Dockerfile",
  };
}
...
function validatePortAccessible(input: string): CriterionValidation {
  ...
  return {
    ...
    error: isAccessible
      ? undefined
      : "Application not accessible on specified port",
    suggestion:
      "Test with: curl localhost:8080 or visit http://localhost:8080 in browser",
  };
}

function validateHealthCheck(input: string): CriterionValidation {
  const healthOk =
    input.includes("200") || input.includes("healthy") || input.includes("ok");
  return {
    passed: healthOk,
...
function validateNoLogErrors(input: string): CriterionValidation {
  ...
  return {
    ...
    suggestion: hasErrors
      ? "Review logs and fix errors before proceeding"
      : undefined,
  };
}
...
function validateRollback(input: string): CriterionValidation {
  ...
  return {
    ...
    suggestion:
      "Use kubectl rollout undo or docker tag/redeploy previous image",
  };
}

function validateFirewall(input: string): CriterionValidation {
  const firewallSet =
    input.includes("firewall") ||
    input.includes("ufw") ||
  ...
  return {
    ...
    suggestion:
      "Configure with: ufw allow 80/tcp or iptables -A INPUT -p tcp --dport 80 -j ACCEPT",
  };
}
...
function validateBackup(input: string): CriterionValidation {
  ...
  return {
    ...
    suggestion:
      "Create backup with: docker exec db mysqldump > backup.sql or pg_dump",
  };
}
...
function validateKeywords(
  ...
  return {
    ...
    suggestion: passed
      ? undefined
      : "Review the requirement and verify your implementation matches",
  };
}
...
const dockerfileValidators: ValidatorEntry[] = [
  {
    match: (c) => c.includes("exists"),
    validate: (i) => validateDockerfileExists(i),
  },
  {
    match: (c) => c.includes("base image"),
    validate: (i) => validateDockerfileBaseImage(i),
  },
  {
    match: (c) => c.includes("copy") && c.includes("run"),
    validate: (i) => validateDockerfileCopyRun(i),
  },
  {
    match: (c) => c.includes("expose"),
    validate: (i) => validateDockerfileExpose(i),
  },
  {
    match: (c) => c.includes("cmd") || c.includes("entrypoint"),
    validate: (i) => validateDockerfileCmd(i),
  },
];

const buildValidators: ValidatorEntry[] = [
  {
    match: (c) => c.includes("build") && c.includes("successfully"),
    validate: (i) => validateBuildSuccess(i),
  },
...
const containerValidators: ValidatorEntry[] = [
  {
    match: (c) => c.includes("container") && c.includes("starts"),
    validate: (i) => validateContainerStarts(i),
  },
  {
    match: (c) => c.includes("accessible") && c.includes("port"),
    validate: (i) => validatePortAccessible(i),
  },
  {
    match: (c) => c.includes("health check") && c.includes("200"),
    validate: (i) => validateHealthCheck(i),
  },
  {
    match: (c) => c.includes("logs") && c.includes("no") && c.includes("error"),
    validate: (i) => validateNoLogErrors(i),
  },
  {
    match: (c) => c.includes("environment") && c.includes("set"),
    validate: (i) => validateEnvVars(i),
  },
];

const gitValidators: ValidatorEntry[] = [
  {
    match: (c) => c.includes("git") && c.includes("commit"),
    validate: (i) => validateGitCommit(i),
  },
  {
    match: (c) => c.includes("rollback") || c.includes("previous version"),
    validate: (i) => validateRollback(i),
  },
];

const securityValidators: ValidatorEntry[] = [
  {
    match: (c) => c.includes("firewall") && c.includes("configured"),
    validate: (i) => validateFirewall(i),
  },
  {
    match: (c) => c.includes("ssl") || c.includes("https"),
    validate: (i) => validateSSL(i),
  },
];

const infraValidators: ValidatorEntry[] = [
  {
    match: (c) => c.includes("replica") || c.includes("scale"),
    validate: (i) => validateScaling(i),
  },
  {
    match: (c) => c.includes("pipeline") && c.includes("created"),
    validate: (i) => validatePipeline(i),
  },
  {
    match: (c) => c.includes("backup") && c.includes("created"),
    validate: (i) => validateBackup(i),
  },
];
  ...
  if (criterionLower.includes("dockerfile")) {
    for (const v of dockerfileValidators) {
      if (v.match(criterionLower)) {
        return (
          v.validate(inputLower, criterionLower) ??
          validateKeywords(inputLower, criterionLower)
        );
      }
    }
  }
