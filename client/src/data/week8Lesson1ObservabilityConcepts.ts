...
export const week8Lesson1ObservabilityConcepts: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description: 'Master the three pillars of observability and learn to build observable systems that enable rapid debugging and proactive issue detection.',
    learningObjectives: [
  ...
  crawl: {
    introduction: 'Master observability fundamentals: the three pillars (metrics, logs, traces) and why production systems need comprehensive visibility.',
    steps: [
      {
        stepNumber: 1,
        instruction: 'Why Observability? Understanding the production blindness problem',
        command: 'Scenario: Application deployed to production\n- Users report "site is slow"\n- No visibility into what\'s happening inside\n- Can\'t answer: Which service? What resource? Why now?\n\nObservability = ability to understand internal system state from external outputs',
        explanation: 'Traditional monitoring alerts you that something is wrong. Observability lets you investigate WHY it\'s wrong. In microservices with 10+ services, you need to trace requests across systems, correlate metrics with logs, and identify root causes quickly.',
        expectedOutput: 'Understanding: Observability provides the data needed to debug unknown problems in production systems.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Pillar 1 - Metrics: Numerical measurements over time\n- CPU usage: 45%, 62%, 78%\n- Request rate: 1000 req/sec\n- Error rate: 0.5%\n- Response time: p95 = 250ms\n\nGood for: Aggregation, trending, alerting',
        explanation: 'Metrics are numbers that change over time. Low storage cost (just numbers). Great for dashboards and alerts. Example: "API response time increased from 100ms to 500ms" tells you there\'s a problem but not why.',
        expectedOutput: 'Understanding: Metrics provide quantitative data about system performance and health.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Logs: Timestamped event messages\n- 2025-12-11 10:32:15 ERROR: Database connection failed: timeout\n- 2025-12-11 10:32:16 INFO: Retrying connection (attempt 2/3)\n- 2025-12-11 10:32:17 ERROR: User checkout failed - order_id: 12345\n\nGood for: Debugging specific events, context-rich investigation',
        explanation: 'Logs tell the story of what happened. Rich context (user IDs, error messages, stack traces). Higher storage cost than metrics. Example: Log shows "DatabaseConnectionException: Connection pool exhausted" - that\'s your root cause.',
        expectedOutput: 'Understanding: Logs provide detailed context about discrete events in your system.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Distributed Trace: Single request through microservices\nRequest ID: abc-123\n1. API Gateway: 5ms\n2. Auth Service: 50ms\n3. Product Service: 200ms (SLOW!)\n4. Database Query: 180ms (bottleneck found!)\n5. Cache Service: 10ms\nTotal: 445ms\n\nGood for: Understanding request flow, finding bottlenecks',
        explanation: 'Traces follow a single request through multiple services. Each service adds a "span" showing its processing time. Reveals where time is spent. Example: Trace shows Product Service took 200ms, and 180ms was database - optimize that query.',
        expectedOutput: 'Understanding: Traces show request paths through distributed systems, revealing bottlenecks.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Counters track totals (requests served). Gauges track current state (memory usage). Histograms track distributions (most requests fast, few slow). Different types answer different questions. Prometheus uses these types.',
      ...
      },
      {
        ...
        command: 'Unstructured (bad):\nERROR: User 12345 checkout failed with error payment declined at 2025-12-11\n\nStructured (good - JSON):\n{\n  "timestamp": "2025-12-11T10:32:15Z",\n  "level": "ERROR",\n  "message": "checkout_failed",\n  "user_id": "12345",\n  "order_id": "67890",\n  "error": "payment_declined",\n  "service": "checkout"\n}\n\nStructured logs = queryable, filterable, aggregatable',
        explanation: 'Structured logging writes logs as JSON (or similar). Every field is parseable. Can query "all payment_declined errors" or "all errors for user 12345". Log aggregation tools (ELK, Splunk) parse structured logs automatically.',
        expectedOutput: 'Understanding: Structured logging enables powerful querying and analysis of log data.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Trace propagation:\n1. Request enters API Gateway\n   - Generate trace_id: "abc-123"\n   - Create span_id: "span-1"\n2. Gateway calls Auth Service\n   - Pass trace_id: "abc-123" in header\n   - New span_id: "span-2", parent: "span-1"\n3. Auth calls Database\n   - Same trace_id: "abc-123"\n   - New span_id: "span-3", parent: "span-2"\n\nAll spans with same trace_id form complete trace',
        explanation: 'Every service receives trace_id in HTTP headers (or gRPC metadata). Service creates new span for its work, linking to parent span. Traces sent to collector (Jaeger, Zipkin). Collector assembles complete trace from all spans.',
        expectedOutput: 'Understanding: Distributed tracing requires context propagation across all services.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Unified observability:\n1. Alert fires: "API latency p95 > 500ms" (METRIC)\n2. Dashboard shows spike at 10:32 AM\n3. Query logs for timestamp 10:32:00 - 10:33:00 (LOGS)\n4. Logs show trace_id: "abc-123" with errors\n5. Pull up trace "abc-123" (TRACE)\n6. Trace shows Database service took 480ms\n7. Database logs show slow query on products table\n\nRoot cause found: Unoptimized query',
        explanation: 'The three pillars work together. Metrics alert you. Logs provide context. Traces show the path. Correlation IDs (trace_id, request_id) link them all. This unified view enables rapid debugging.',
        expectedOutput: 'Understanding: Correlating metrics, logs, and traces enables efficient root cause analysis.',
        validationCriteria: [
      ...
      {
        stepNumber: 9,
        instruction: 'Service Level Objectives (SLOs): Defining reliability targets',
        ...
        explanation: 'SLO = target reliability level. SLI = metric you measure. Error budget = allowed failures. If success rate is 99.95% when target is 99.9%, you have spare budget for deployments. If you hit 99.9%, freeze deployments and fix reliability.',
        expectedOutput: 'Understanding: SLOs quantify acceptable reliability and guide operational decisions.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Alerts should be actionable (clear problem, clear fix), not informational. Alert on symptoms users see (latency, errors), not internal metrics (CPU). Include runbooks. Page only for urgent issues. Otherwise, alert fatigue causes real alerts to be ignored.',
        expectedOutput: 'Understanding: Effective alerts are actionable, user-focused, and documented with remediation steps.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Kubernetes observability challenges:\n1. Dynamic pods: IPs change, pods restart\n2. Multi-layer: Pod, container, node, cluster\n3. Networking: Service mesh adds complexity\n\nSolutions:\n- Metrics: Prometheus with service discovery\n- Logs: Aggregate from all pods (Fluentd, Fluent Bit)\n- Traces: Inject sidecars or use service mesh\n- Use labels to correlate (app=frontend, version=v1.2)',
        explanation: 'Kubernetes pods are ephemeral - IP changes when pod restarts. Need service discovery to find metrics endpoints. Must aggregate logs from all pod replicas. Labels enable correlation (all pods with app=frontend). Tools integrate with Kubernetes API.',
        expectedOutput: 'Understanding: Kubernetes observability requires tools that handle dynamic, ephemeral infrastructure.',
        validationCriteria: [
      ...
      {
        stepNumber: 12,
        instruction: 'Observability-Driven Development: Building observable systems',
        command: 'Observable system design:\n1. Instrument from day 1\n   - Add metrics to every service\n   - Structured logging everywhere\n   - Trace context propagation\n2. Design for debugging\n   - Include correlation IDs\n   - Log state transitions\n   - Expose health/ready endpoints\n3. Think like an operator\n   - "How will I debug this at 2 AM?"\n   - "What data do I need for root cause analysis?"\n\nObservability is a feature, not an afterthought',
        explanation: 'Building observability in from the start is cheaper than adding it later. Every service should expose metrics (/metrics), health checks (/health), and emit structured logs. Developers should test observability (can I debug this with the data available?).',
        expectedOutput: 'Understanding: Observability should be designed into systems, not bolted on after production issues.',
        validationCriteria: [
  ...
  runGuided: {
    objective: 'Design and implement complete observability platform with metrics, logs, traces, and correlation',
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck: 'Start with Prometheus for metrics, Loki for logs, Jaeger for traces. Use Kubernetes service discovery for dynamic pods. Define sampling rate (e.g., 10% of traces to control cost). Separate hot storage (7 days) from cold storage (90 days).'
      },
      {
        ...
        hintIfStuck: 'Use OpenTelemetry libraries for standardization. Include correlation IDs in all logs. Track both technical (latency) and business (conversion) metrics. Test instrumentation locally before deploying.'
      },
      {
        ...
        hintIfStuck: 'Use Prometheus Operator for Kubernetes integration. Create dashboard for each service (latency, errors, throughput). Test alerting by triggering threshold violations. Validate trace propagation across all services.'
      }
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective: 'Build production-grade observability platform enabling 10-minute mean time to resolution for a multi-service e-commerce application',
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 30,
        passingThreshold: 'All three pillars implemented (metrics, logs, traces). Services fully instrumented with consistent standards. No observability blind spots. Can monitor all key user journeys.'
      },
      {
        criterion: 'Incident Response Capability',
        weight: 25,
        passingThreshold: 'Dashboards provide actionable insights. Can correlate metrics → logs → traces. Demonstrate complete investigation workflow from alert to root cause in under 10 minutes.'
      },
      {
        criterion: 'Reliability Engineering',
        weight: 25,
        passingThreshold: 'SLOs defined for key services. Error budget tracking working. Alerting tuned to avoid fatigue (no false positives). Runbooks guide responders effectively.'
      },
      {
        criterion: 'Operational Excellence',
        weight: 20,
        passingThreshold: 'Cost-optimized with retention and sampling. Documentation enables team self-service. High availability configured. Security best practices followed (RBAC, encryption).'
      }
    ]
  },
