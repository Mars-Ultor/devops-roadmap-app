...
export const week7Lesson2KubernetesBasics: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description:
      "Master core Kubernetes concepts through hands-on practice with kubectl, pods, deployments, and services.",
    learningObjectives: [
  ...
  crawl: {
    introduction:
      "Get hands-on with Kubernetes by setting up a local cluster and practicing with core objects: pods, deployments, and services.",
    steps: [
      {
        stepNumber: 1,
        instruction:
          "Install and start a local Kubernetes cluster with Docker Desktop or Minikube",
        command:
      ...
      },
      {
        ...
        explanation:
          "`kubectl run` creates a pod quickly for testing (not for production - use Deployments). The pod pulls the nginx image from Docker Hub and starts it. `--watch` monitors status changes. `describe` shows events (pulling image, starting container). `logs` shows container stdout. `exec` runs commands in the container, `-it` makes it interactive (like docker exec). This is how you debug running pods.",
        expectedOutput:
          "Pod transitions from Pending → ContainerCreating → Running, nginx logs show startup, exec shows nginx version",
        validationCriteria: [
      ...
      },
      {
        ...
        expectedOutput:
          "3 nginx pods created, all Running, deployment shows 3/3 ready replicas",
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          "Services provide stable network endpoints for dynamic pods. `selector: app: nginx` makes the Service find all pods with that label. The Service gets a cluster IP (virtual IP) and DNS name (nginx-service.default.svc.cluster.local). `port: 80` is the Service port, `targetPort: 80` is the container port. `type: ClusterIP` means internal only (default). Endpoints show actual pod IPs the Service load balances across.",
        expectedOutput:
          "Service created with ClusterIP, endpoints list shows 2 pod IPs (current replica count), service DNS name resolves",
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          "Changing the image triggers a rolling update. Kubernetes creates new pods with nginx:1.25 while keeping old nginx:latest pods running. Once new pods are Ready, old pods terminate. This ensures zero downtime. `rollout status` tracks progress. `rollout history` shows revision history. You'll see new pods created, old pods terminated gradually. This is how production deployments work - update declaratively, Kubernetes handles the rollout.",
        expectedOutput:
          "New pods created with nginx:1.25, old pods terminated after new ones ready, no downtime, rollout succeeds",
        validationCriteria: [
      ...
      },
      {
        ...
        expectedOutput:
          "Bad image causes ImagePullBackOff, rollback restores previous working version, pods return to Running",
        validationCriteria: [
    ...
    expectedOutcome:
      "Hands-on proficiency with kubectl, creating pods and deployments, scaling, exposing with services, performing rolling updates, and rollbacks.",
  },
  walk: {
    introduction:
      "Apply Kubernetes knowledge to practical scenarios, completing manifests and troubleshooting common issues.",
    exercises: [
      {
        exerciseNumber: 1,
        scenario:
          "Create a complete Deployment and Service for a Node.js API. Fill in the missing configuration.",
...
spec:
  ...
  type: ClusterIP`,
        explanation:
          "Deployments use apps/v1 API. Selector matchLabels must match template labels exactly. Service selector must match pod labels. Service port (external) can differ from targetPort (container), enabling flexibility. ClusterIP for internal services, LoadBalancer for external.",
      },
      {
        exerciseNumber: 2,
        scenario:
          "You deployed an application but `kubectl get pods` shows ImagePullBackOff. Debug and fix the issue.",
...
3. If image tag is wrong: ___________`,
        blanks: [
          ...
          },
          {
            ...
            correctValue:
              "Image doesn't exist or tag is wrong (typo in image name or tag)",
            validationPattern:
              "image.*doesn.*t.*exist|wrong.*tag|typo|not.*found",
          },
          {
            id: "cause_2",
            ...
            correctValue:
              "Image is in private registry and no credentials provided",
            validationPattern:
              "private.*registry|credentials|authentication|secret",
          },
          {
            id: "cause_3",
          ...
          },
          {
            ...
            correctValue:
              "Verify image exists, fix image name/tag, rebuild and push image, update deployment",
            validationPattern:
              "verify.*image|fix.*tag|push.*image|build|update.*deployment",
          },
          {
            id: "solution_private",
            ...
            correctValue:
              "Create imagePullSecret with registry credentials, reference in deployment spec.imagePullSecrets",
            validationPattern:
              "imagePullSecret|secret|credentials|registry.*auth",
          },
          {
            ...
            hint: "How to fix tag",
            correctValue:
              "Update image tag in deployment (kubectl set image or edit YAML), verify tag exists in registry",
...
3. If image tag is wrong: Update image tag (kubectl set image deployment/myapp myapp=myregistry.com/myapp:v1.0.1)`,
        explanation:
          "ImagePullBackOff means Kubernetes can't pull the container image. Common causes: wrong image name/tag, private registry without credentials, network issues. Use kubectl describe pod to see exact error. For private registries, create imagePullSecret and reference it in pod spec.",
      },
      {
        exerciseNumber: 3,
        scenario:
          "Scale a deployment manually and with HorizontalPodAutoscaler. Complete the configurations.",
...
- Maximum pods: 10`,
        explanation:
          "Manual scaling with kubectl scale or editing replicas in YAML is static. HPA provides automatic scaling based on metrics (CPU, memory, custom). HPA continuously monitors metrics and adjusts replicas between min and max to maintain target utilization. Requires metrics-server installed in cluster.",
      },
      {
        exerciseNumber: 4,
        scenario:
          "A Service is not routing traffic to pods. Diagnose and fix the configuration.",
        template: `Deployment:
apiVersion: apps/v1
kind: Deployment
metadata:
    ...
    targetPort: ___________`,
        blanks: [
          ...
          },
          {
            ...
            correctValue:
              'Selector "app: web" doesn\'t match pod label "app: website" - no endpoints',
            validationPattern:
              "doesn.*t.*match|wrong.*label|mismatch|web.*website",
          },
          {
...
spec:
  ...
  - protocol: TCP
    ...
    targetPort: 80`,
        explanation:
          "Service selector must exactly match pod labels. If selector doesn't match, Service has no endpoints (no pods to route to). Use kubectl get endpoints to verify Service found pods. targetPort must match the actual container port. Service port (external) can differ from targetPort (internal).",
      },
      {
        exerciseNumber: 5,
        scenario:
          "Perform a rolling update and rollback. Complete the workflow.",
...
- maxUnavailable: ___________`,
        blanks: [
          ...
          },
          {
            ...
            validationPattern:
              "kubectl.*(describe.*deployment|get.*pod.*yaml).*image",
          },
          {
            id: "undo_command",
          ...
          },
          {
            ...
            validationPattern:
              "extra.*pod|above.*desired|additional.*pod|create.*during",
          },
          {
...
- maxUnavailable: Maximum number of pods that can be unavailable during update (0 means all pods must be available throughout update - true zero downtime)`,
        explanation:
          "Rolling updates enable zero-downtime deployments by gradually replacing pods. maxSurge controls how many extra pods can exist during update. maxUnavailable controls how many pods can be down. Setting maxUnavailable=0 ensures all pods remain available throughout update. Rollback with kubectl rollout undo reverts to previous revision.",
      },
    ],
    hints: [
  ...
  runGuided: {
    objective:
      "Deploy a complete multi-tier application to Kubernetes with proper networking, configuration, and operational practices.",
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        description:
          "Design 3-tier architecture: frontend (React/nginx), API (Node.js/Python), database (PostgreSQL/MySQL). Create Deployment manifests for frontend and API, StatefulSet for database. Define resource requests/limits.",
        validationCriteria: [
        ...
        hintIfStuck:
          "Start with frontend: Deployment with nginx image, 3 replicas, containerPort 80, resources, readiness probe (httpGet /index.html). API: similar but with your API image and port. Database: StatefulSet with PVC for persistent storage.",
      },
      {
        checkpoint: "Networking configured and services accessible",
        description:
          "Create Services for each tier. Frontend: LoadBalancer or Ingress for external access. API: ClusterIP for frontend to reach. Database: ClusterIP for API to reach. Verify connectivity between tiers.",
        validationCriteria: [
        ...
        hintIfStuck:
          "Services use selectors matching pod labels. Frontend Service: type LoadBalancer, selector app=frontend. API Service: type ClusterIP, selector app=api. Test with kubectl run test-pod -- curl api-service to verify internal connectivity.",
      },
      {
        checkpoint: "Configuration and operational readiness",
        description:
          "Externalize configuration with ConfigMaps and Secrets. Implement health checks. Perform rolling update and rollback. Document kubectl commands for common operations.",
        validationCriteria: [
        ...
        hintIfStuck:
          "Create ConfigMap: kubectl create configmap app-config --from-literal=API_URL=http://api-service. Create Secret: kubectl create secret generic db-secret --from-literal=DB_PASSWORD=secret123. Mount in deployment with env.valueFrom.configMapKeyRef and secretKeyRef. Add readinessProbe httpGet to containers.",
      },
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective:
      "Independently design, deploy, and operate a production-ready multi-tier application on Kubernetes with complete configuration, networking, monitoring, and operational documentation.",
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 20,
        passingThreshold:
          "3-tier architecture with appropriate K8s objects (Deployments for stateless, StatefulSet for database), proper separation of concerns, labels used consistently for Service selectors",
      },
      {
        criterion: "High Availability",
        weight: 15,
        passingThreshold:
          "Stateless tiers have 3+ replicas, readiness/liveness probes configured, rolling update strategy with maxUnavailable=0 or 1, resource requests ensure proper scheduling",
      },
      {
        criterion: "Networking Configuration",
        weight: 15,
        passingThreshold:
          "Services configured correctly (ClusterIP for internal, LoadBalancer/Ingress for external), all tiers can communicate, external access works, DNS resolution verified",
      },
      {
        criterion: "Configuration Management",
        weight: 15,
        passingThreshold:
          "ConfigMaps used for non-sensitive config, Secrets for passwords/keys, configuration injected as env vars or volume mounts, no hardcoded values in manifests",
      },
      {
        criterion: "Resource Management",
        weight: 10,
        passingThreshold:
          "Resource requests and limits set for all containers, appropriate values (not too high or too low), prevents resource starvation and ensures scheduling",
      },
      {
        criterion: "Storage and Persistence",
        weight: 10,
        passingThreshold:
          "Database uses PersistentVolumeClaim with appropriate storage class and size, data persists across pod restarts",
      },
      {
        criterion: "Operational Readiness",
        weight: 15,
        passingThreshold:
          "Runbook documented with deploy/scale/update/rollback/logs/troubleshoot procedures, rolling update and rollback tested and working, health checks functional, logs accessible",
      },
    ],
  },
