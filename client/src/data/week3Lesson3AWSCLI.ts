...
export const week3Lesson3AWSCLI: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description:
      "Master AWS CLI for infrastructure automation and management from the terminal.",
    learningObjectives: [
  ...
  crawl: {
    introduction:
      "Learn AWS CLI step-by-step: installation, configuration, EC2 management, S3 operations, and automation.",
    steps: [
      {
        stepNumber: 1,
        ...
        explanation:
          "AWS CLI lets you control AWS from terminal. Alternative to web console. Essential for automation and scripting. Version 2 is current.",
      ...
      },
      {
        ...
        explanation:
          "Configuration stores credentials in ~/.aws/credentials and config in ~/.aws/config. Access keys from IAM user. Region determines where resources created. Output: json, text, or table.",
        expectedOutput:
          "Configuration saved. Subsequent commands use these credentials.",
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          "get-caller-identity shows your AWS account ID, user ARN, and user ID. Confirms credentials work. No cost, safe to run.",
        expectedOutput:
      ...
      },
      {
        ...
        explanation:
          "describe-instances shows all EC2 instances. --filters narrows results. --query extracts specific fields (JMESPath syntax). --output table makes readable.",
        expectedOutput:
          "Table showing instance IDs, types, states, and IP addresses",
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          "stop-instances shuts down instance (stops billing for compute, EBS storage still charged). start-instances powers on. describe-instance-status shows current state.",
        expectedOutput:
          "State transitions: running → stopping → stopped → pending → running",
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          "run-instances launches new EC2. Need: AMI ID (OS image), instance type (size), key pair (SSH access), security group (firewall), subnet (network).",
        expectedOutput:
          "JSON with instance details including instance ID, state (pending), private IP",
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          "s3 ls lists buckets or objects. Similar to Linux ls command. --recursive shows all objects in all subdirectories.",
        expectedOutput:
          "List of bucket names with creation dates, or list of objects with sizes and timestamps",
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          "s3 cp copies files to/from S3. s3 sync synchronizes directories (like rsync). Sync only transfers changed files, efficient for large directories.",
        expectedOutput:
          "Upload/download progress shown, completed successfully. Sync shows which files transferred.",
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          "s3 mb makes bucket (like mkdir). Bucket names must be globally unique. s3 rb removes bucket. --force deletes all objects first (dangerous).",
        expectedOutput:
          "make_bucket: my-unique-bucket-name-12345 (or error if name taken)",
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          "--query uses JMESPath to extract specific fields from JSON output. Powerful for filtering and transforming data. Combines with --output for readable results.",
        expectedOutput:
          "Filtered list showing only requested fields in specified format",
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          "Profiles manage multiple AWS accounts/regions. Each profile has own credentials and config. Switch with --profile flag or AWS_PROFILE environment variable.",
      ...
      },
      {
        ...
        explanation:
          "AWS CLI in scripts enables automation. Loop through resources, apply actions, save costs. Use cron for scheduling. Combine with jq for complex JSON processing.",
        expectedOutput:
          "Script stops all dev instances automatically, can run on schedule",
        validationCriteria: [
    ...
    expectedOutcome:
      "You can install and configure AWS CLI, manage EC2 instances (list, start, stop, launch), work with S3 (upload, download, sync), use filters and queries, manage multiple profiles, and automate tasks with scripts.",
  },
    ...
    exercises: [
      {
        ...
        explanation:
          "Profiles prevent credential mixing and enable multi-account management. Use --profile flag or AWS_PROFILE environment variable.",
      },
      {
        exerciseNumber: 2,
        scenario:
          "Find all running EC2 instances with tag Environment=production.",
        template:
          'aws ec2 __DESCRIBE_INSTANCES__ \\\n  --filters \\\n    "Name=__INSTANCE_STATE__,Values=running" \\\n    "Name=tag:Environment,Values=__PRODUCTION__" \\\n  --query "__QUERY__" \\\n  --output table',
        blanks: [
          {
        ...
        explanation:
          "Filters narrow results. JMESPath queries extract specific fields. Output format controls presentation.",
      },
      {
        exerciseNumber: 3,
        scenario:
          "Upload website files to S3 and make bucket public for static hosting.",
        template:
        ...
        explanation:
          "S3 sync uploads files efficiently. Static website hosting serves HTML/CSS/JS. Bucket policy controls public access.",
      },
      {
        exerciseNumber: 4,
        scenario:
          "Create script to stop all dev instances outside business hours.",
        template:
        ...
        explanation:
          "Automation saves costs by stopping unused dev instances. Combine with cron for scheduling (e.g., 6pm daily).",
      },
      {
        exerciseNumber: 5,
        ...
        explanation:
          "S3 sync creates local backups. Only downloads new/changed files. Schedule with cron for regular backups.",
      },
    ],
    hints: [
  ...
  runGuided: {
    objective:
      "Create automated infrastructure management scripts using AWS CLI",
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        description:
          "Script that stops/starts instances based on schedule or tags",
        validationCriteria: [
        ...
        hintIfStuck:
          'Filter instances with --filters "Name=tag:Schedule,Values=business-hours". Use cron: "0 18 * * * /path/to/stop-instances.sh" (6pm) and "0 8 * * * /path/to/start-instances.sh" (8am).',
      },
      {
        ...
        hintIfStuck:
          'Use "aws ec2 create-snapshot" for EBS. Use "aws s3 sync" for S3. Delete old snapshots: "aws ec2 delete-snapshot" with date filter. SNS for notifications.',
      },
      {
        checkpoint: "Reporting script developed",
        description:
          "Generate infrastructure reports (running instances, costs, security)",
        validationCriteria: [
        ...
        hintIfStuck:
          'Query all resources with filters. Check for public S3: "aws s3api get-bucket-acl". Find open security groups: check for 0.0.0.0/0. Format with awk/sed or convert JSON to HTML.',
      },
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective:
      "Build comprehensive AWS management toolkit with CLI scripts for common operations, documentation, and scheduling",
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 30,
        passingThreshold:
          "Scripts work correctly. Perform intended AWS operations. Error handling prevents failures. Dry-run mode for testing. Production-ready code quality.",
      },
      {
        criterion: "Automation Value",
        weight: 25,
        passingThreshold:
          "Solves real operational problems. Saves time on repetitive tasks. Cost optimization realized. Security posture improved. Scheduled for unattended execution.",
      },
      {
        criterion: "Code Quality",
        weight: 25,
        passingThreshold:
          "Clean, readable code. Reusable functions. Proper error handling. Logging with timestamps. Parameters for flexibility. Comments explain complex logic.",
      },
      {
        criterion: "Documentation",
        weight: 20,
        passingThreshold:
          "Clear README. Usage examples for each script. IAM permissions documented. Cron schedule examples. Troubleshooting guide. Someone else can use your toolkit.",
      },
    ],
  },
