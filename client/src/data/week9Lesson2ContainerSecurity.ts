...
export const week9Lesson2ContainerSecurity: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description:
      "Master container security practices including image scanning, runtime security, secrets management, and application-level security controls.",
    learningObjectives: [
  ...
  crawl: {
    introduction:
      "Learn container and application security step-by-step. You will scan images, manage secrets securely, and implement security controls.",
    expectedOutcome:
      "Complete understanding of container security: image scanning with Trivy, multi-stage builds, Pod Security Standards, External Secrets Operator, Falco runtime monitoring, and zero-trust networking",
    steps: [
      {
        stepNumber: 1,
        instruction: "Scan container image for vulnerabilities",
        command:
          "docker pull nginx:latest && trivy image nginx:latest --severity HIGH,CRITICAL",
        explanation:
          "Trivy scans container images for OS and application vulnerabilities. Always scan before deploying to production.",
      ...
      },
      {
        ...
        explanation:
          "Multi-stage builds reduce image size and attack surface. Alpine base image is minimal. Non-root user improves security.",
      ...
      },
      {
        ...
        explanation:
          "Security context enforces: non-root user, read-only filesystem, dropped capabilities, seccomp profile. These prevent container breakout.",
      ...
      },
      {
        ...
        explanation:
          "External Secrets Operator syncs secrets from external stores (AWS Secrets Manager, Vault) into Kubernetes. Secrets never in code.",
      ...
      },
      {
        ...
        explanation:
          "ExternalSecret syncs AWS Secrets Manager into Kubernetes Secret. Secrets rotate automatically.",
        ...
        commonMistakes: [
          "Not configuring IRSA for authentication",
          "Hardcoding secret ARNs",
        ],
      },
      {
        stepNumber: 6,
        ...
        command:
          "docker run -t owasp/zap2docker-stable zap-baseline.py -t https://myapp.example.com",
        explanation:
          "OWASP ZAP performs dynamic application security testing by actually making HTTP requests. Finds runtime issues like XSS, injection.",
        ...
        commonMistakes: [
          "Only running SAST, not DAST",
          "Not testing authenticated pages",
        ],
      },
      {
        stepNumber: 7,
        ...
        command:
          'cat <<EOF > csp-middleware.js\nconst helmet = require("helmet");\n\napp.use(\n  helmet.contentSecurityPolicy({\n    directives: {\n      defaultSrc: ["\'self\'"],\n      scriptSrc: ["\'self\'", "\'unsafe-inline\'", "cdn.example.com"],\n      styleSrc: ["\'self\'", "\'unsafe-inline\'"],\n      imgSrc: ["\'self\'", "data:", "https:"],\n      connectSrc: ["\'self\'", "api.example.com"],\n      fontSrc: ["\'self\'", "fonts.gstatic.com"],\n      objectSrc: ["\'none\'"],\n      upgradeInsecureRequests: [],\n    },\n  })\n);\nEOF',
        explanation:
          "CSP prevents XSS by whitelisting allowed sources for scripts, styles, images. Report-only mode helps test before enforcing.",
      ...
      },
      {
        ...
        explanation:
          "Falco monitors kernel events to detect suspicious behavior: shell in container, privilege escalation, unexpected network connections.",
        ...
        commonMistakes: [
          "Not routing alerts to SIEM",
          "Ignoring Falco warnings",
        ],
      },
      {
        stepNumber: 9,
        ...
        explanation:
          "Custom rules extend Falco to detect specific threats. Crypto mining is common in compromised containers.",
      ...
      },
      {
        ...
        explanation:
          "Network policies enforce zero-trust: pods can only communicate with explicitly allowed services. Default deny everything.",
      ...
      },
      {
        ...
        command:
          "cosign generate-key-pair && cosign sign --key cosign.key myapp:v1.0.0",
        explanation:
          "Image signing proves authenticity and integrity. Kubernetes can verify signatures before running containers.",
        ...
        commonMistakes: [
          "Not protecting private key",
          "Signing unsigned base images",
        ],
      },
      {
        stepNumber: 12,
        ...
        explanation:
          "Kyverno policy blocks unsigned/tampered images. Only images signed by trusted keys can run.",
        ...
        commonMistakes: [
          "Not testing with valid signatures first",
          "Public key mismatch",
        ],
      },
    ],
  ...
  walk: {
    introduction:
      "Apply container and application security through hands-on exercises.",
    exercises: [
      {
        exerciseNumber: 1,
        scenario:
          "Build secure multi-stage Docker image with vulnerability scanning.",
        template:
        ...
        solution:
          "Multi-stage build: builder stage has dev dependencies, final image only has production artifacts. Alpine reduces size. Non-root user (nodejs:1001) prevents privilege escalation. Trivy scan fails build if critical/high vulnerabilities found.",
        explanation:
          "Secure container images: minimal base, multi-stage build, non-root user, vulnerability scanning",
      },
      {
        exerciseNumber: 2,
        scenario:
          "Configure Kubernetes Pod Security Context with all recommended settings.",
        template:
        ...
        solution:
          "Security context: runAsNonRoot prevents root, readOnlyRootFilesystem prevents tampering, drop ALL capabilities minimizes attack surface, seccomp filters syscalls. Mount /tmp and /app/cache as emptyDir for writes.",
        explanation:
          "Pod Security Standards enforce defense-in-depth at container runtime",
      },
      {
        exerciseNumber: 3,
        scenario:
          "Set up External Secrets Operator to sync from AWS Secrets Manager.",
        template:
        ...
        solution:
          "External Secrets Operator: SecretStore configures AWS Secrets Manager connection. ExternalSecret syncs specific secrets into Kubernetes. Secrets auto-refresh every 1 hour. Never store secrets in Git or container images.",
        explanation:
          "External secrets management separates sensitive data from code and configs",
      },
      {
        exerciseNumber: 4,
        scenario:
          "Implement network policies for zero-trust pod communication.",
        template:
        ...
        solution:
          "Zero-trust network: Default deny all traffic. Explicitly allow: frontend → api:8080, api → database:5432, all → kube-system:53 (DNS). Every pod only communicates with necessary services.",
        explanation:
          "Network policies implement micro-segmentation and zero-trust networking",
      },
      {
        exerciseNumber: 5,
        ...
        solution:
          "Image signing workflow: 1) Generate key pair with cosign 2) Sign images in CI/CD 3) Deploy Kyverno policy with public key 4) Kubernetes rejects unsigned images. Protects against supply chain attacks.",
        explanation:
          "Image signing ensures authenticity and prevents tampering in the supply chain",
      },
    ],
    hints: [
  ...
  runGuided: {
    objective:
      "Build comprehensive container and application security platform with hardening, secrets management, and runtime protection",
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck:
          "Use node:18-alpine or gcr.io/distroless/nodejs. Multi-stage: builder + minimal runtime. Add non-root user (RUN adduser). Scan with: trivy image --severity CRITICAL,HIGH. Block deployment if vulnerabilities found.",
      },
      {
        ...
        hintIfStuck:
          "Install External Secrets Operator. Create SecretStore pointing to AWS Secrets Manager or Vault. Create ExternalSecret for each app. Mount as environment variables or files. Test rotation by updating secret in AWS.",
      },
      {
        ...
        hintIfStuck:
          "Label namespaces: pod-security.kubernetes.io/enforce=restricted. Add securityContext to all deployments. Create default-deny NetworkPolicy. Install Falco via Helm. Create Kyverno policy to verify image signatures.",
      },
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective:
      "Build production-ready container security platform with comprehensive hardening, secrets management, runtime monitoring, and supply chain security",
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 30,
        passingThreshold:
          "Multi-stage builds minimize attack surface. Alpine or Distroless bases. Non-root users. Read-only root filesystems. All capabilities dropped. Images scanned in CI/CD with zero high/critical CVEs.",
      },
      {
        criterion: "Secrets Management",
        weight: 20,
        passingThreshold:
          "External Secrets Operator or Vault deployed. All secrets synced from external store. Auto-rotation configured. Access controlled via RBAC. Zero secrets in Git, manifests, or images. Rotation tested.",
      },
      {
        criterion: "Runtime Security",
        weight: 30,
        passingThreshold:
          "Pod Security Standards enforced. Security contexts on all workloads. Network policies default deny. Falco monitoring all events. Alerts configured and routed. Image signatures verified. Non-compliant resources blocked.",
      },
      {
        criterion: "Application Security",
        weight: 20,
        passingThreshold:
          "DAST scanning integrated. Security headers implemented (CSP, HSTS, X-Frame-Options). Input validation prevents injection. Rate limiting prevents abuse. Comprehensive security testing in CI/CD.",
      },
    ],
  },
