...
export const week11Lesson2TerraformAdvanced: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description: 'Master advanced Terraform concepts including reusable modules, remote state, workspaces, and infrastructure testing.',
    learningObjectives: [
  ...
  crawl: {
    introduction: 'Learn advanced Terraform step-by-step. You will create modules, manage state, use workspaces, and test infrastructure code.',
    expectedOutcome: 'Complete understanding of Terraform modules, remote state with S3/DynamoDB, workspaces for environments, infrastructure testing with Terratest, and production best practices',
    steps: [
      {
        stepNumber: 1,
        instruction: 'Understand Terraform module structure',
        command: 'echo "Terraform Module Structure:\nmodules/vpc/\n  main.tf - Resources\n  variables.tf - Input variables\n  outputs.tf - Output values\n  README.md - Documentation\n  versions.tf - Provider versions\n\nBenefits:\n- Reusability across projects\n- Encapsulation of complexity\n- Versioning and testing\n- Team collaboration"',
        explanation: 'Modules are reusable Terraform configurations. Root module calls child modules with input variables, gets output values. Like functions in programming.',
        ...
        commonMistakes: ['Not separating modules properly', 'Hardcoding values in modules']
      },
      {
        stepNumber: 2,
        instruction: 'Create reusable VPC module',
        command: 'mkdir -p modules/vpc && cat > modules/vpc/main.tf <<EOF\nresource "aws_vpc" "main" {\n  cidr_block           = var.cidr_block\n  enable_dns_hostnames = true\n  enable_dns_support   = true\n  tags = merge(var.tags, { Name = var.name })\n}\n\nresource "aws_subnet" "public" {\n  count                   = length(var.public_subnet_cidrs)\n  vpc_id                  = aws_vpc.main.id\n  cidr_block              = var.public_subnet_cidrs[count.index]\n  availability_zone       = data.aws_availability_zones.available.names[count.index]\n  map_public_ip_on_launch = true\n  tags                    = merge(var.tags, { Name = "${var.name}-public-${count.index + 1}" })\n}\n\nresource "aws_internet_gateway" "main" {\n  vpc_id = aws_vpc.main.id\n  tags   = merge(var.tags, { Name = "${var.name}-igw" })\n}\n\ndata "aws_availability_zones" "available" {\n  state = "available"\n}\nEOF',
        explanation: 'VPC module encapsulates VPC, subnets, internet gateway. Takes inputs: cidr_block, subnet CIDRs, tags. Reusable across environments.',
        ...
        commonMistakes: ['Hardcoding CIDR blocks', 'Not using count for subnets']
      },
      {
        stepNumber: 3,
        instruction: 'Define module variables and outputs',
        command: 'cat > modules/vpc/variables.tf <<EOF\nvariable "name" {\n  description = "Name prefix for VPC resources"\n  type        = string\n}\n\nvariable "cidr_block" {\n  description = "CIDR block for VPC"\n  type        = string\n  validation {\n    condition     = can(cidrhost(var.cidr_block, 0))\n    error_message = "Must be valid IPv4 CIDR."\n  }\n}\n\nvariable "public_subnet_cidrs" {\n  description = "CIDR blocks for public subnets"\n  type        = list(string)\n}\n\nvariable "tags" {\n  description = "Tags to apply to resources"\n  type        = map(string)\n  default     = {}\n}\nEOF\ncat > modules/vpc/outputs.tf <<EOF\noutput "vpc_id" {\n  description = "VPC ID"\n  value       = aws_vpc.main.id\n}\n\noutput "public_subnet_ids" {\n  description = "Public subnet IDs"\n  value       = aws_subnet.public[*].id\n}\n\noutput "vpc_cidr" {\n  description = "VPC CIDR block"\n  value       = aws_vpc.main.cidr_block\n}\nEOF',
        explanation: 'Variables define module inputs with types, descriptions, validation. Outputs expose resource IDs for use in root module. Documentation via descriptions.',
      ...
      },
      {
        ...
        command: 'cat > main.tf <<EOF\nmodule "vpc_prod" {\n  source = "./modules/vpc"\n  \n  name               = "production"\n  cidr_block         = "10.0.0.0/16"\n  public_subnet_cidrs = ["10.0.1.0/24", "10.0.2.0/24", "10.0.3.0/24"]\n  \n  tags = {\n    Environment = "production"\n    ManagedBy   = "terraform"\n  }\n}\n\noutput "vpc_id" {\n  value = module.vpc_prod.vpc_id\n}\nEOF\nterraform init && terraform plan',
        explanation: 'Root module calls VPC module with source path, passes input variables, uses outputs. Multiple module instances for different environments.',
      ...
      },
      {
        ...
        explanation: 'Remote state stores terraform.tfstate in S3 with encryption. DynamoDB table provides state locking preventing concurrent modifications. Versioning enables rollback.',
      ...
      },
      {
        ...
        command: 'terraform init -backend-config="bucket=my-terraform-state" -backend-config="key=prod/terraform.tfstate" && terraform state list',
        explanation: 'terraform init configures backend, migrates local state to S3. State locking prevents race conditions. Team members share same state.',
        ...
        commonMistakes: ['Losing local state', 'No state backup before migration']
      },
      {
        stepNumber: 7,
        instruction: 'Use workspaces for multi-environment management',
        command: 'terraform workspace list && terraform workspace new staging && terraform workspace new production && terraform workspace select staging && terraform workspace show',
        explanation: 'Workspaces isolate state for different environments using same config. Default workspace + custom workspaces (staging, prod). Switch with terraform workspace select.',
        ...
        commonMistakes: ['Applying to wrong workspace', 'Not using workspace in resource names']
      },
      {
        stepNumber: 8,
        ...
        explanation: 'terraform.workspace gives current workspace name. Use to vary instance counts, sizes, names by environment. Conditional logic: production gets larger instances.',
        ...
        commonMistakes: ['Not tagging with workspace', 'Same config for all environments']
      },
      {
        stepNumber: 9,
        instruction: 'Implement infrastructure testing with Terratest',
        command: 'mkdir -p test && cat > test/vpc_test.go <<EOF\npackage test\n\nimport (\n  "testing"\n  "github.com/gruntwork-io/terratest/modules/terraform"\n  "github.com/stretchr/testify/assert"\n)\n\nfunc TestVPCModule(t *testing.T) {\n  terraformOptions := &terraform.Options{\n    TerraformDir: "../modules/vpc",\n    Vars: map[string]interface{}{\n      "name":                "test",\n      "cidr_block":          "10.0.0.0/16",\n      "public_subnet_cidrs": []string{"10.0.1.0/24"},\n    },\n  }\n  \n  defer terraform.Destroy(t, terraformOptions)\n  terraform.InitAndApply(t, terraformOptions)\n  \n  vpcID := terraform.Output(t, terraformOptions, "vpc_id")\n  assert.NotEmpty(t, vpcID)\n}\nEOF\ncd test && go mod init vpc-test && go mod tidy',
        explanation: 'Terratest tests Terraform modules in real cloud. Applies config, validates outputs, destroys resources. Catches issues before production.',
        ...
        commonMistakes: ['Not cleaning up test resources', 'Testing in production account']
      },
      {
        stepNumber: 10,
        instruction: 'Validate configuration with terraform validate and tflint',
        command: 'terraform validate && curl -s https://raw.githubusercontent.com/terraform-linters/tflint/master/install_linux.sh | bash && tflint --init && tflint',
        explanation: 'terraform validate checks syntax and internal consistency. tflint detects deprecated syntax, provider errors, best practice violations.',
        ...
        commonMistakes: ['Skipping validation in CI/CD', 'Ignoring tflint warnings']
      },
      {
        stepNumber: 11,
        instruction: 'Implement cost estimation with Infracost',
        command: 'curl -fsSL https://raw.githubusercontent.com/infracost/infracost/master/scripts/install.sh | sh && infracost auth login && infracost breakdown --path . && infracost diff --path .',
        explanation: 'Infracost estimates cloud costs from Terraform. Shows monthly cost breakdown before apply. Diff shows cost changes. Integrate in pull requests.',
        ...
        commonMistakes: ['Not reviewing costs before apply', 'Deploying expensive resources accidentally']
      },
      {
        stepNumber: 12,
        instruction: 'Implement automated testing in CI/CD',
        command: 'cat > .github/workflows/terraform.yml <<EOF\nname: Terraform\non: [pull_request]\njobs:\n  validate:\n    runs-on: ubuntu-latest\n    steps:\n    - uses: actions/checkout@v3\n    - uses: hashicorp/setup-terraform@v2\n    - run: terraform fmt -check\n    - run: terraform init\n    - run: terraform validate\n    - uses: terraform-linters/setup-tflint@v3\n    - run: tflint --init\n    - run: tflint\n    - uses: infracost/actions/setup@v2\n    - run: infracost breakdown --path . --format json --out-file /tmp/infracost.json\n    - uses: infracost/actions/comment@v1\n      with:\n        path: /tmp/infracost.json\nEOF',
        explanation: 'CI/CD validates Terraform on every PR: format check, syntax validation, linting, cost estimation. Fails PR if validation fails. Prevents bad code merging.',
        ...
        commonMistakes: ['No automated validation', 'Merging without cost review']
      }
    ]
  },
...
output "bucket_arn" {
  ...
  value       = aws_s3_bucket.this.__ARN__
        ...
        solution: 'S3 bucket module with versioning. Variables define inputs (bucket_name, tags) with types. Outputs expose bucket ID and ARN for use in root module. Reusable across projects.',
        explanation: 'Terraform modules encapsulate resources with typed inputs and outputs'
      },
      {
        exerciseNumber: 2,
...
resource "aws_dynamodb_table" "terraform_locks" {
  ...
  attribute {
    ...
    type = "__S__"
        ...
        solution: 'Remote S3 backend with encryption and versioning. DynamoDB table provides state locking preventing concurrent modifications. KMS encryption for security. Versioning enables state rollback.',
        explanation: 'Remote state with locking enables team collaboration and prevents conflicts'
      },
      {
        exerciseNumber: 3,
...
resource "aws_instance" "app" {
  ...
  tags = {
    ...
    Environment = local.environment
        ...
        solution: 'Workspaces manage environments with single config. Dev gets 1 t3.micro, staging 2 t3.small, production 5 t3.large. Lookup function selects config by workspace. Tags include environment name.',
        explanation: 'Workspaces enable environment-specific configurations from single codebase'
      },
      {
        exerciseNumber: 4,
...
resource "aws_instance" "app" {
  ...
  tags = {
    Environment = var.environment
        ...
        solution: 'Variable validation ensures CIDR is valid IPv4 and environment is allowed value. Lifecycle rules: prevent_destroy protects critical resources, ignore_changes prevents drift, create_before_destroy enables zero-downtime replacement.',
        explanation: 'Validation and lifecycle rules enforce safety and prevent errors'
      },
      {
        exerciseNumber: 5,
...
jobs:
  validate:
    ...
    - name: Cost estimate
      run: infracost __BREAKDOWN__ --path . --format json`,
        ...
        solution: 'GitHub Actions pipeline validates Terraform on PRs: format check, init, validate, plan, cost estimate. Prevents merging invalid code. Costs visible before approval.',
        explanation: 'CI/CD automation ensures code quality and cost visibility'
      }
    ],
    hints: [
  ...
  runGuided: {
    objective: 'Build production Terraform infrastructure with reusable modules, remote state, multi-environment management, and automated testing',
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck: 'Create modules/vpc, modules/compute. Add variables.tf with validation. Define outputs.tf. Document in README. Test with Terratest or manual apply/destroy.'
      },
      {
        ...
        hintIfStuck: 'Create S3 bucket with versioning and encryption. Create DynamoDB table with LockID key. Configure backend in backend.tf. Run terraform init to migrate state.'
      },
      {
        ...
        hintIfStuck: 'Use terraform workspace for environments. Reference terraform.workspace in locals. Create map of environment configs. Apply to staging workspace first, then production.'
      }
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective: 'Build production-grade infrastructure-as-code platform with comprehensive module library, remote state, multi-environment management, automated testing, and complete CI/CD integration',
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 25,
        passingThreshold: 'Modules properly structured with variables/outputs. Input validation configured. Reusable across projects. Documentation complete. Versioned with Git tags. Examples provided. Tested independently.'
      },
      {
        criterion: 'State Management',
        weight: 25,
        passingThreshold: 'Remote S3 backend with encryption and versioning. DynamoDB locking preventing conflicts. State organized by environment. Locking working correctly. Team can collaborate safely. Backup strategy documented.'
      },
      {
        criterion: 'Environment Management',
        weight: 25,
        passingThreshold: 'Workspaces or separate states per environment. Environment-specific configs (sizes, counts). Promotion workflow documented. No environment interference. Can deploy staging before production. Tags identify environment.'
      },
      {
        criterion: 'Automation & Testing',
        weight: 25,
        passingThreshold: 'CI/CD validating on PRs (format, validate, plan). Cost estimation automated. Terratest covering critical modules. No manual validation needed. Infracost integrated. Security scanning active.'
      }
    ]
  },
