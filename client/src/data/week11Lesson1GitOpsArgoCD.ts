...
export const week11Lesson1GitOpsArgoCD: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description:
      "Implement GitOps workflows with ArgoCD for declarative, Git-based continuous deployment to Kubernetes.",
    learningObjectives: [
  ...
  crawl: {
    introduction:
      "Learn GitOps and ArgoCD step-by-step. You will deploy ArgoCD, configure applications, and implement declarative deployment workflows.",
    expectedOutcome:
      "Complete understanding of GitOps principles, ArgoCD installation and configuration, application deployment, sync strategies, automated rollback, and multi-cluster management",
    steps: [
      {
        stepNumber: 1,
        ...
        explanation:
          "GitOps uses Git as single source of truth. Operators like ArgoCD watch Git repo, automatically apply changes to cluster. Rollback = git revert.",
        ...
        commonMistakes: [
          "Manual kubectl changes breaking GitOps",
      ...
      },
      {
        ...
        explanation:
          "ArgoCD installs: API server, repo server, application controller, UI server, Redis cache. Monitors Git repos, syncs manifests to cluster.",
      ...
      },
      {
        ...
        explanation:
          "ArgoCD UI provides visual app management. Default username: admin. Password in secret. LoadBalancer exposes UI externally.",
      ...
      },
      {
        ...
        command:
          'mkdir -p gitops-demo/apps && cd gitops-demo && git init && cat > apps/deployment.yaml <<EOF\napiVersion: apps/v1\nkind: Deployment\nmetadata:\n  name: demo-app\nspec:\n  replicas: 3\n  selector:\n    matchLabels:\n      app: demo\n  template:\n    metadata:\n      labels:\n        app: demo\n    spec:\n      containers:\n      - name: app\n        image: nginx:1.21\n        ports:\n        - containerPort: 80\nEOF\ngit add . && git commit -m "Initial app" && git push',
        explanation:
          "GitOps repo contains Kubernetes manifests. ArgoCD watches this repo. Changes to Git automatically deploy to cluster.",
      ...
      },
      {
        ...
        command:
          "argocd app create demo-app --repo https://github.com/myorg/gitops-demo.git --path apps --dest-server https://kubernetes.default.svc --dest-namespace default",
        explanation:
          "ArgoCD Application CRD defines: Git repo URL, path to manifests, target cluster/namespace. ArgoCD syncs Git state to cluster.",
      ...
      },
      {
        ...
        command:
          "argocd app sync demo-app && argocd app wait demo-app --health",
        explanation:
          "Manual sync applies Git manifests to cluster. Wait command blocks until app healthy. ArgoCD compares desired (Git) vs actual (cluster) state.",
      ...
      },
      {
        ...
        explanation:
          "Automated sync: ArgoCD polls Git every 3 minutes, auto-applies changes. Auto-prune deletes resources removed from Git. Self-heal reverts manual kubectl changes.",
        ...
        commonMistakes: [
          "Auto-prune deleting unintended resources",
          "Self-heal fighting manual changes",
        ],
      },
      {
        stepNumber: 8,
        ...
        command:
          'cd gitops-demo && sed -i "s/replicas: 3/replicas: 5/" apps/deployment.yaml && git commit -am "Scale to 5 replicas" && git push',
        explanation:
          "GitOps workflow: change Git → ArgoCD detects change (within 3 min) → auto-syncs to cluster. No kubectl needed. Audit trail in Git history.",
      ...
      },
      {
        ...
        explanation:
          "Sync waves control deployment order. Wave 0 (namespace) deploys before wave 1 (deployment). Use for dependencies: CRDs before CRs, namespaces before resources.",
      ...
      },
      {
        ...
        command:
          "cd gitops-demo && git log --oneline && git revert HEAD && git push",
        explanation:
          "GitOps rollback: revert Git commit, push, ArgoCD syncs old state. Full audit trail. No manual kubectl rollback needed.",
        ...
        commonMistakes: [
          "Using kubectl rollout undo instead of Git",
          "Not pushing revert",
        ],
      },
      {
        ...
        explanation:
          "SyncOptions: CreateNamespace auto-creates target namespace. Retry with exponential backoff handles transient failures. Limits prevent infinite retries.",
      ...
      },
      {
        ...
        command:
          "kubectl config get-contexts && argocd cluster add staging-cluster --name staging && argocd app create staging-app --repo https://github.com/myorg/gitops-demo.git --path apps/staging --dest-server https://staging-cluster-api --dest-namespace staging",
        explanation:
          "ArgoCD manages multiple clusters from single control plane. Add cluster with credentials, deploy apps to any cluster. Single Git repo, multiple environments.",
  ...
  syncPolicy:
    automated:
      ...
      selfHeal: __TRUE__`,
        ...
        solution:
          "ArgoCD Application syncs Git repo (apps/ directory) to Kubernetes cluster. Automated sync with prune (delete removed resources) and self-heal (revert manual changes). targetRevision: HEAD tracks latest commit.",
        explanation:
          "ArgoCD Applications define declarative deployment from Git to cluster",
      },
      {
        exerciseNumber: 2,
  ...
  template:
    ...
    spec:
      ...
      - name: app
        ...
        solution:
          "Sync waves ensure ordered deployment: wave 0 (namespace), wave 1 (ConfigMap), wave 2 (Deployment). Prevents failures from missing dependencies. Lower numbers deploy first.",
        explanation:
          "Sync waves control resource creation order for dependencies",
      },
      {
        exerciseNumber: 3,
  ...
  syncPolicy:
    ...
    retry:
      ...
      backoff:
        ...
        solution:
          "Automated sync with prune and self-heal. CreateNamespace auto-creates target namespace. Retry with exponential backoff: 5s, 10s, 20s, 40s, 80s, max 3m. Up to 5 retries for transient failures.",
        explanation:
...
spec:
  ...
  destination:
    namespace: staging`,
        ...
        solution:
          "Kustomize overlays customize base manifests per environment. Staging overlay: prefix resources, 2 replicas, specific image tag, staging namespace. Single base, multiple environments.",
        explanation:
          "Kustomize enables environment-specific configurations from single base",
      },
      {
        exerciseNumber: 5,
...
spec:
  ...
  template:
    ...
    spec:
      ...
      syncPolicy:
        ...
        solution:
          "ApplicationSet generates multiple Applications from template. List generator creates app per cluster: myapp-production, myapp-staging. Each deploys to different cluster from environment-specific path.",
        explanation:
          "ApplicationSets automate multi-cluster deployments from templates",
      },
    ],
    hints: [
  ...
  runGuided: {
    objective:
      "Build production GitOps platform with ArgoCD managing multi-environment, multi-cluster deployments with automated sync and rollback",
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck:
          "Install ArgoCD with HA manifest. Create Application CRDs for each environment. Enable syncPolicy.automated with prune and selfHeal. Test by changing Git and watching auto-sync.",
      },
      {
        ...
        hintIfStuck:
          "Structure repo: base/ with common manifests, overlays/dev, overlays/staging, overlays/prod. Each overlay has kustomization.yaml customizing base. ArgoCD Applications point to overlay paths.",
      },
      {
        ...
        hintIfStuck:
          "Add clusters with argocd cluster add. Create ApplicationSet with list generator for each cluster. Use templates to customize per cluster. Verify apps deployed to all clusters.",
      },
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective:
      "Build production-grade GitOps platform with comprehensive ArgoCD setup, multi-environment deployment, multi-cluster management, automated sync, RBAC, and complete observability",
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 30,
        passingThreshold:
          "ArgoCD HA deployed. All apps managed declaratively via Git. Automated sync with self-heal preventing drift. Manual kubectl changes reverted automatically. Git history as complete audit trail. Rollback via git revert tested.",
      },
      {
        criterion: "Multi-Environment Management",
        weight: 25,
        passingThreshold:
          "Kustomize base + overlays for dev/staging/prod. Environment-specific configs (replicas, images, resources). Promotion workflow documented. No manifest duplication. Sync waves ensuring ordered deployment. Each environment isolated.",
      },
      {
        criterion: "Multi-Cluster Orchestration",
        weight: 25,
        passingThreshold:
          "ApplicationSets deploying to multiple clusters. Single ArgoCD control plane. Cluster-specific configurations. Independent sync per cluster. Credentials secured. Multi-region deployment capability.",
      },
      {
        criterion: "Operations & Security",
        weight: 20,
        passingThreshold:
          "RBAC restricting access by team. Projects isolating resources. SSO integrated. TLS enabled. Metrics exported to Prometheus. Alerts on sync failures. Documentation complete. Teams trained on GitOps workflow.",
      },
    ],
  },
