...
export const DOCKER_BASICS_TCS: TCSTask = {
  ...
  conditions: {
    timeLimit: 45,
    environment: "Linux system with Docker Engine 20.10+ installed and Docker daemon running",
    resources: [
  ...
  standards: [
    {
      id: "std-1",
      description: "Create valid Dockerfile with FROM, COPY, RUN, EXPOSE, and CMD instructions",
      required: true,
      met: false
    },
    {
    ...
    {
      id: "std-3",
      description: "Run container from built image and verify it starts without errors",
      required: true,
      met: false
    },
    {
    ...
    {
      id: "std-5",
      description: "Stop and remove container cleanly, verify removal with docker ps -a",
      required: true,
      met: false
    },
    {
      id: "std-6",
      description: "List images and demonstrate image size awareness (< 500MB for simple apps)",
      required: false,
      met: false
    }
  ]
...
export const K8S_DEPLOYMENT_TCS: TCSTask = {
  ...
  conditions: {
    timeLimit: 60,
    environment: "Kubernetes cluster (minikube, kind, or cloud provider) with kubectl configured",
    resources: [
  ...
  standards: [
    {
      id: "std-1",
      description: "Create Deployment manifest with minimum 3 replicas and valid selector",
      required: true,
      met: false
    },
    {
      id: "std-2",
      description: "Set resource requests (CPU: 100m, Memory: 128Mi) and limits on containers",
      required: true,
      met: false
    },
    {
      id: "std-3",
      description: "Create Service of type LoadBalancer or NodePort to expose application",
      required: true,
      met: false
    },
    {
      id: "std-4",
      description: "Create ConfigMap for application configuration and mount in deployment",
      required: true,
      met: false
    },
    {
      id: "std-5",
      description: "Verify all 3 replicas are running and distributed across nodes",
      required: true,
      met: false
    },
    {
      id: "std-6",
      description: "Demonstrate rolling update: change image version, verify zero-downtime deployment",
      required: true,
      met: false
    },
    {
      id: "std-7",
      description: "Application accessible via Service, responds to HTTP requests",
      required: true,
      met: false
    },
    {
      id: "std-8",
      description: "Clean up all resources (deployment, service, configmap) successfully",
      required: true,
      met: false
    }
  ]
...
export const CICD_PIPELINE_TCS: TCSTask = {
  ...
  conditions: {
    timeLimit: 90,
    environment: "Jenkins server, Docker registry, Kubernetes cluster, Git repository",
    resources: [
  ...
  standards: [
    {
      id: "std-1",
      description: "Jenkinsfile defines multi-stage pipeline: Build, Test, Push, Deploy",
      required: true,
      met: false
    },
    {
      id: "std-2",
      description: "Build stage successfully compiles code and creates Docker image",
      required: true,
      met: false
    },
    {
      id: "std-3",
      description: "Test stage runs automated tests and fails pipeline if tests fail",
      required: true,
      met: false
    },
    {
      id: "std-4",
      description: "Push stage uploads image to registry with proper tagging (version + 'latest')",
      required: true,
      met: false
    },
    {
      id: "std-5",
      description: "Deploy stage updates Kubernetes deployment without manual intervention",
      required: true,
      met: false
    },
    {
    ...
    {
      id: "std-8",
      description: "Deployment verification: application is accessible and returns HTTP 200",
      required: true,
      met: false
    },
    {
      id: "std-9",
      description: "Rollback capability demonstrated: can revert to previous version",
      required: true,
      met: false
    },
    {
      id: "std-10",
      description: "Pipeline execution time under 10 minutes from commit to deployment",
      required: false,
      met: false
    },
    {
      id: "std-11",
      description: "All pipeline stages include proper error handling and notifications",
      required: true,
      met: false
    }
  ]
...
export const INCIDENT_RESPONSE_TCS: TCSTask = {
  ...
  standards: [
    {
      id: "std-1",
      description: "Identify failing pod within 2 minutes using kubectl commands",
      required: true,
      met: false
    },
    {
    ...
    {
      id: "std-3",
      description: "Post initial status update in incident channel within 3 minutes",
      required: true,
      met: false
    },
    {
...
export const TCS_GUIDELINES = {
  task: {
    ...
    shouldNotBe: "Vague goals, multiple unrelated tasks, or learning objectives"
  },
