...
export const week5Lesson2GitHubActions: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description: 'Master GitHub Actions to automate build, test, and deployment workflows directly from your repository.',
    learningObjectives: [
  ...
  crawl: {
    introduction: 'Learn GitHub Actions step-by-step: workflow syntax, triggers, jobs, running tests, building Docker images, and deployment.',
    steps: [
      {
        stepNumber: 1,
        ...
        explanation: 'GitHub Actions workflows live in .github/workflows/ directory. YAML file defines workflow. "on: [push]" triggers on every push. "jobs" contains work to do. "runs-on: ubuntu-latest" specifies runner OS. "run" executes shell command.',
        expectedOutput: 'Workflow file created, committed, pushed. GitHub Actions tab shows workflow running. "Hello, GitHub Actions!" in logs.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'cat > .github/workflows/triggers.yml << \'EOF\'\nname: Understanding Triggers\n\non:\n  push:\n    branches: [main, develop]  # Only these branches\n  pull_request:\n    branches: [main]  # PRs to main\n  schedule:\n    - cron: \'0 0 * * *\'  # Daily at midnight\n  workflow_dispatch:  # Manual trigger from UI\n\njobs:\n  show-trigger:\n    runs-on: ubuntu-latest\n    steps:\n      - run: echo "Triggered by: ${{ github.event_name }}"\nEOF',
        explanation: 'Multiple triggers: push (on code push), pull_request (on PR), schedule (cron syntax), workflow_dispatch (manual button). Can filter by branches. ${{ github.event_name }} shows trigger type.',
        expectedOutput: 'Workflow can be triggered multiple ways: push to main/develop, PR to main, daily schedule, manual trigger button.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'cat > .github/workflows/checkout.yml << \'EOF\'\nname: Checkout Code\n\non: [push]\n\njobs:\n  build:\n    runs-on: ubuntu-latest\n    steps:\n      - name: Checkout repository\n        uses: actions/checkout@v4\n      \n      - name: List files\n        run: ls -la\n      \n      - name: Show current directory\n        run: pwd\nEOF',
        explanation: 'actions/checkout@v4 clones your repository into runner. Without it, runner is empty. All subsequent steps have access to your code. Always first step in most workflows.',
        expectedOutput: 'Workflow checks out code, lists files showing your repository contents.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'cat > .github/workflows/test.yml << \'EOF\'\nname: Run Tests\n\non: [push, pull_request]\n\njobs:\n  test:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      \n      - name: Set up Node.js\n        uses: actions/setup-node@v4\n        with:\n          node-version: \'20\'\n      \n      - name: Install dependencies\n        run: npm ci\n      \n      - name: Run tests\n        run: npm test\nEOF',
        explanation: 'actions/setup-node installs Node.js runtime. npm ci installs dependencies (faster than npm install). npm test runs test suite. If tests fail, workflow fails (red X).',
        expectedOutput: 'Tests run automatically on every push and PR. Workflow shows test results. Failures block merging.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'cat > .github/workflows/python.yml << \'EOF\'\nname: Python CI\n\non: [push]\n\njobs:\n  test:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      \n      - name: Set up Python\n        uses: actions/setup-python@v5\n        with:\n          python-version: \'3.11\'\n      \n      - name: Install dependencies\n        run: |\n          pip install -r requirements.txt\n          pip install pytest pytest-cov\n      \n      - name: Run tests with coverage\n        run: pytest --cov=. --cov-report=term\nEOF',
        explanation: 'actions/setup-python installs Python. Install dependencies from requirements.txt. pytest runs tests. --cov generates coverage report. Multi-line commands use | syntax.',
        expectedOutput: 'Python tests run, coverage report shows percentage of code tested.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'cat > .github/workflows/docker.yml << \'EOF\'\nname: Build Docker Image\n\non: [push]\n\njobs:\n  build:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      \n      - name: Build Docker image\n        run: docker build -t myapp:${{ github.sha }} .\n      \n      - name: Test image\n        run: docker run myapp:${{ github.sha }} echo "Image works!"\nEOF',
        explanation: 'docker build creates image. Tag with git commit SHA (${{ github.sha }}) for traceability. Test image by running simple command. Ensures image builds successfully.',
        expectedOutput: 'Docker image builds successfully, tagged with commit SHA, runs basic test.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'cat > .github/workflows/docker-push.yml << \'EOF\'\nname: Push to Docker Hub\n\non:\n  push:\n    branches: [main]\n\njobs:\n  build-and-push:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      \n      - name: Log in to Docker Hub\n        uses: docker/login-action@v3\n        with:\n          username: ${{ secrets.DOCKER_USERNAME }}\n          password: ${{ secrets.DOCKER_PASSWORD }}\n      \n      - name: Build and push\n        uses: docker/build-push-action@v5\n        with:\n          push: true\n          tags: yourusername/myapp:latest\nEOF',
        explanation: 'docker/login-action authenticates to Docker Hub using secrets. docker/build-push-action builds and pushes in one step. Secrets protect credentials (never hardcode).',
        expectedOutput: 'Image pushed to Docker Hub, tagged as latest. Visible on Docker Hub repository.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: '# In GitHub repo: Settings → Secrets and variables → Actions → New repository secret\n# Add: DATABASE_URL, API_KEY, etc.\n\ncat > .github/workflows/secrets.yml << \'EOF\'\nname: Use Secrets\n\non: [push]\n\njobs:\n  deploy:\n    runs-on: ubuntu-latest\n    env:\n      NODE_ENV: production\n    steps:\n      - name: Use secret\n        env:\n          DATABASE_URL: ${{ secrets.DATABASE_URL }}\n          API_KEY: ${{ secrets.API_KEY }}\n        run: |\n          echo "Connecting to database..."\n          # Secrets are masked in logs\nEOF',
        explanation: 'GitHub Secrets store sensitive data. Access via ${{ secrets.NAME }}. Never echoed in logs (automatically masked). Env variables available to all steps.',
        expectedOutput: 'Secrets used in workflow, values masked in logs, environment variables accessible.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'cat > .github/workflows/matrix.yml << \'EOF\'\nname: Matrix Build\n\non: [push]\n\njobs:\n  test:\n    runs-on: ${{ matrix.os }}\n    strategy:\n      matrix:\n        os: [ubuntu-latest, windows-latest, macos-latest]\n        node: [18, 20, 22]\n    steps:\n      - uses: actions/checkout@v4\n      - uses: actions/setup-node@v4\n        with:\n          node-version: ${{ matrix.node }}\n      - run: npm ci\n      - run: npm test\nEOF',
        explanation: 'Matrix creates multiple jobs (3 OS × 3 Node versions = 9 jobs). Tests compatibility across platforms and versions. Runs in parallel for speed.',
        expectedOutput: '9 jobs run in parallel, testing all combinations. Ensures app works on all platforms.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'cat > .github/workflows/cache.yml << \'EOF\'\nname: Cache Dependencies\n\non: [push]\n\njobs:\n  build:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      - uses: actions/setup-node@v4\n        with:\n          node-version: \'20\'\n          cache: \'npm\'  # Automatic caching\n      \n      - run: npm ci\n      - run: npm test\nEOF',
        explanation: 'cache: npm enables automatic dependency caching. Subsequent runs skip download if package-lock.json unchanged. Speeds up builds significantly (minutes to seconds).',
        expectedOutput: 'First run: downloads dependencies (slow). Subsequent runs: uses cache (fast). Logs show "Cache restored".',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'cat > .github/workflows/conditional.yml << \'EOF\'\nname: Conditional Steps\n\non: [push, pull_request]\n\njobs:\n  build:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      \n      - name: Run on main only\n        if: github.ref == \'refs/heads/main\'\n        run: echo "This is main branch"\n      \n      - name: Run on PRs only\n        if: github.event_name == \'pull_request\'\n        run: echo "This is a PR"\n      \n      - name: Deploy to production\n        if: github.ref == \'refs/heads/main\' && success()\n        run: echo "Deploying..."\nEOF',
        explanation: 'if conditions control step execution. Check branch (github.ref), event (github.event_name), previous step status (success(), failure()). Combine with &&/||.',
        expectedOutput: 'Steps execute conditionally based on branch, event, or previous step status.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'needs: build ensures deploy runs after build succeeds. environment: production creates deployment with approval gate (configure in GitHub settings). URL shown in deployment.',
        expectedOutput: 'Build runs, then waits for manual approval before deploying. Deployment tracked in Environments tab.',
        validationCriteria: [
    ...
    expectedOutcome: 'You can create GitHub Actions workflows with triggers, jobs, and steps. You understand how to checkout code, run tests, build Docker images, use secrets, implement caching, and deploy with approval gates. You can read workflow logs and troubleshoot failures.'
  },
  ...
  walk: {
    ...
    exercises: [
      {
        exerciseNumber: 1,
        scenario: 'Create Node.js CI workflow: install dependencies, run tests, build project.',
        template: 'name: Node.js CI\n\non: [__TRIGGER__, pull_request]\n\njobs:\n  build:\n    runs-on: __OS__\n    steps:\n      - uses: __CHECKOUT__\n      \n      - name: Setup Node\n        uses: actions/setup-node@v4\n        with:\n          node-version: \'__VERSION__\'\n          cache: \'__CACHE__\'\n      \n      - run: __INSTALL__\n      - run: __TEST__\n      - run: __BUILD__',
        blanks: [
          {
            id: 'TRIGGER',
            label: 'TRIGGER',
        ...
        explanation: 'Basic Node.js CI: checkout code, setup Node with caching, install dependencies, run tests, build project.'
      },
      {
        exerciseNumber: 2,
        scenario: 'Build and push Docker image to Docker Hub on main branch pushes.',
        template: 'name: Docker Build and Push\n\non:\n  push:\n    branches: [__BRANCH__]\n\njobs:\n  docker:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      \n      - name: Login to Docker Hub\n        uses: __LOGIN_ACTION__\n        with:\n          username: ${{ secrets.__USERNAME_SECRET__ }}\n          password: ${{ secrets.__PASSWORD_SECRET__ }}\n      \n      - name: Build and push\n        uses: __BUILD_PUSH_ACTION__\n        with:\n          push: __PUSH__\n          tags: yourusername/app:__TAG__',
        blanks: [
          {
            id: 'BRANCH',
            label: 'BRANCH',
      ...
      },
      {
        ...
        solution: 'name: Python Matrix Test\n\non: [push]\n\njobs:\n  test:\n    runs-on: ubuntu-latest\n    strategy:\n      matrix:\n        python-version: [3.8, 3.9, 3.11]\n    steps:\n      - uses: actions/checkout@v4\n      \n      - name: Setup Python\n        uses: actions/setup-python@v5\n        with:\n          python-version: ${{ matrix.python-version }}\n      \n      - run: pip install -r requirements.txt\n      - run: pytest',
        explanation: 'Matrix strategy tests code on multiple Python versions in parallel.'
      },
      {
        exerciseNumber: 4,
        scenario: 'Deploy to production with manual approval, only on main branch, only if tests pass.',
        ...
        explanation: 'Deploy job waits for test to pass, requires manual approval (environment), only runs on main branch.'
      },
      {
        exerciseNumber: 5,
        scenario: 'Create workflow with caching, linting, testing, and build artifact upload.',
        template: 'name: Complete CI\n\non: [push]\n\njobs:\n  build:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      \n      - uses: actions/setup-node@v4\n        with:\n          node-version: \'20\'\n          cache: __CACHE__\n      \n      - run: npm ci\n      \n      - name: Lint\n        run: npm run __LINT__\n      \n      - name: Test\n        run: npm __TEST__\n      \n      - name: Build\n        run: npm run build\n      \n      - name: Upload artifact\n        uses: __UPLOAD_ACTION__\n        with:\n          name: __ARTIFACT_NAME__\n          path: __BUILD_PATH__',
        blanks: [
          {
            id: 'CACHE',
            label: 'CACHE',
        ...
        solution: 'name: Complete CI\n\non: [push]\n\njobs:\n  build:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      \n      - uses: actions/setup-node@v4\n        with:\n          node-version: \'20\'\n          cache: npm\n      \n      - run: npm ci\n      \n      - name: Lint\n        run: npm run lint\n      \n      - name: Test\n        run: npm test\n      \n      - name: Build\n        run: npm run build\n      \n      - name: Upload artifact\n        uses: actions/upload-artifact@v4\n        with:\n          name: build\n          path: dist/',
        explanation: 'Complete CI pipeline: cache dependencies, lint code, run tests, build project, upload build artifacts.'
      }
    ],
    hints: [
  ...
  runGuided: {
    objective: 'Create complete CI/CD pipeline using GitHub Actions with testing, Docker build, and deployment',
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck: 'Start simple: on: [push, pull_request, workflow_dispatch]. Add checkout step and echo command to verify it works.'
      },
      {
        ...
        hintIfStuck: 'Add test job: setup language runtime → install deps → run tests. Add build job: docker build → docker push (use docker/login-action and docker/build-push-action).'
      },
      {
        ...
        hintIfStuck: 'Create deploy job with needs: [test, build]. Add environment: production to require approval. Configure environment in Settings → Environments → Add protection rules. Status badge: Actions tab → workflow → ... → Create status badge.'
      }
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective: 'Build production-ready GitHub Actions CI/CD pipeline with comprehensive testing, security scanning, multi-stage deployment, monitoring, and complete documentation',
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 30,
        passingThreshold: 'All stages present (lint, test, security, build, deploy). Proper job dependencies. Fast feedback (<10 min). Workflow handles failures gracefully. Status clearly visible.'
      },
      {
        criterion: 'Quality and Security',
        weight: 25,
        passingThreshold: 'Comprehensive testing with coverage >80%. Security scanning integrated (dependencies, secrets, containers). Code quality gates (linting, formatting). Only secure, quality code reaches production.'
      },
      {
        criterion: 'Deployment Automation',
        weight: 25,
        passingThreshold: 'Multi-environment deployment (dev, staging, prod). Proper approval gates. Secrets managed securely. Semantic versioning. Deployment notifications. Rollback procedure documented and tested.'
      },
      {
        criterion: 'Operational Excellence',
        weight: 20,
        passingThreshold: 'Caching optimizes build time. Status badges visible. Comprehensive documentation. Workflow is maintainable and understandable. Team can operate independently. Monitoring shows deployment health.'
      }
    ]
  },
