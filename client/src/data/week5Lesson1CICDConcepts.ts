...
export const week5Lesson1CICDConcepts: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description: 'Understand CI/CD principles, benefits, and how they transform software delivery.',
    learningObjectives: [
  ...
  crawl: {
    introduction: 'Learn CI/CD step-by-step: problems it solves, core concepts, pipeline stages, and business value.',
    steps: [
      {
        stepNumber: 1,
        instruction: 'The problem: Manual integration hell',
        command: 'Traditional workflow:\n- Developers work in isolation for weeks\n- Integration happens right before release\n- Merge conflicts everywhere\n- "Works on my machine" but fails on server\n- Testing done manually, takes days\n- Release process takes hours, high failure rate',
        explanation: 'Before CI/CD: Long development cycles, infrequent integration, manual testing, risky deployments. Integration happens late, conflicts are massive. Testing is slow and incomplete. Releases are stressful events.',
        expectedOutput: 'Understanding: Manual processes lead to integration problems, slow feedback, and risky releases.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'CI = integrate code constantly (many times per day). Every commit triggers automated build and tests. Fast feedback loop. Problems caught early when easy to fix. No integration hell.',
        expectedOutput: 'Understanding: CI provides rapid feedback on code quality through automated builds and tests.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'CI finds bugs immediately while context is fresh. Small changes are easier to understand and fix. Continuous integration prevents conflict accumulation. Automated tests give confidence. Team can release anytime.',
        expectedOutput: 'Understanding: CI reduces risk and accelerates development through automation and fast feedback.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Continuous Delivery = code is always in deployable state. Every change that passes tests can go to production. Deployment is automated but gated by manual approval. Reduces deployment risk.',
        expectedOutput: 'Understanding: CD automates deployment pipeline but retains manual control over production releases.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Continuous Deployment = every commit that passes tests goes to production automatically. No manual gate. Ultimate speed. Requires excellent test coverage, monitoring, and rollback capability. Not for everyone.',
        expectedOutput: 'Understanding: Continuous Deployment removes manual approval for maximum speed but requires maturity.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Typical pipeline:\n1. Source: Code commit triggers pipeline\n2. Build: Compile code, build Docker image\n3. Test: Unit tests, integration tests, linting\n4. Security: Vulnerability scanning, dependency checks\n5. Stage: Deploy to staging environment\n6. Test (stage): Smoke tests, performance tests\n7. Production: Deploy to production (manual or automatic)\n8. Monitor: Track metrics, logs, errors',
        explanation: 'Pipeline = automated workflow from code to production. Each stage validates code. Failures stop pipeline. Artifacts move through stages. Each stage increases confidence. Monitoring provides feedback loop.',
        expectedOutput: 'Understanding: CI/CD pipeline automates validation stages from commit to production.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Test pyramid:\n- Unit tests (fast, many): Test individual functions\n- Integration tests (medium, some): Test components together\n- End-to-end tests (slow, few): Test full user workflows\n\nAll run automatically in pipeline. Fast feedback. High confidence.',
        explanation: 'CI/CD depends on automated tests. Unit tests run in seconds. Integration tests validate interactions. E2E tests verify user experience. More unit tests than E2E (speed vs coverage). All must pass for deployment.',
        expectedOutput: 'Understanding: Automated tests at multiple levels provide fast, comprehensive validation.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Build once principle: Same artifact through all environments. Guarantees staging and production run identical code. Version tagging enables rollback. Artifact repo provides audit trail.',
        expectedOutput: 'Understanding: Build artifacts once, deploy everywhere for consistency.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'IaC enables version-controlled infrastructure. CI/CD pipeline validates infrastructure changes. Same rigor for servers as for code. Prevents configuration drift. Enables reproducible environments.',
        expectedOutput: 'Understanding: Infrastructure changes flow through CI/CD pipeline like application code.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Blue-Green strategy:\n1. Blue (current production) serving traffic\n2. Deploy new version to Green (idle environment)\n3. Test Green thoroughly\n4. Switch router to Green (instant cutover)\n5. Blue becomes idle (instant rollback available)\n\nNo downtime. Instant rollback.',
        explanation: 'Two identical production environments. Deploy to idle one. Switch traffic instantly. Old version still running for instant rollback. Zero downtime. Low risk. Requires double infrastructure.',
        expectedOutput: 'Understanding: Blue-Green deployments enable zero-downtime releases with instant rollback.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Canary strategy:\n1. Deploy new version to small subset (5% of servers)\n2. Monitor metrics (errors, latency, business KPIs)\n3. If healthy, gradually increase (10%, 25%, 50%, 100%)\n4. If issues detected, rollback immediately\n\nReduces blast radius of bad deployments.',
        explanation: 'Canary = test new version on small percentage of users first. Monitor carefully. Gradual rollout limits impact of bugs. Can rollback affecting only small percentage. Requires robust monitoring.',
        expectedOutput: 'Understanding: Canary deployments reduce risk by gradually rolling out changes.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Key metrics (DORA metrics):\n- Deployment Frequency: How often you deploy (elite: multiple per day)\n- Lead Time for Changes: Commit to production (elite: <1 hour)\n- Mean Time to Recovery (MTTR): How fast you fix issues (elite: <1 hour)\n- Change Failure Rate: % of deployments causing failures (elite: 0-15%)\n\nTrack to measure CI/CD effectiveness',
        explanation: 'DORA metrics measure software delivery performance. Elite teams deploy frequently, with short lead times, quick recovery, and low failure rates. CI/CD enables these metrics. Track and improve continuously.',
        expectedOutput: 'Understanding: DORA metrics quantify CI/CD effectiveness and delivery performance.',
        validationCriteria: [
    ...
    expectedOutcome: 'You understand CI/CD principles, benefits, pipeline stages, testing strategies, deployment approaches (blue-green, canary), and DORA metrics. You can explain when CI/CD adds value and what infrastructure it requires.'
  },
  ...
  walk: {
    ...
    exercises: [
      ...
      },
      {
        ...
        explanation: 'Delivery has manual gate, Deployment is fully automated. Deployment requires higher maturity but enables maximum speed.'
      },
      {
        exerciseNumber: 3,
      ...
      },
      {
        ...
        solution: 'E-commerce site (no downtime allowed): Blue-Green deployment\nSocial media app (gradual rollout): Canary deployment\nInternal tool (downtime acceptable): Rolling or simple deployment\nDatabase schema change: Backward-compatible migrations first',
        explanation: 'Choose deployment strategy based on downtime tolerance, risk appetite, and infrastructure constraints.'
      },
      {
        exerciseNumber: 5,
        ...
        explanation: 'DORA metrics measure software delivery performance. CI/CD dramatically improves all four metrics.'
      }
    ],
    hints: [
  ...
  runGuided: {
    objective: 'Design complete CI/CD pipeline for a real application including all stages, testing, and deployment strategy',
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck: 'Start simple: Source (git) → Build (Docker) → Test (pytest/jest) → Deploy to Staging → Manual approval → Deploy to Prod → Monitor (logs/metrics)'
      },
      {
        ...
        hintIfStuck: 'GitHub Actions example: on push → checkout code → build Docker image → run tests → push to registry → deploy to staging. Use GitHub secrets for credentials.'
      },
      {
        ...
        hintIfStuck: 'Test by making commit. Watch pipeline execute. Document: what each stage does, how to view logs, how to manually trigger, how to rollback, troubleshooting common errors.'
      }
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective: 'Build production-ready CI/CD pipeline with comprehensive testing, security scanning, multi-environment deployment, and monitoring',
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 30,
        passingThreshold: 'All essential stages present: build, test, security, deploy, monitor. Stages execute automatically. Failures stop pipeline. Production-ready configuration.'
      },
      {
        criterion: 'Quality Gates',
        weight: 25,
        passingThreshold: 'Comprehensive automated testing. Security scanning integrated. Code quality checks (linting, coverage). Only quality code reaches production. Fast feedback (<10 minutes).'
      },
      {
        criterion: 'Deployment Automation',
        weight: 25,
        passingThreshold: 'Fully automated deployment. Health checks verify deployment success. Rollback procedure defined and tested. Zero-downtime strategy (blue-green or canary). Secrets managed securely.'
      },
      {
        criterion: 'Operational Excellence',
        weight: 20,
        passingThreshold: 'Monitoring and alerting configured. Documentation comprehensive. Runbook helps troubleshoot issues. DORA metrics tracked. Team can operate pipeline independently.'
      }
    ]
  },
