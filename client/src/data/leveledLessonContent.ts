...
export const dockerBasicsLesson: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description:
      "Learn to create, run, and manage Docker containers for application deployment.",
    learningObjectives: [
  ...
  crawl: {
    introduction:
      "In this Crawl-level exercise, you will follow step-by-step instructions to run your first Docker container. Every command is provided - your job is to execute them perfectly and understand what each does.",

    steps: [
      {
        stepNumber: 1,
        instruction:
          "First, let's verify Docker is installed and running on your system.",
        command: "docker --version",
        explanation:
          'This command checks the installed Docker version. You should see output like "Docker version 24.0.7".',
      ...
      },
      {
        ...
        explanation:
          'This downloads the nginx image from Docker Hub. The "latest" tag gets the most recent stable version.',
      ...
      },
      {
        ...
        explanation:
          "Lists all nginx images on your system with their size, creation date, and tags.",
      ...
      },
      {
        stepNumber: 4,
        instruction:
          "Run an nginx container in detached mode, mapping port 8080 to container port 80.",
        command: "docker run -d -p 8080:80 --name my-nginx nginx:latest",
        explanation:
          "-d runs in background (detached), -p maps host port 8080 to container port 80, --name gives it a friendly name.",
      ...
      },
      {
        ...
        command: "docker ps",
        explanation:
          "Lists all currently running containers with their status, ports, and names.",
      ...
      },
      {
        stepNumber: 6,
        instruction:
          "Test the nginx web server by accessing it from the command line.",
        command: "curl http://localhost:8080",
        explanation:
          "Sends HTTP request to the nginx server running in the container.",
      ...
      },
      {
        ...
        explanation:
          "Shows all output from the container, including access logs from your curl request.",
      ...
      },
      {
        ...
        explanation:
          "Sends SIGTERM to container, giving it 10 seconds to shut down gracefully before forcing.",
      ...
      },
      {
        ...
        explanation:
          "Permanently deletes the container. The image remains available for future use.",
        expectedOutput: "my-nginx",
        validationCriteria: [
          "Container name echoed",
    ...
    expectedOutcome:
      "You have successfully pulled a Docker image, run a container, tested it, viewed logs, and cleaned up. You understand the basic Docker workflow and can repeat these steps for any Docker image.",
  },

  // WALK: Fill-in-the-blank templates
  walk: {
    introduction:
      "Now you'll practice Docker commands with fill-in-the-blank templates. Key parameters are missing - use your knowledge from Crawl level to complete them correctly.",

    exercises: [
      {
        exerciseNumber: 1,
        scenario:
          'You need to run a Redis container in the background, mapping host port 6379 to container port 6379, with the name "my-redis".',
        template:
          "docker run __DETACH_FLAG__ -p __HOST_PORT__:__CONTAINER_PORT__ --name __CONTAINER_NAME__ redis:latest",
        blanks: [
          {
            id: "detach",
            label: "DETACH_FLAG",
        ...
        explanation:
          "This runs Redis in detached mode with port mapping. Both host and container use port 6379, which is Redis's default port.",
      },
      {
        exerciseNumber: 2,
        ...
        explanation:
          'Tags let you specify exact versions. Using "15" instead of "latest" ensures consistent behavior.',
      },
      {
        exerciseNumber: 3,
        ...
        explanation:
          "The -a flag shows all containers. Without it, only running containers appear.",
      },
    ],
  ...
  runGuided: {
    objective:
      "Deploy a web application using Docker with proper port mapping, environment variables, and volume mounting.",
    ...
    checkpoints: [
      ...
      },
      {
        ...
        hintIfStuck:
          "Check if required environment variables are set and ports aren't conflicting",
      },
      {
        ...
        hintIfStuck:
          "Verify port mapping with docker ps and test with curl localhost:PORT",
      },
      {
        ...
        hintIfStuck:
          "Use -v flag to mount volumes: -v /host/path:/container/path",
      },
    ],
  ...
  runIndependent: {
    objective:
      "Deploy a multi-container application (web server + database) that is production-ready with health checks, restart policies, and proper networking.",
    ...
    evaluationRubric: [
      {
        ...
        weight: 0.3,
        passingThreshold:
          'Both containers in "Up" status with no errors in logs',
      },
      {
        criterion: "Networking configured properly",
        weight: 0.2,
        passingThreshold:
          "Containers on custom bridge network, can communicate by name",
      },
      {
    ...
    commonErrors: [
      {
        ...
        solution:
          'Use "docker ps" and "sudo lsof -i :PORT" to find what\'s using the port, then either stop it or choose a different port',
        preventionTip:
      ...
      },
      {
        ...
        solution:
          'Remove existing container with "docker rm CONTAINER_NAME" or use a different name',
        preventionTip:
          "Use unique container names or let Docker auto-generate names",
      },
      {
        error:
