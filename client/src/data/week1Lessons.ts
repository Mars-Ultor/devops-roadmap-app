...
export const week1Lesson2LinuxBasics: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description: 'Master essential Linux commands for navigating, managing files, and understanding the filesystem.',
    learningObjectives: [
  ...
  crawl: {
    introduction: 'You will learn core Linux commands by executing each one step-by-step. Every command is provided with explanation.',
    steps: [
      {
        stepNumber: 1,
        instruction: 'Check your current location in the filesystem',
        command: 'pwd',
        explanation: 'pwd (print working directory) shows where you are in the filesystem. You always start in your home directory.',
        ...
        commonMistakes: ['Typing pwd with capital letters (Linux is case-sensitive)']
      },
      {
        stepNumber: 2,
        instruction: 'List all files in the current directory including hidden files',
        command: 'ls -la',
        explanation: 'ls lists files. -l shows details (permissions, owner, size). -a shows hidden files (starting with .).',
        expectedOutput: 'List of files with permissions, owner, size, and modification date',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'mkdir creates a new directory. This will be your workspace for learning.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'cd (change directory) moves you to a different folder. You can use relative paths (from current location) or absolute paths (from root /).',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'echo prints text. > redirects output to a file (creates or overwrites). This is how you create simple text files from the command line.',
      ...
      },
      {
        ...
        explanation: 'cat (concatenate) displays file contents. For large files, use less or head/tail instead.',
      ...
      },
      {
        ...
        explanation: 'cp copies files. First argument is source, second is destination. Original file remains unchanged.',
        validationCriteria: [
          'backup.txt created',
      ...
      },
      {
        ...
        explanation: '&& runs second command only if first succeeds. mv moves (renames) files. backups/ means "move into backups directory".',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Permissions show as: -rw-r--r-- meaning: - (file), rw- (owner can read/write), r-- (group can read), r-- (others can read).',
      ...
      },
      {
        ...
        explanation: 'chmod changes file permissions. +x adds execute permission for everyone. Verify with ls -l (permissions now show x).',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'rm removes files. Be careful! There\'s no recycle bin in Linux. Once deleted, it\'s gone. Use rm -i for confirmation prompts.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'cd .. moves up one directory. rm -r recursively deletes directory and all contents. -r flag required for directories.',
        validationCriteria: [
    ...
    expectedOutcome: 'You have successfully navigated the Linux filesystem, created files and directories, managed permissions, and cleaned up. You understand the basic commands needed for DevOps work.'
  },

  // WALK: Fill-in-the-blank templates
  walk: {
    introduction: 'Now practice Linux commands with fill-in-the-blank exercises. Key parameters are missing - use your knowledge from Crawl level.',
    exercises: [
      {
        exerciseNumber: 1,
        ...
        explanation: 'mkdir creates directories, cd navigates to them. && ensures second command runs only if first succeeds.'
      },
      {
        exerciseNumber: 2,
        scenario: 'Create a file "app.log" containing "Server started on port 8080"',
        ...
        explanation: 'echo outputs text, > writes to file (overwrites if exists). Use >> to append instead.'
      },
      {
        exerciseNumber: 3,
        scenario: 'List all files including hidden ones with detailed information',
        ...
        solution: 'ls -la',
        explanation: 'ls lists files. -l shows permissions/ownership/size. -a shows hidden files (starting with .). Flags can be combined as -la.'
      },
      {
        exerciseNumber: 4,
        ...
        explanation: 'chmod changes permissions. u+x adds execute for user (owner). Could also use numeric: chmod 744 app.log'
      },
      {
        exerciseNumber: 5,
        scenario: 'Copy app.log to backup.log and then move backup.log into a new "logs" directory',
        template: '__COPY__ app.log backup.log && mkdir logs && __MOVE__ backup.log logs/',
        blanks: [
          {
            id: '__COPY__',
            label: 'COPY',
        ...
        explanation: 'cp copies (source remains), mv moves (source removed). logs/ means "into logs directory".'
      }
    ],
    hints: [
  ...
  runGuided: {
    objective: 'Set up a directory structure for a web application with proper permissions',
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        description: 'webapp/ directory with src/, config/, logs/, backups/ subdirectories',
        validationCriteria: [
          'All 4 subdirectories exist',
          'Directory tree is organized logically',
          'pwd confirms you\'re in the right location'
        ],
        hintIfStuck: 'Use mkdir -p to create parent and child directories in one command'
      },
      {
        ...
        hintIfStuck: 'Use echo with > to create file, chmod 600 to restrict access to owner only'
      },
      {
        ...
        hintIfStuck: 'chmod 777 logs/ makes directory fully permissive (use with caution in production!)'
      }
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective: 'Create a complete Linux filesystem environment for a CI/CD pipeline with proper security',
    successCriteria: [
    ...
    evaluationRubric: [
      ...
      },
      {
        ...
        weight: 0.3,
        passingThreshold: 'secrets/ is 700, logs/ is 755, deploy.sh is executable'
      },
      {
        criterion: 'Functionality',
        weight: 0.2,
        passingThreshold: 'deploy.sh can be executed and contains valid commands'
      },
      {
