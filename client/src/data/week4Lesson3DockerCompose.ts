...
export const week4Lesson3DockerCompose: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description:
      "Master Docker Compose for defining and running multi-container applications with a single configuration file.",
    learningObjectives: [
  ...
  crawl: {
    introduction:
      "Learn Docker Compose step-by-step: YAML syntax, services, networks, volumes, and managing multi-container apps.",
    steps: [
      {
        stepNumber: 1,
        ...
        explanation:
          "Multi-container apps need networks, environment variables, volumes, dependencies. Running manually is error-prone. Hard to reproduce. Compose solves this with declarative config.",
        expectedOutput:
          "Understanding: Manual container orchestration is tedious and error-prone.",
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          "Compose file defines services (containers), networks, volumes. YAML format. Declare desired state. Run with one command: docker-compose up. Compose creates network automatically.",
        expectedOutput:
          "Understanding: Compose simplifies multi-container management with declarative configuration.",
        validationCriteria: [
      ...
      },
      {
        ...
        command:
          "docker-compose up\n# Or in detached mode:\ndocker-compose up -d",
        explanation:
          "docker-compose up reads docker-compose.yml, creates network, starts all services. -d runs in background. Compose ensures dependencies start first. Streams logs from all containers.",
        expectedOutput:
          "All containers start, network created, application ready. Logs show startup sequence.",
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          "Shows services defined in Compose file with status, ports. Like docker ps but filtered to this project. Lists service name, command, state, ports.",
        expectedOutput:
          "Table showing service names, state (Up/Exited), ports, container names",
        validationCriteria: [
          "All services listed",
      ...
      },
      {
        ...
        explanation:
          "docker-compose logs shows output from all services. -f follows (like tail -f). Specify service name for filtering. Color-coded by service. Useful for debugging.",
        expectedOutput:
          "Logs from all services, color-coded, optionally following new output",
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          "docker-compose down stops and removes containers, network. Preserves volumes by default. -v removes volumes (deletes data). Clean shutdown.",
        expectedOutput:
          "Containers stopped and removed, network removed, volumes preserved (unless -v)",
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          "Named volumes persist data across container restarts. Define in volumes section. Mount in service with service_name:/container/path. Data survives docker-compose down (unless -v).",
        expectedOutput:
          "Volume created, database data persists across container recreations",
        validationCriteria: [
      ...
      },
      {
        ...
        command:
          "version: '3.8'\nservices:\n  web:\n    networks:\n      - frontend\n  api:\n    networks:\n      - frontend\n      - backend\n  db:\n    networks:\n      - backend\n\nnetworks:\n  frontend:\n  backend:",
        explanation:
          "Custom networks segment services. Web only on frontend, API on both, DB only on backend. Security: web can't access DB directly. Define in networks section.",
        expectedOutput:
          "Multiple networks created, services connected appropriately, isolation enforced",
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          ".env file stores environment variables. Compose loads automatically. Use ${VAR_NAME} in Compose file. Keep secrets out of Compose file. Add .env to .gitignore.",
        expectedOutput:
          "Variables loaded from .env, containers receive environment variables",
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          "Compose can build images from Dockerfile. build: section specifies context and Dockerfile. docker-compose build builds images. --build rebuilds before starting.",
      ...
      },
      {
        ...
        explanation:
          "Run multiple instances of a service. --scale web=3 starts 3 web containers. Load balancer needed to distribute traffic. Useful for testing horizontal scaling.",
      ...
      },
      {
        ...
        explanation:
          "Complete app: Nginx (web server), API (application), PostgreSQL (database), Redis (cache). Dependencies ensure startup order. Volumes persist data. Environment variables configure connections.",
        expectedOutput:
          "Full-stack application running: web server, API, database, cache all connected",
        validationCriteria: [
    ...
    expectedOutcome:
      "You can write docker-compose.yml files for multi-tier applications, manage lifecycle (up, down, logs, ps), configure networks and volumes, use environment variables, build images, and scale services. You understand Compose as orchestration tool for development and simple production.",
  },
    ...
    exercises: [
      {
        ...
        explanation:
          "WordPress depends on MySQL. Volume persists database data. Environment variables configure database connection.",
      },
      {
        exerciseNumber: 2,
        ...
        solution:
          "services:\n  web:\n    networks:\n      - frontend\n  api:\n    networks:\n      - frontend\n      - backend\n  db:\n    networks:\n      - backend\n\nnetworks:\n  frontend:\n  backend:",
        explanation:
          "Network segmentation prevents web from directly accessing database. API bridges both networks.",
      },
      {
        exerciseNumber: 3,
        ...
        solution:
          'services:\n  app:\n    build:\n      context: ./app\n      dockerfile: Dockerfile\n    image: myapp:latest\n    ports:\n      - "3000:3000"\n\nRun:\ndocker-compose up --build',
        explanation:
          "Compose builds image from Dockerfile before running. --build flag ensures rebuild with latest changes.",
      },
      {
        exerciseNumber: 4,
        ...
        explanation:
          ".env file stores secrets. Compose loads automatically. Use ${VAR} syntax to reference.",
      },
      {
        exerciseNumber: 5,
        ...
        explanation:
          "Complete Compose lifecycle: start, monitor, scale, cleanup.",
      },
    ],
    hints: [
  ...
  runGuided: {
    objective:
      "Create Docker Compose configuration for a production-ready multi-tier application",
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck:
          "Example: nginx (reverse proxy) → node app (API) → postgres (database) + redis (cache). Each in separate service. Use networks to control communication.",
      },
      {
        checkpoint: "Production hardening implemented",
        description:
          "Health checks, restart policies, resource limits configured",
        validationCriteria: [
        ...
        hintIfStuck:
          "Add healthcheck with curl/wget to check if service responds. Set restart: unless-stopped. Use mem_limit: 512m, cpus: 0.5 to limit resources. Store secrets in .env file.",
      },
      {
        ...
        hintIfStuck:
          "Create README.md: architecture overview, prerequisites, how to run (docker-compose up), how to access (http://localhost:8080), how to stop. Include .env.example with empty/example values.",
      },
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective:
      "Build complete development environment with Docker Compose including application services, databases, monitoring, and tooling",
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 30,
        passingThreshold:
          "Well-designed multi-tier architecture. Proper service separation. Network segmentation for security. Volume strategy for persistence. Follows Docker and Compose best practices.",
      },
      {
        criterion: "Production Readiness",
        weight: 30,
        passingThreshold:
          "Health checks configured. Restart policies set. Resource limits prevent runaway containers. Secrets externalized. Logging configured. Monitoring in place. Ready for actual use.",
      },
      {
        criterion: "Developer Experience",
        weight: 20,
        passingThreshold:
          "Easy to start (docker-compose up). Hot reload for development. Clear logs. Good error messages. .env.example guides configuration. Single command to get running environment.",
      },
      {
        criterion: "Documentation",
        weight: 20,
        passingThreshold:
          "Architecture explained. Services documented. Setup instructions clear. Troubleshooting guide helpful. .env.example complete. Someone new can get environment running in under 10 minutes.",
      },
    ],
  },
