...
export const week1Lesson1WhatIsDevOps: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description: 'Understand DevOps philosophy, culture, and core practices that bridge development and operations.',
    learningObjectives: [
  ...
  crawl: {
    introduction: 'Learn DevOps concepts step-by-step through guided explanations and visual mental models. No coding yet - focus on understanding the WHY before the HOW.',
    steps: [
      {
        stepNumber: 1,
        instruction: 'Understand the problem: Traditional Waterfall Development',
        command: 'Imagine: Dev team writes code for 6 months → hands to Ops → "It works on my machine!" → Ops struggles to deploy → blame game begins',
        explanation: 'Before DevOps, developers and operations were separate silos. Developers wrote code, threw it "over the wall" to operations, who struggled to deploy and maintain it. No collaboration = slow releases, frequent failures.',
        expectedOutput: 'Mental model: Two teams with a wall between them, throwing code packages over, finger-pointing when things break.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'New model: Dev + Ops = DevOps → shared responsibility → build it, ship it, run it',
        explanation: 'DevOps merges development and operations into collaborative teams. Developers care about production stability. Ops cares about development velocity. Everyone owns the full lifecycle.',
        expectedOutput: 'Mental model: One unified team collaborating throughout planning, coding, building, testing, deploying, and monitoring.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Manual deployments = slow, error-prone, not repeatable. Automated deployments = fast, consistent, reliable.',
        explanation: 'Automation is the foundation of DevOps. Automate testing, building, deploying, infrastructure provisioning. Humans make mistakes; scripts don\'t (if written correctly). Automation enables speed and consistency.',
        expectedOutput: 'Mental model: Developer pushes code → automated tests run → automated build → automated deployment → automated monitoring alerts if broken.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Developers merge code to main branch multiple times per day → automated tests run → catch bugs early',
        explanation: 'CI means integrating code frequently (hourly/daily) instead of waiting weeks. Every commit triggers automated tests. Find bugs when they\'re fresh, not 6 months later. Small, frequent changes = easier to debug.',
        expectedOutput: 'Mental model: Developer commits → CI server pulls code → runs tests → notifies team if tests fail → fix immediately.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Continuous Delivery = code is always ready to deploy. Continuous Deployment = code auto-deploys to production.',
        explanation: 'CD extends CI: after code passes tests, it\'s automatically built and packaged for deployment. Continuous Delivery = manual approval to deploy. Continuous Deployment = fully automated to production. Both enable rapid releases.',
        expectedOutput: 'Mental model: CI passes → build artifact → staging deployment → (manual approval OR auto) → production deployment → monitoring.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Instead of manually configuring servers, write code that provisions infrastructure automatically.',
        explanation: 'IaC treats infrastructure (servers, networks, databases) as code. Version controlled, reviewable, repeatable. Spin up identical environments with one command. No more "snowflake servers" configured manually.',
        expectedOutput: 'Mental model: Infrastructure configuration in Git → code review → automated provisioning → identical dev/staging/prod environments.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Monitor everything: app performance, errors, user behavior → fast feedback → continuous improvement',
        explanation: 'DevOps requires observability. Monitoring alerts you to problems before users notice. Logs help debug issues. Metrics show trends. Feedback loops: monitor production → learn → improve → deploy changes → monitor again.',
        expectedOutput: 'Mental model: Production app → metrics/logs/alerts → dashboards → team responds to issues → deploys fixes → monitors effectiveness.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Stages: Code → Build → Test → Deploy → Monitor (repeat continuously)',
        explanation: 'A CI/CD pipeline is an automated workflow. Developer commits code, pipeline builds it, runs tests (unit, integration, security), deploys to environments (dev → staging → prod), and monitors results. Fully automated end-to-end.',
        expectedOutput: 'Mental model: Pipeline visualization with stages: Source Control → CI Server → Automated Tests → Build Artifacts → Deployment → Production Monitoring.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Version Control: Git. CI/CD: Jenkins, GitHub Actions. Containers: Docker. Orchestration: Kubernetes. IaC: Terraform. Monitoring: Prometheus, Grafana.',
        explanation: 'DevOps has many tools, but they fall into categories: version control (Git), CI/CD platforms (Jenkins, CircleCI), containerization (Docker), orchestration (Kubernetes), infrastructure automation (Terraform, Ansible), monitoring (Datadog, New Relic). Tools change, principles stay.',
        expectedOutput: 'Mental model: Tool categories mapped to pipeline stages. Different tools, same workflow.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Blameless postmortems: When failure happens, focus on fixing systems, not blaming people.',
        explanation: 'DevOps culture emphasizes psychological safety. Failures are learning opportunities. Blameless postmortems analyze what went wrong without pointing fingers. Goal: improve processes and prevent recurrence. Blame culture kills innovation.',
        expectedOutput: 'Mental model: Incident → blameless postmortem → root cause analysis → system improvements → better resilience.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Key metrics: Deployment Frequency, Lead Time, Mean Time to Recovery (MTTR), Change Failure Rate',
        explanation: 'DevOps performance is measurable. Deploy Frequency = how often you release. Lead Time = time from code commit to production. MTTR = how fast you recover from failures. Change Failure Rate = % of deployments causing incidents. Elite teams deploy multiple times/day with <1hr recovery.',
        expectedOutput: 'Mental model: Metrics dashboard showing trends over time, comparing team to industry benchmarks.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Faster time to market, higher quality, better reliability, happier teams, competitive advantage',
        explanation: 'DevOps delivers measurable business value: ship features faster (competitive advantage), fewer bugs (automated testing), less downtime (monitoring + fast recovery), reduced costs (automation), happier engineers (less firefighting, more innovation). Not just tech improvement - business transformation.',
        expectedOutput: 'Mental model: DevOps practices → faster delivery + higher quality + better stability → business growth + customer satisfaction.',
        validationCriteria: [
    ...
    expectedOutcome: 'You understand DevOps as a cultural and technical transformation. You can explain core principles (automation, CI/CD, IaC, monitoring), describe pipeline stages, identify key tools, and articulate business value. You\'re ready to apply DevOps thinking.'
  },

  walk: {
    introduction: 'Apply DevOps knowledge by analyzing scenarios and matching practices to problems. Fill in blanks to demonstrate understanding.',
    exercises: [
      {
        exerciseNumber: 1,
        scenario: 'A company deploys to production once per quarter. Deployments take 12 hours and frequently fail. Teams blame each other.',
        template: 'Problem: __LONG_RELEASE_CYCLE__ and __MANUAL_PROCESS__\nDevOps Solution: Implement __CI_CD__ to enable __FREQUENT_RELEASES__\nCultural Fix: Establish __BLAMELESS_CULTURE__ and __SHARED_OWNERSHIP__',
        blanks: [
          {
        ...
        solution: 'Problem: infrequent deployments and lack of automation\nDevOps Solution: Implement CI/CD pipeline to enable daily/weekly releases\nCultural Fix: Establish blameless postmortems and shared responsibility',
        explanation: 'Quarterly releases = large, risky changes. Automation + frequent small releases reduce risk. Blameless culture enables learning from failures.'
      },
      {
        exerciseNumber: 2,
        scenario: 'Development works fine locally, but production environment is configured differently. Deployments break due to environment inconsistencies.',
        template: 'Root Cause: __MANUAL_CONFIG__ leads to __ENVIRONMENT_DRIFT__\nDevOps Practice: Use __IAC__ to define infrastructure as __VERSION_CONTROLLED__ code\nBenefit: __REPRODUCIBLE__ environments across dev/staging/prod',
        blanks: [
          {
            id: 'MANUAL_CONFIG',
            label: 'MANUAL_CONFIG',
        ...
        solution: 'Root Cause: manual server configuration leads to configuration drift\nDevOps Practice: Use Infrastructure as Code to define infrastructure as version-controlled code\nBenefit: consistent and reproducible environments across dev/staging/prod',
        explanation: 'Manual configuration creates "snowflake servers". IaC ensures all environments match, eliminating "works on my machine" problems.'
      },
      {
        exerciseNumber: 3,
        scenario: 'A critical bug is discovered in production. It takes 4 hours to identify the root cause because there\'s no centralized logging.',
        template: 'Missing DevOps Practice: __MONITORING_OBSERVABILITY__\nImplementation: Deploy __CENTRALIZED_LOGGING__ and __METRICS_DASHBOARD__\nResult: Reduce __MTTR__ from 4 hours to __MINUTES__\nEnables: __PROACTIVE__ issue detection before users report',
        blanks: [
          {
        ...
        solution: 'Missing DevOps Practice: monitoring and observability\nImplementation: Deploy centralized logging and metrics dashboards\nResult: Reduce MTTR from 4 hours to minutes\nEnables: proactive issue detection before users report',
        explanation: 'Without observability, debugging is guesswork. Centralized logging + metrics enable fast root cause analysis and proactive alerting.'
      },
      {
        exerciseNumber: 4,
        scenario: 'Developers merge a week\'s worth of code at once. Integration takes 2 days and reveals hundreds of conflicts and test failures.',
        template: 'Anti-pattern: __BIG_BANG_INTEGRATION__ (infrequent, large merges)\nDevOps Practice: __CONTINUOUS_INTEGRATION__ with __SMALL_COMMITS__\nRequires: __AUTOMATED_TESTS__ that run on every __COMMIT__\nBenefit: Catch bugs __IMMEDIATELY__ when code is fresh, not days later',
        blanks: [
          {
      ...
      },
      {
        ...
        explanation: 'DORA metrics (from Accelerate book) measure DevOps performance. Elite teams optimize all four metrics simultaneously.'
      }
    ],
    hints: [
  ...
  runGuided: {
    objective: 'Analyze your current organization\'s development workflow and identify DevOps improvement opportunities',
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        description: 'Written analysis of existing development and deployment workflow',
        validationCriteria: [
        ...
        hintIfStuck: 'Interview team members: developers, QA, operations. Ask: "Describe the process from code commit to production." Map every step.'
      },
      {
        ...
        hintIfStuck: 'Common pain points: manual testing, environment drift, slow deployments, lack of monitoring, siloed teams. Which apply to your org?'
      },
      {
        checkpoint: 'DevOps transformation proposal',
        description: 'Concrete plan with pilot project, timeline, and success criteria',
        validationCriteria: [
        ...
        hintIfStuck: 'Start small: pick one service, automate its testing and deployment first. Prove value before scaling.'
      }
    ],
    resourcesAllowed: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 25,
        passingThreshold: 'Current state includes specific metrics, not vague statements. Pain points are concrete and well-understood.'
      },
      {
        criterion: 'DevOps Understanding',
        weight: 30,
        passingThreshold: 'Demonstrates understanding of culture, automation, CI/CD, IaC, and monitoring. Proposed solutions align with DevOps principles.'
      },
      {
        criterion: 'Practical Roadmap',
        weight: 25,
        passingThreshold: 'Transformation plan is realistic, phased, and prioritized. Starts small, scales incrementally. Includes timelines and milestones.'
      },
      {
        criterion: 'Business Value',
        weight: 20,
        passingThreshold: 'Articulates business impact: faster time to market, higher quality, reduced costs. Uses metrics to quantify improvement.'
      }
    ]
  },
