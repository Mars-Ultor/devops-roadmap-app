...
export const week6Lesson1IaCConcepts: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description: 'Understand IaC principles, benefits, and how it revolutionizes infrastructure management.',
    learningObjectives: [
  ...
  crawl: {
    introduction: 'Learn Infrastructure as Code step-by-step: problems it solves, declarative vs imperative, IaC tools, version control benefits, and best practices.',
    steps: [
      {
        stepNumber: 1,
        instruction: 'The problem: Manual infrastructure management',
        command: 'Traditional manual process:\n1. Log into AWS console\n2. Click buttons to create EC2 instance\n3. Configure security groups manually\n4. Set up load balancer through UI\n5. Document changes in wiki (maybe)\n6. Repeat for staging and production\n\nProblems:\n- Inconsistent environments (different settings)\n- No audit trail (who changed what?)\n- Slow to replicate (click everything again)\n- Human error (forget security group rule)\n- No versioning (can\'t rollback infrastructure)',
        explanation: 'Manual infrastructure: Click-ops through console. Inconsistent configurations. No history of changes. Mistakes happen. Can\'t easily replicate or rollback. Knowledge in people\'s heads, not documented.',
        expectedOutput: 'Understanding: Manual infrastructure management is error-prone, slow, and not reproducible.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'IMPERATIVE (How to do it):\n# Script with explicit steps\naws ec2 run-instances --image-id ami-123\naws ec2 create-security-group --group-name web\naws ec2 authorize-security-group-ingress --port 80\n\nDECLARATIVE (What you want):\n# Describe desired state\nresource "aws_instance" "web" {\n  ami = "ami-123"\n  instance_type = "t2.micro"\n}\n\nresource "aws_security_group" "web" {\n  ingress {\n    from_port = 80\n    to_port = 80\n  }\n}\n\nTool figures out how to create it',
        explanation: 'Imperative: Explicit steps (create this, then that). Declarative: Describe desired end state, tool handles how. Declarative is easier to understand and maintain. Most modern IaC tools are declarative.',
        expectedOutput: 'Understanding: Declarative IaC describes desired state, tool determines steps to achieve it.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Git for infrastructure:\n\n# History of all changes\ngit log infrastructure/main.tf\n\n# Who changed what when\ngit blame infrastructure/main.tf\n\n# Rollback to previous version\ngit revert abc123\nterraform apply\n\n# Branch for new feature\ngit checkout -b add-monitoring\n# Add monitoring infrastructure\ngit commit -m "Add CloudWatch alarms"\n# PR review before merging',
        explanation: 'Infrastructure in Git = full audit trail. See who changed what, when, why (commit messages). Rollback to previous state. Branch for experimentation. PR review for infrastructure changes. Same workflow as application code.',
        expectedOutput: 'Understanding: Version control provides infrastructure change history, rollback, and review processes.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Idempotent operation:\n\n# First run: Creates 5 EC2 instances\nterraform apply\n\n# Second run (no code changes): No changes made\nterraform apply\n# Output: "No changes. Infrastructure is up-to-date."\n\n# If instance manually deleted:\nterraform apply\n# Output: "Will create 1 EC2 instance to match desired state"\n\nResult: Same every time, regardless of starting state',
        explanation: 'Idempotency = same result no matter how many times you run it. IaC tools detect current state, only make necessary changes. Run again with no code changes = no infrastructure changes. Corrects drift (manual changes).',
        expectedOutput: 'Understanding: Idempotent IaC can be run repeatedly safely, ensuring desired state.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Configuration drift = actual infrastructure differs from code. Manual changes cause drift. IaC detects drift on next run. Reverts to code-defined state. Enforces "code is truth" principle.',
        expectedOutput: 'Understanding: IaC detects and corrects drift, maintaining infrastructure consistency with code.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Major IaC tools:\n\nTERRAFORM (HashiCorp):\n- Multi-cloud (AWS, Azure, GCP, etc.)\n- Declarative HCL language\n- Large provider ecosystem\n- State management\n\nCLOUDFORMATION (AWS):\n- AWS only\n- Declarative YAML/JSON\n- Native AWS integration\n- No external state\n\nPULUMI:\n- Multi-cloud\n- Use real programming languages (TypeScript, Python)\n- Modern, flexible\n\nANSIBLE:\n- Configuration management + provisioning\n- Agentless\n- YAML playbooks',
        explanation: 'Terraform: Most popular, multi-cloud, large ecosystem. CloudFormation: AWS native, simpler for AWS-only. Pulumi: Code in real languages. Ansible: Broader config management. Choice depends on cloud, team skills, requirements.',
        expectedOutput: 'Understanding: Multiple IaC tools available, each with strengths for different use cases.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'State file (terraform.tfstate):\n{\n  "version": 4,\n  "resources": [\n    {\n      "type": "aws_instance",\n      "name": "web",\n      "instances": [{"id": "i-123456", "public_ip": "1.2.3.4"}]\n    }\n  ]\n}\n\nState tracks:\n- What resources exist\n- Their current configuration\n- Dependencies between resources\n- Metadata (created time, tags)\n\nCRITICAL: Store state remotely (S3, Terraform Cloud), not locally',
        explanation: 'State file = IaC tool\'s memory of infrastructure. Maps code to actual resources. Used to detect drift and plan changes. Must be shared across team (remote storage). Losing state = losing track of infrastructure.',
        expectedOutput: 'Understanding: State files track infrastructure reality, must be stored remotely for team collaboration.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Modules = reusable infrastructure components. Define once, use many times with different parameters. Promotes consistency. Reduces duplication. Teams share modules. Like functions in programming.',
        expectedOutput: 'Understanding: Modules enable infrastructure code reuse and standardization.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Environment strategy:\n\nOPTION 1: Separate directories\ninfrastructure/\n  dev/\n    main.tf\n  staging/\n    main.tf\n  production/\n    main.tf\n\nOPTION 2: Workspaces\nterraform workspace new dev\nterraform workspace new staging\nterraform workspace new production\n\nOPTION 3: Separate repos\ninfra-dev/\ninfra-staging/\ninfra-production/\n\nBest practice: Same code, different variable files',
        explanation: 'Multiple environment management strategies. Separate dirs/workspaces/repos. Key: use same IaC code with different variables (instance sizes, counts). Ensures consistency while allowing environment-specific config.',
        expectedOutput: 'Understanding: Multiple strategies for managing dev, staging, production with IaC.',
        validationCriteria: [
    ...
    expectedOutcome: 'You understand IaC principles, benefits over manual infrastructure, declarative vs imperative approaches, idempotency, state management, modules, and environment strategies. You can explain when and why to use IaC.'
  },

  walk: {
    introduction: 'Apply Infrastructure as Code concepts through scenario-based exercises.',
    exercises: [
      {
        exerciseNumber: 1,
        scenario: 'Compare manual infrastructure vs IaC for multi-environment setup.',
        template: 'MANUAL APPROACH:\n- Time: __HOURS__ to set up each environment\n- Consistency: __INCONSISTENT__ (human error)\n- Documentation: __OUTDATED__ wiki pages\n- Rollback: __DIFFICULT__ (manual steps)\n- Audit: __NO_TRAIL__ of who changed what\n\nIaC APPROACH:\n- Time: __MINUTES__ (terraform apply)\n- Consistency: __IDENTICAL__ environments (same code)\n- Documentation: __CODE_IS_DOCS__ (self-documenting)\n- Rollback: __GIT_REVERT__ (seconds)\n- Audit: __FULL_HISTORY__ in Git',
        blanks: [
          {
            id: 'HOURS',
            label: 'HOURS',
  ...
  runGuided: {
    objective: 'Analyze existing manual infrastructure and create IaC migration plan with benefits justification',
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck: 'Interview team members who create infrastructure manually. Time how long it takes to set up dev environment. Document differences between dev, staging, production.'
      },
      {
        ...
        hintIfStuck: 'For AWS-heavy: consider Terraform for flexibility or CloudFormation for simplicity. Plan modules for common patterns (VPC, web server, database). Use separate directories or workspaces for environments. Store state in S3 with DynamoDB locking.'
      },
      {
        ...
        hintIfStuck: 'Calculate: current setup time vs IaC (80% reduction). Estimate error cost (downtime, debugging). Value of instant rollback (vs hours of manual work). Plan pilot with one environment first.'
      }
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective: 'Create comprehensive Infrastructure as Code adoption proposal for organization including technical design, migration strategy, business case, and implementation roadmap',
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 25,
        passingThreshold: 'Thorough documentation of current state with specific pain points, time measurements, and concrete examples. Clear understanding of problems IaC will solve. Quantified benefits (hours saved, error reduction).'
      },
      {
        criterion: 'Technical Design',
        weight: 30,
        passingThreshold: 'Complete IaC architecture with well-designed modules, clear environment strategy, proper state management. Sample code demonstrates understanding. CI/CD integration planned. Rollback procedures defined.'
      },
      {
        criterion: 'Business Value',
        weight: 25,
        passingThreshold: 'Compelling business case with ROI calculation, risk analysis, and clear value proposition. Success metrics defined. Implementation roadmap is realistic and phased. Addresses stakeholder concerns.'
      },
      {
        criterion: 'Implementation Readiness',
        weight: 20,
        passingThreshold: 'Detailed implementation plan with training, documentation, and team enablement. Risk mitigation strategies. Realistic timeline. Team can execute plan independently. Lessons learned captured for continuous improvement.'
      }
    ]
  },
