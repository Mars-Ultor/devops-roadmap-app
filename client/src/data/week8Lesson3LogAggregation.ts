...
export const week8Lesson3LogAggregation: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description: 'Deploy and operate centralized logging solutions (ELK or Loki) for distributed systems. Master structured logging, log querying, and correlation with traces.',
    learningObjectives: [
  ...
  crawl: {
    introduction: 'Master centralized log aggregation for microservices. Learn to collect, parse, search, and analyze logs from distributed Kubernetes applications using ELK Stack or Grafana Loki.',
    steps: [
      {
        stepNumber: 1,
        instruction: 'Why Log Aggregation? The distributed logging problem',
        command: 'Problem: Microservices on Kubernetes\n- 10 services × 3 replicas = 30 pods\n- Pods restart (ephemeral logs lost)\n- Need to search across all pods\n- Logs scattered across cluster\n\nSolution: Centralized log aggregation\n1. Collect logs from all pods automatically\n2. Store in searchable database\n3. Provide UI for querying\n4. Retain logs after pods die',
        explanation: 'kubectl logs only shows one pod. Logs disappear when pod restarts. Debugging requires searching all replicas. Centralized logging collects from all pods, stores centrally, survives pod restarts, enables search across services.',
        expectedOutput: 'Understanding: Centralized log aggregation is essential for debugging distributed systems.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Install Loki stack with Helm:\n\nhelm repo add grafana https://grafana.github.io/helm-charts\nhelm repo update\nhelm install loki grafana/loki-stack \\\n  --namespace logging --create-namespace \\\n  --set grafana.enabled=true \\\n  --set promtail.enabled=true \\\n  --set loki.persistence.enabled=true \\\n  --set loki.persistence.size=10Gi\n\nComponents deployed:\n- Loki: Log storage\n- Promtail: DaemonSet (runs on every node, collects logs)\n- Grafana: UI for querying',
        explanation: 'Promtail runs as DaemonSet - one pod per node. It reads container logs (/var/log/pods/) and sends to Loki. Loki stores logs with labels (pod, namespace, container). Grafana queries Loki. Enable persistence so logs survive Loki pod restarts.',
        expectedOutput: 'Understanding: Loki stack deploys centralized logging with automatic collection from all pods.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Promtail operation:\n1. Runs on every Kubernetes node (DaemonSet)\n2. Watches /var/log/pods/* (container stdout/stderr)\n3. Extracts labels from file path:\n   /var/log/pods/namespace_pod_uid/container/0.log\n   → {namespace="default", pod="api-123", container="api"}\n4. Adds custom labels from pod annotations\n5. Sends log lines to Loki with labels\n\nLoki indexes labels (not log content) for fast filtering',
        explanation: 'Kubernetes writes container logs to files on node. Promtail reads these files. File path contains metadata (namespace, pod, container). Promtail extracts this as labels. Loki stores logs with labels. Query by label (namespace="prod") instead of full-text search.',
        expectedOutput: 'Understanding: Promtail automatically discovers and labels logs from Kubernetes pod metadata.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Unstructured log (bad):\n2025-12-11 10:32:15 ERROR: Payment failed for user 12345 - Insufficient funds\n\nStructured log (good - JSON):\n{\n  "timestamp": "2025-12-11T10:32:15.123Z",\n  "level": "ERROR",\n  "message": "payment_failed",\n  "user_id": "12345",\n  "order_id": "67890",\n  "error_code": "INSUFFICIENT_FUNDS",\n  "amount": 99.99,\n  "service": "payment-service",\n  "trace_id": "abc-123"\n}\n\nBenefits:\n- Machine-parseable\n- Filterable by any field\n- Correlatable with traces (trace_id)',
        explanation: 'Structured logs are JSON (or key-value format). Every field is extractable. Can filter "all INSUFFICIENT_FUNDS errors" or "all errors for user 12345". Loki can parse JSON and create labels. Correlation IDs (trace_id) link logs to distributed traces.',
        expectedOutput: 'Understanding: Structured JSON logs enable powerful querying and correlation with traces.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'LogQL query examples:\n\n# All logs from namespace\n{namespace="production"}\n\n# Logs from specific service\n{namespace="production", app="api-service"}\n\n# Filter by log content (grep-like)\n{app="api-service"} |= "error"\n\n# Parse JSON and filter by field\n{app="api-service"} | json | error_code="INSUFFICIENT_FUNDS"\n\n# Count error logs\nsum(count_over_time({app="api-service"} |= "ERROR" [5m]))\n\n# Logs for specific trace_id (correlation)\n{namespace="production"} | json | trace_id="abc-123"',
        explanation: 'LogQL is like PromQL for logs. Start with label selector {namespace="prod"}. Filter content with |= (contains). Parse JSON with | json. Extract fields. Aggregate with count_over_time(). Query by trace_id to correlate with distributed traces.',
        expectedOutput: 'Understanding: LogQL enables label-based querying and JSON field extraction.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'ELK deployment (alternative to Loki):\n\n# Add Elastic Helm repo\nhelm repo add elastic https://helm.elastic.co\n\n# Install Elasticsearch\nhelm install elasticsearch elastic/elasticsearch \\\n  --namespace logging --create-namespace \\\n  --set replicas=3 \\\n  --set volumeClaimTemplate.resources.requests.storage=30Gi\n\n# Install Kibana\nhelm install kibana elastic/kibana --namespace logging\n\n# Install Fluentd (log collector)\nkubectl apply -f fluentd-daemonset.yaml\n\nComponents:\n- Elasticsearch: Search engine and storage\n- Fluentd: Collects logs, parses, sends to Elasticsearch\n- Kibana: Web UI for search and visualization',
        explanation: 'Elasticsearch is distributed search engine. Fluentd (or Fluent Bit, Logstash) collects logs from all pods. Parses and enriches logs. Sends to Elasticsearch. Kibana provides UI for querying. More powerful than Loki but resource-intensive.',
        expectedOutput: 'Understanding: ELK Stack provides full-text indexed search with rich querying capabilities.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Fluentd config (fluentd.conf):\n\n<source>\n  @type tail\n  path /var/log/containers/*.log\n  pos_file /var/log/fluentd-containers.log.pos\n  tag kubernetes.*\n  read_from_head true\n  <parse>\n    @type json\n    time_format %Y-%m-%dT%H:%M:%S.%NZ\n  </parse>\n</source>\n\n# Add Kubernetes metadata\n<filter kubernetes.**>\n  @type kubernetes_metadata\n  @id filter_kube_metadata\n</filter>\n\n# Send to Elasticsearch\n<match **>\n  @type elasticsearch\n  host elasticsearch.logging.svc.cluster.local\n  port 9200\n  index_name k8s-logs\n</match>',
        explanation: 'Fluentd reads container logs. Parses JSON. kubernetes_metadata filter adds pod name, namespace, labels. Sends to Elasticsearch with index name. Fluentd can transform logs, add fields, filter, route. More flexible than Promtail.',
        expectedOutput: 'Understanding: Fluentd provides powerful log parsing, transformation, and routing capabilities.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Kibana operations:\n\n1. Create index pattern:\n   Management → Index Patterns → Create\n   Pattern: k8s-logs-*\n   Time field: @timestamp\n\n2. Search logs (Discover):\n   kubernetes.namespace: "production" AND level: "ERROR"\n   message: "payment failed"\n   trace_id: "abc-123"\n\n3. Save search as visualization\n\n4. Create dashboard:\n   - Error count over time (line chart)\n   - Error breakdown by service (pie chart)\n   - Recent errors (data table)\n   - Top error messages (tag cloud)',
        explanation: 'Kibana Discover lets you search logs. Create visualizations from searches. Combine visualizations into dashboards. Powerful query language (KQL or Lucene). Can create alerts based on log patterns. Export dashboards for sharing.',
        expectedOutput: 'Understanding: Kibana provides rich search, visualization, and dashboarding for logs.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Elasticsearch Index Lifecycle (ILM):\n\nPUT _ilm/policy/k8s-logs-policy\n{\n  "policy": {\n    "phases": {\n      "hot": {\n        "actions": {\n          "rollover": {\n            "max_size": "50GB",\n            "max_age": "1d"\n          }\n        }\n      },\n      "warm": {\n        "min_age": "7d",\n        "actions": {\n          "shrink": { "number_of_shards": 1 },\n          "forcemerge": { "max_num_segments": 1 }\n        }\n      },\n      "delete": {\n        "min_age": "30d",\n        "actions": { "delete": {} }\n      }\n    }\n  }\n}\n\nStrategy:\n- Hot: Recent logs (7 days), fast SSD storage\n- Warm: Older logs (7-30 days), compressed, slower storage\n- Delete: After 30 days',
        explanation: 'Logs grow infinitely if not managed. ILM automatically moves logs through lifecycle. Hot phase: recent, fast. Warm phase: compressed, cheaper storage. Delete: remove old logs. Prevents storage exhaustion and controls costs.',
        expectedOutput: 'Understanding: Index lifecycle management automates log retention and storage optimization.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Correlation strategy:\n\n1. Application emits log:\n   {\n     "trace_id": "abc-123",\n     "span_id": "span-456",\n     "service": "api",\n     "message": "payment_failed",\n     "timestamp": "2025-12-11T10:32:15Z"\n   }\n\n2. Distributed trace has same trace_id\n\n3. Grafana or Kibana:\n   - View trace in Jaeger\n   - Click span to see logs (filtered by trace_id)\n   - See metrics for same time period\n\n4. Complete picture:\n   Metric → spike in error rate\n   Log → error message "database timeout"\n   Trace → slow span in database service',
        explanation: 'Correlation IDs (trace_id, request_id) are the key. Include in all logs. When debugging, start with metric alert, find trace_id in logs, pull up trace, see full request path. Complete observability requires linking all three pillars.',
        expectedOutput: 'Understanding: Correlation IDs enable seamless navigation between metrics, logs, and traces.',
        validationCriteria: [
      ...
      {
        stepNumber: 12,
        instruction: 'Log aggregation best practices: Security, performance, cost',
        ...
        explanation: 'Logging production systems requires balancing visibility, security, performance, and cost. Don\'t log secrets. Retain logs long enough for debugging (30-90 days typical). High-volume services may need sampling. Secure access to logs (sensitive data). Monitor log collector health.',
        expectedOutput: 'Understanding: Production log aggregation requires careful balance of visibility, security, performance, and cost.',
        validationCriteria: [
    ...
    expectedOutcome: 'You understand centralized log aggregation architectures (ELK vs Loki), can deploy and configure log collectors (Promtail, Fluentd), query logs (LogQL, KQL), implement structured logging, manage retention, and correlate logs with traces and metrics.'
  },
        ...
        solution: `PUT _ilm/policy/k8s-logs-policy
{
  "policy": {
    "phases": {
      ...
      "delete": {
        ...
        explanation: 'Elasticsearch Index Lifecycle Management (ILM) policy for log retention'
      }
    ],
    hints: [
  ...
  runGuided: {
    objective: 'Deploy complete centralized logging solution for Kubernetes application',
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck: 'Loki: Lightweight, integrates with Grafana, cheaper. ELK: More powerful search, better for compliance. Use Helm charts for both options. Verify DaemonSet pods running on all nodes. Test with: {namespace="default"} or k8s.namespace: "default"'
      },
      {
        ...
        hintIfStuck: 'Use logging libraries: Winston (Node.js), Logrus (Go), Log4j2 (Java). Test JSON parsing: kubectl logs pod-name | jq. Include trace_id from OpenTelemetry context. Add structured fields, not formatted strings. Review for sensitive data before deploying.'
      },
      {
        ...
        hintIfStuck: 'Loki: Use LogQL in Grafana Explore. ELK: Use KQL in Kibana Discover. Create visualizations from saved searches. Test correlation: generate trace_id, find in logs. Configure ILM or retention in Loki config. Use Kubernetes RBAC or Grafana teams for access control.'
      }
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective: 'Build production-ready centralized logging platform with complete observability correlation',
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 30,
        passingThreshold: 'High-availability logging stack deployed (Loki or ELK). Log collection from all pods via DaemonSet. Persistence configured. Retention policy defined. Production-ready performance and reliability.'
      },
      {
        criterion: 'Structured Logging',
        weight: 25,
        passingThreshold: 'All services emit JSON logs. Consistent schema (timestamp, level, service, trace_id, message). Context propagation working. Logs include relevant metadata for debugging. Query performance optimized via proper structure.'
      },
      {
        criterion: 'Query and Correlation',
        weight: 25,
        passingThreshold: 'Saved queries enable common investigations. Can correlate logs via trace_id. Dashboards show log trends. Can navigate from alert → logs → trace → root cause. Fast query performance (<5 seconds).'
      },
      {
        criterion: 'Operational Excellence',
        weight: 20,
        passingThreshold: 'Retention policy prevents storage issues. Access controls configured. Log-based alerting working. Complete observability demonstrated (metrics + logs + traces). Documentation enables team self-service.'
      }
    ]
  },
