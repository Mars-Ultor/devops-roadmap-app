...
export const week4Lesson2DockerBasics: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description: 'Learn Docker basics: images, containers, running applications, and basic lifecycle management.',
    learningObjectives: [
  ...
  crawl: {
    introduction: 'Learn Docker by running containers step-by-step. You will pull images, run containers, and manage their lifecycle.',
    steps: [
      {
        stepNumber: 1,
        instruction: 'Verify Docker is installed and running',
        command: 'docker --version',
        explanation: 'Check that Docker is installed. This shows the version of Docker engine.',
      ...
      },
      {
        ...
        explanation: 'Download the nginx web server image from Docker Hub. "latest" is a tag indicating the most recent version.',
        expectedOutput: 'latest: Pulling from library/nginx\n...\nStatus: Downloaded newer image for nginx:latest',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'See all images on your system. Shows repository name, tag, image ID, creation time, and size.',
      ...
      },
      {
        ...
        explanation: '-d runs detached (background), -p maps port 8080 on host to port 80 in container, --name gives it a friendly name.',
      ...
      },
      {
        ...
        command: 'docker ps',
        explanation: 'Shows all currently running containers with details: ID, image, command, status, ports, and name.',
      ...
      },
      {
        ...
        explanation: 'Access the nginx web server running in the container through the exposed port 8080.',
        expectedOutput: 'HTML content starting with <!DOCTYPE html> and containing "Welcome to nginx!"',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'See stdout and stderr output from the container. Shows nginx access logs and errors.',
        expectedOutput: '172.17.0.1 - - [date/time] "GET / HTTP/1.1" 200 615 "-" "curl/7.68.0"',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Opens an interactive bash shell inside the running container. -it means interactive with TTY (terminal).',
        expectedOutput: 'root@abc123def456:/# (you are now inside the container)',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Gracefully stop the container. Sends SIGTERM, waits 10 seconds, then SIGKILL if needed.',
      ...
      },
      {
        ...
        explanation: 'Restart a stopped container. It resumes with same configuration (ports, name, etc.).',
        expectedOutput: 'my-nginx',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Stop then delete the container. && ensures rm only runs if stop succeeds.',
        expectedOutput: 'my-nginx\nmy-nginx',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Delete the nginx image from your system. Frees up disk space.',
    ...
    expectedOutcome: 'You have mastered Docker basics: pulling images, running containers, executing commands inside containers, managing lifecycle (start/stop/remove). You can now run containerized applications.'
  },

  walk: {
    introduction: 'Apply Docker knowledge with fill-in-the-blank exercises covering common container workflows.',
    exercises: [
      {
        exerciseNumber: 1,
        scenario: 'You need to run a Redis database container in the background with port mapping.',
        template: 'docker _____ redis:latest\ndocker _____ -d -p 6379:6379 --name my-redis redis:latest\ndocker _____ my-redis',
        blanks: [
          {
            id: 'pull',
        ...
        solution: 'docker pull redis:latest\ndocker run -d -p 6379:6379 --name my-redis redis:latest\ndocker ps',
        explanation: 'pull downloads image, run -d starts container in background with port mapping, ps confirms it is running.'
      },
      {
        exerciseNumber: 2,
        scenario: 'Inspect logs and execute a command inside a running PostgreSQL container.',
        template: 'docker _____ my-postgres\ndocker _____ -it my-postgres psql -U postgres',
        blanks: [
          {
            id: 'logs',
        ...
        solution: 'docker logs my-postgres\ndocker exec -it my-postgres psql -U postgres',
        explanation: 'logs shows container output, exec runs command inside running container. -it makes it interactive.'
      },
      {
        exerciseNumber: 3,
        scenario: 'Stop and remove a container, then clean up the image.',
        template: 'docker _____ my-app\ndocker _____ my-app\ndocker _____ myapp-image:1.0',
        blanks: [
          {
            id: 'stop',
        ...
        solution: 'docker stop my-app\ndocker rm my-app\ndocker rmi myapp-image:1.0',
        explanation: 'stop pauses container, rm deletes container, rmi removes image. Must remove container before image.'
      },
      {
        exerciseNumber: 4,
        scenario: 'Run a temporary container that auto-removes after execution.',
        template: 'docker run _____ ubuntu echo "Hello Docker"\ndocker _____ -a',
        blanks: [
          {
            id: 'autoRemove',
        ...
        explanation: '--rm flag auto-deletes container after exit. Useful for one-off commands. ps -a shows it is gone.'
      },
      {
        exerciseNumber: 5,
        scenario: 'List all images, then remove unused/dangling images to free space.',
        ...
        explanation: 'images lists all images, prune -a removes unused images to free disk space.'
      }
    ],
    hints: [
  ...
  runGuided: {
    objective: 'Deploy a multi-tier web application using Docker containers (web server + database)',
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        description: 'MySQL container running with persistent volume and accessible on port 3306',
        validationCriteria: [
        ...
        hintIfStuck: 'Use: docker run -d --name mysql-db -e MYSQL_ROOT_PASSWORD=secret -e MYSQL_DATABASE=myapp -v mysql-data:/var/lib/mysql -p 3306:3306 mysql:8.0'
      },
      {
        checkpoint: 'Web server accessible',
        description: 'Web server container running and accessible via browser on port 80 or 8080',
        validationCriteria: [
        ...
        hintIfStuck: 'Use nginx or httpd image, map port with -p, mount custom HTML with -v or copy files in Dockerfile'
      },
      {
        checkpoint: 'Data persists across restarts',
        description: 'Verified data persistence: stop/start containers without data loss',
        validationCriteria: [
        ...
        hintIfStuck: 'Volumes persist independently of containers. Use docker volume inspect <volume> to see location.'
      }
    ],
    resourcesAllowed: [
      'docker run --help',
  ...
  runIndependent: {
    objective: 'Deploy a complete 3-tier application stack using Docker: frontend, backend API, and database',
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 30,
        passingThreshold: 'All three tiers running in separate containers. Proper separation of concerns. Containers use official base images.'
      },
      {
        criterion: 'Networking & Communication',
        weight: 25,
        passingThreshold: 'Custom Docker network created. Containers communicate by name. Proper port exposure for external access. No hardcoded IPs.'
      },
      {
        criterion: 'Data Persistence',
        weight: 25,
        passingThreshold: 'Named volumes for database and any user uploads. Data survives container removal. Volume configuration documented.'
      },
      {
        criterion: 'Documentation & Reproducibility',
        weight: 20,
        passingThreshold: 'README with clear start/stop instructions. All commands documented. Can rebuild stack from scratch following docs.'
      }
    ]
  },
