...
export const week5Lesson3BuildingPipelines: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description: 'Design and implement end-to-end CI/CD pipelines with testing, deployment, and monitoring.',
    learningObjectives: [
  ...
  crawl: {
    introduction: 'Build complete CI/CD pipeline step-by-step: quality gates, testing stages, deployment automation, monitoring, and rollback procedures.',
    steps: [
      {
        stepNumber: 1,
        ...
        explanation: 'Pipeline has clear stages. Each stage validates code quality. Quality gates prevent bad code from advancing. Staging mirrors production. Manual approval for production. Monitoring provides feedback.',
        expectedOutput: 'Understanding: Complete pipeline architecture from code commit to production monitoring.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'cat > .github/workflows/pipeline.yml << \'EOF\'\nname: CI/CD Pipeline\n\non:\n  push:\n    branches: [main, develop]\n  pull_request:\n    branches: [main]\n\njobs:\n  lint:\n    name: Code Quality\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      \n      - uses: actions/setup-node@v4\n        with:\n          node-version: \'20\'\n          cache: \'npm\'\n      \n      - run: npm ci\n      \n      - name: ESLint\n        run: npm run lint\n      \n      - name: Prettier\n        run: npm run format:check\nEOF',
        explanation: 'Linting runs first (fastest feedback). ESLint checks code quality. Prettier validates formatting. Fails fast if code doesn\'t meet standards. Forces developers to fix style issues before tests.',
        expectedOutput: 'Linting stage catches code style issues immediately, before running slower tests.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Test stage runs after lint passes (needs: lint). Unit tests run first (fast). Integration tests validate components work together. Coverage uploaded to Codecov for tracking. Gates prevent untested code from deploying.',
        expectedOutput: 'Comprehensive testing with coverage reporting, preventing untested code from reaching production.',
        validationCriteria: [
          'Unit tests execute',
      ...
      },
      {
        ...
        explanation: 'Security scans run in parallel with tests (both need: lint). Snyk checks for vulnerabilities in dependencies. Dependency Review checks new dependencies in PRs. Catches security issues before deployment.',
        expectedOutput: 'Automated security scanning catches vulnerabilities before they reach production.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Build runs after tests and security pass. metadata-action generates semantic tags (branch name, git SHA). Docker layer caching speeds up builds. Image pushed to registry for deployment stages. Output makes tag available to deploy jobs.',
        expectedOutput: 'Docker image built, tagged semantically, pushed to registry, ready for deployment.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Staging deployment runs after successful build. Only on main branch (if condition). Environment tracks deployment history. Health check validates deployment worked. Fails if staging is broken.',
        expectedOutput: 'Automatic deployment to staging with health check validation.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Smoke tests validate critical user workflows work on staging. Performance check ensures app is responsive. Catches issues before production. Fails pipeline if staging broken.',
        expectedOutput: 'Automated smoke tests validate staging deployment before production gate.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Production deployment requires manual approval (environment: production configured in GitHub settings). Blue-green strategy enables zero-downtime deployment. Health checks validate production. Old version kept for instant rollback.',
        expectedOutput: 'Production deployment with manual approval gate and blue-green zero-downtime strategy.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Notifications run always (even on failure). Slack message shows deployment status. Team knows immediately if deployment succeeded or failed. Can integrate with PagerDuty, email, etc.',
        expectedOutput: 'Automatic notifications keep team informed of deployment status.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'cat > .github/workflows/rollback.yml << \'EOF\'\nname: Rollback Production\n\non:\n  workflow_dispatch:\n    inputs:\n      version:\n        description: \'Version to rollback to\'\n        required: true\n\njobs:\n  rollback:\n    runs-on: ubuntu-latest\n    environment:\n      name: production\n    steps:\n      - name: Confirm rollback\n        run: |\n          echo "Rolling back to version: ${{ github.event.inputs.version }}"\n      \n      - name: Deploy previous version\n        run: |\n          # Pull previous image from registry\n          # Deploy to production\n          # Verify health checks\n      \n      - name: Notify rollback\n        uses: 8398a7/action-slack@v3\n        with:\n          status: \'warning\'\n          text: |\n            ROLLBACK executed to version ${{ github.event.inputs.version }}\n            By: ${{ github.actor }}\n          webhook_url: ${{ secrets.SLACK_WEBHOOK }}\nEOF',
        explanation: 'Separate rollback workflow triggered manually. Input allows specifying version to rollback to. Requires production environment approval. Notifies team of rollback. Critical safety mechanism.',
        expectedOutput: 'Manual rollback workflow enables quick recovery from bad deployments.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Pipeline metrics to track:\n\n1. DEPLOYMENT FREQUENCY:\n   - Count: Number of deployments per week\n   - Target: Daily deployments\n   \n2. LEAD TIME:\n   - Measure: Commit to production time\n   - Target: < 1 hour\n   \n3. PIPELINE DURATION:\n   - Measure: Time from start to finish\n   - Target: < 10 minutes\n   \n4. FAILURE RATE:\n   - Calculate: Failed pipelines / Total pipelines\n   - Target: < 10%\n   \n5. MTTR (Mean Time To Recovery):\n   - Measure: Time to rollback/fix broken deployment\n   - Target: < 30 minutes\n\nView in GitHub Actions Insights',
        explanation: 'Track pipeline performance with metrics. GitHub Actions Insights shows success rate, duration, trends. Use data to improve pipeline. Slow pipelines get optimized. High failure rates get investigated.',
        expectedOutput: 'Understanding: Pipeline metrics measure effectiveness and guide improvements.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Systematic debugging approach. Logs show exact failure point. Local reproduction isolates issues. Debug logging reveals hidden problems. Act allows testing workflows locally before pushing.',
        expectedOutput: 'Understanding: Systematic approach to troubleshooting pipeline failures.',
        validationCriteria: [
  ...
  walk: {
    ...
    exercises: [
      ...
      },
      {
        ...
        blanks: [
          ...
          },
          {
            ...
            correctValue: 'kubectl set image deployment/app app=myimage:$VERSION',
        ...
        solution: 'deploy-prod:\n  needs: [smoke-tests]\n  runs-on: ubuntu-latest\n  environment:\n    name: production\n  steps:\n    - name: Deploy\n      run: |\n        # Deploy new version\n        kubectl set image deployment/app app=myimage:$VERSION\n    \n    - name: Health check\n      run: |\n        for i in {1..5}; do\n          if curl -f https://myapp.com/health; then\n            exit 0\n          fi\n          sleep 10\n        done\n        echo "Health check failed"\n        # kubectl rollout undo deployment/app\n        exit 1',
        explanation: 'Production deployment with retry logic on health checks and rollback capability on failure.'
      },
      {
        exerciseNumber: 3,
        ...
        explanation: 'Tests and security run in parallel after lint passes, build waits for both to complete.'
      },
      {
        exerciseNumber: 4,
      ...
      },
      {
        ...
        solution: 'Pipeline metrics:\n\nCurrent state:\n- Total duration: 15 minutes\n- Lint: 1 min\n- Tests (sequential): 8 min\n- Build: 3 min\n- Deploy: 3 min\n\nOptimized (parallel tests):\n- Total duration: 10 minutes\n- Lint: 1 min\n- Tests (parallel): 5 min\n- Build: 2 min (with cache)\n- Deploy: 2 min\n\nImprovement: 33% faster',
        explanation: 'Parallel execution and caching significantly reduce pipeline duration, enabling faster feedback.'
      }
    ],
    hints: [
  ...
  runGuided: {
    objective: 'Build production-ready CI/CD pipeline with all stages, quality gates, deployment automation, and monitoring',
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck: 'Create separate jobs: lint (ESLint/Prettier), test (jest/pytest with coverage), security (Snyk/npm audit). Use needs to enforce order: test needs lint, build needs [test, security].'
      },
      {
        ...
        hintIfStuck: 'Create deploy jobs with environment: staging/production. Staging auto-deploys on main. Production needs environment protection rules (Settings → Environments → Add protection). Add health check steps after deployment. Test rollback workflow manually.'
      },
      {
        ...
        hintIfStuck: 'Add Slack notification job with if: always(). Check Actions Insights for metrics. Document in README: how to trigger manually, how to approve production, how to rollback, common errors and fixes. Add workflow status badge from Actions UI.'
      }
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective: 'Build enterprise-grade CI/CD pipeline with comprehensive quality gates, security scanning, multi-region deployment, monitoring, and complete operational documentation',
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 25,
        passingThreshold: 'All stages present and properly ordered. Quality gates fail fast. Independent stages parallelized. Pipeline completes in <10 minutes. Clear separation of concerns. Production-ready configuration.'
      },
      {
        criterion: 'Quality and Security',
        weight: 25,
        passingThreshold: 'Comprehensive testing (unit, integration, smoke) with >80% coverage. Security scanning integrated (dependencies, containers, secrets). Linting enforced. Only high-quality, secure code reaches production. Vulnerabilities block deployment.'
      },
      {
        criterion: 'Deployment Excellence',
        weight: 30,
        passingThreshold: 'Multi-environment deployment (dev, staging, prod) with proper gates. Zero-downtime deployment strategy. Health checks validate deployments. Automatic rollback on failure. Manual rollback tested and fast (<5 min). Deployment history tracked.'
      },
      {
        criterion: 'Operations and Maintainability',
        weight: 20,
        passingThreshold: 'Comprehensive monitoring and notifications. Pipeline metrics tracked. Troubleshooting guide helps diagnose issues. Team can operate independently. Documentation is excellent. Rollback procedures are clear and tested.'
      }
    ]
  },
