...
export const week4Lesson1WhyContainers: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description: 'Learn why containers revolutionized software deployment and how they solve critical infrastructure challenges.',
    learningObjectives: [
  ...
  crawl: {
    introduction: 'Learn why containers exist by understanding the problems they solve: dependency conflicts, environment inconsistencies, and deployment complexity.',
    steps: [
      {
        stepNumber: 1,
        instruction: 'The problem: "Works on my machine" syndrome',
        command: 'Developer: "The app works fine on my laptop!"\nProduction: App crashes with missing dependencies, wrong versions, configuration issues',
        explanation: 'Classic problem: Code works in development but fails in production. Different OS versions, library versions, environment variables. Hard to debug. Frustrating for everyone.',
        expectedOutput: 'Understanding: Environment differences cause production failures even when code is identical.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Traditional servers run multiple apps. Apps need different language versions, libraries. Installing conflicting versions breaks things. Workarounds (virtualenv, nvm) are complex.',
        expectedOutput: 'Understanding: Shared servers create dependency conflicts that are hard to manage.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Container = Package containing:\n- Application code\n- Dependencies (exact versions)\n- Runtime (Python, Node, etc.)\n- OS libraries\n- Configuration\nAll bundled together, isolated from other containers',
        explanation: 'Container packages everything app needs. Runs in isolation. No conflicts with other containers. Same container runs everywhere (dev, test, prod). Consistent environment.',
        expectedOutput: 'Understanding: Containers solve dependency hell through isolation and bundling.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'VM: Full OS + app (GBs, minutes to start)\n  - Hypervisor virtualizes hardware\n  - Each VM runs complete OS (kernel, drivers, services)\n  - Isolated at hardware level\n\nContainer: App + libraries only (MBs, seconds to start)\n  - Docker Engine shares host OS kernel\n  - Containers include only app dependencies\n  - Isolated at process level',
        explanation: 'VMs virtualize hardware. Heavy (each has full OS). Containers virtualize OS. Lightweight (share kernel). VMs = minutes to boot, GBs. Containers = seconds to start, MBs.',
        expectedOutput: 'Understanding: Containers are lighter and faster than VMs by sharing the OS kernel.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Traditional: Install dependencies on each server (manual, error-prone)\nContainers: Build image once, run on any Docker host (automated, consistent)\n\nDev laptop → Test server → AWS → Azure → On-premises\nSame container image runs everywhere',
        explanation: 'Container images are portable. Build once, deploy everywhere. No "works on my machine" because it\'s the same image. Docker provides consistent runtime across cloud providers.',
        expectedOutput: 'Understanding: Container images ensure consistency across all environments.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Traditional: Provision new server (hours/days), install dependencies (hours), configure (hours)\nContainers: docker run (seconds), already configured\n\nTraffic spike → Launch 10 more containers in 30 seconds\nTraffic drops → Stop containers, save money',
        explanation: 'Containers start instantly. No server provisioning, dependency installation. Scale horizontally by running more containers. Orchestrators (Kubernetes) automate scaling.',
        expectedOutput: 'Understanding: Containers enable rapid scaling to handle variable load.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Monolith: One large app, all services bundled, deploy everything together\nMicroservices: Many small services, each in own container, deploy independently\n\nUpdate payment service → Deploy only payment container\nOther services (auth, catalog, shipping) unaffected',
        explanation: 'Containers perfect for microservices. Each service in own container. Deploy independently. Different languages/versions okay. Failure isolation (one service crashes, others continue).',
        expectedOutput: 'Understanding: Containers enable microservices architecture through isolation and independent deployment.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Code versioning: Git commits, branches, tags\nInfrastructure versioning: Docker image tags\n\nmyapp:1.0.0 → myapp:1.1.0 → myapp:2.0.0\n\nRollback: Redeploy myapp:1.1.0 if myapp:2.0.0 has bugs',
        explanation: 'Container images are versioned artifacts. Tag with version numbers. Store in registry (Docker Hub, AWS ECR). Easy rollback to previous version. Immutable (image doesn\'t change).',
        expectedOutput: 'Understanding: Docker images enable infrastructure versioning and easy rollbacks.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Containers integrate perfectly with CI/CD. Build image in pipeline. Test in container (production-like environment). Deploy by running container. Same image from dev to prod.',
        expectedOutput: 'Understanding: Containers enable consistent CI/CD pipelines with production-like testing.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Containers aren\'t universal solution. Legacy monoliths hard to containerize. Apps needing specific kernel versions problematic. Security-critical apps might need VM isolation. Know trade-offs.',
        expectedOutput: 'Understanding: Containers solve many problems but aren\'t always the best solution.',
        validationCriteria: [
    ...
    expectedOutcome: 'You understand problems containers solve: dependency conflicts, environment inconsistencies, deployment complexity. You can explain containers vs VMs, portability benefits, scalability advantages, microservices enablement, and when containers are appropriate.'
  },

  walk: {
    introduction: 'Apply container concepts through scenario-based decision-making exercises.',
    exercises: [
      {
        exerciseNumber: 1,
        scenario: 'Your team struggles with "works on my machine" problems. Explain the root cause and container solution.',
        template: 'Problem: __ENVIRONMENT_DIFFERENCES__ between dev and production\nDev has: Different __OS_VERSIONS__, __LIBRARY_VERSIONS__, __CONFIGURATIONS__\nContainer solution: Package app with __ALL_DEPENDENCIES__ in __IMAGE__\nResult: __SAME_ENVIRONMENT__ everywhere (dev, test, prod)',
        blanks: [
          {
      ...
      },
      {
        ...
        explanation: 'Containers are ideal for microservices due to lightweight nature and fast startup.'
      },
      {
        exerciseNumber: 3,
        scenario: 'Your app needs to scale during traffic spikes. How do containers help?',
        template: 'Traffic spike detected:\n1. __ORCHESTRATOR__ (Kubernetes) notices load\n2. Launches __NEW_CONTAINERS__ in __SECONDS__\n3. __LOAD_BALANCER__ distributes traffic\n4. Traffic drops → __TERMINATE__ containers\n5. Only pay for __ACTUAL_USAGE__',
        blanks: [
          {
            id: 'ORCHESTRATOR',
            label: 'ORCHESTRATOR',
      ...
      },
      {
        ...
        explanation: 'Containers ensure the tested artifact is exactly what runs in production.'
      }
    ],
    hints: [
  ...
  runGuided: {
    objective: 'Analyze a real application and create migration plan from traditional deployment to containers',
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck: 'Use real app from work or create scenario: "Node.js API + PostgreSQL + Redis, deployed manually to Ubuntu server, takes 2 hours to deploy, dev uses Mac, prod uses Linux, constant environment mismatches."'
      },
      {
        ...
        hintIfStuck: 'Separate concerns: API in one container, database in another. Use official images from Docker Hub (node, postgres, redis). Define network for inter-container communication.'
      },
      {
        ...
        hintIfStuck: 'Incremental: 1) Containerize API first (stateless), test thoroughly. 2) Move Redis (cache, can lose data). 3) Finally PostgreSQL (stateful, backup first). Run old and new in parallel during transition.'
      }
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective: 'Create comprehensive containerization proposal including problem analysis, architecture design, migration plan, cost-benefit analysis, and risk assessment',
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 25,
        passingThreshold: 'Clearly articulates current deployment problems with specific examples. Quantifies pain (hours spent, failure rate, manual steps). Connects problems to business impact.'
      },
      {
        criterion: 'Solution Design',
        weight: 30,
        passingThreshold: 'Container architecture solves identified problems. Appropriate separation of concerns. Realistic dependencies and configuration. Considers persistence, networking, and scaling.'
      },
      {
        criterion: 'Migration Planning',
        weight: 25,
        passingThreshold: 'Incremental migration approach. Clear testing strategy. Risk mitigation for each phase. Rollback procedures defined. Realistic timeline with milestones.'
      },
      {
        criterion: 'Business Case',
        weight: 20,
        passingThreshold: 'Cost-benefit analysis includes time savings, reliability improvements, and potential costs. Success metrics measurable. Executive summary understandable to non-technical stakeholders.'
      }
    ]
  },
