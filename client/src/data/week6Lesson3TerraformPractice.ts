...
export const week6Lesson3TerraformPractice: LeveledLessonContent = {
  ...
  baseLesson: {
    title: 'Terraform in Practice: Advanced Patterns and Production Best Practices',
    description: 'Master advanced Terraform: modules, workspaces, state manipulation, and production-ready patterns.',
    learningObjectives: [
  ...
  crawl: {
    introduction: 'Learn advanced Terraform: creating modules, using workspaces, state operations, remote execution, CI/CD integration, and production best practices.',
    steps: [
      {
        stepNumber: 1,
        instruction: 'Create reusable Terraform module',
        command: 'mkdir -p modules/web-server\n\n# modules/web-server/main.tf\ncat > modules/web-server/main.tf << \'EOF\'\nresource "aws_security_group" "web" {\n  name_prefix = "${var.name_prefix}-web-"\n  vpc_id      = var.vpc_id\n\n  ingress {\n    from_port   = 80\n    to_port     = 80\n    protocol    = "tcp"\n    cidr_blocks = ["0.0.0.0/0"]\n  }\n\n  egress {\n    from_port   = 0\n    to_port     = 0\n    protocol    = "-1"\n    cidr_blocks = ["0.0.0.0/0"]\n  }\n}\n\nresource "aws_instance" "web" {\n  count                  = var.instance_count\n  ami                    = var.ami\n  instance_type          = var.instance_type\n  vpc_security_group_ids = [aws_security_group.web.id]\n  subnet_id              = var.subnet_id\n\n  tags = merge(var.tags, {\n    Name = "${var.name_prefix}-web-${count.index + 1}"\n  })\n}\nEOF',
        explanation: 'Module = reusable infrastructure component. Define in modules/ directory. Uses input variables for customization. Can be used multiple times with different values. Encapsulates best practices.',
        expectedOutput: 'Web server module created with security group and EC2 instances.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: '# modules/web-server/variables.tf\ncat > modules/web-server/variables.tf << \'EOF\'\nvariable "name_prefix" {\n  description = "Prefix for resource names"\n  type        = string\n}\n\nvariable "vpc_id" {\n  description = "VPC ID for resources"\n  type        = string\n}\n\nvariable "subnet_id" {\n  description = "Subnet ID for instances"\n  type        = string\n}\n\nvariable "ami" {\n  description = "AMI for EC2 instances"\n  type        = string\n}\n\nvariable "instance_type" {\n  description = "Instance type"\n  type        = string\n  default     = "t2.micro"\n}\n\nvariable "instance_count" {\n  description = "Number of instances"\n  type        = number\n  default     = 1\n}\n\nvariable "tags" {\n  description = "Additional tags"\n  type        = map(string)\n  default     = {}\n}\nEOF\n\n# modules/web-server/outputs.tf\ncat > modules/web-server/outputs.tf << \'EOF\'\noutput "instance_ids" {\n  description = "IDs of EC2 instances"\n  value       = aws_instance.web[*].id\n}\n\noutput "public_ips" {\n  description = "Public IPs of instances"\n  value       = aws_instance.web[*].public_ip\n}\n\noutput "security_group_id" {\n  description = "ID of security group"\n  value       = aws_security_group.web.id\n}\nEOF',
        explanation: 'Module variables define inputs. Module outputs expose useful values. Consumers provide variables, receive outputs. Clear interface for module usage.',
        expectedOutput: 'Module has complete interface: variables for inputs, outputs for results.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: '# Root main.tf\ncat > main.tf << \'EOF\'\nmodule "dev_web" {\n  source = "./modules/web-server"\n\n  name_prefix    = "dev"\n  vpc_id         = aws_vpc.main.id\n  subnet_id      = aws_subnet.public.id\n  ami            = "ami-0c55b159cbfafe1f0"\n  instance_type  = "t2.micro"\n  instance_count = 1\n\n  tags = {\n    Environment = "dev"\n    ManagedBy   = "terraform"\n  }\n}\n\nmodule "prod_web" {\n  source = "./modules/web-server"\n\n  name_prefix    = "prod"\n  vpc_id         = aws_vpc.main.id\n  subnet_id      = aws_subnet.public.id\n  ami            = "ami-0c55b159cbfafe1f0"\n  instance_type  = "t2.large"\n  instance_count = 3\n\n  tags = {\n    Environment = "production"\n    ManagedBy   = "terraform"\n  }\n}\nEOF',
        explanation: 'Use module{} block to instantiate module. source points to module directory. Provide values for variables. Can use module multiple times with different configs. Promotes consistency.',
        expectedOutput: 'Root configuration uses module twice: dev (1 small instance) and prod (3 large instances).',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Workspaces = multiple state files from same config. Each workspace has isolated state. Use terraform.workspace variable in code. Common for dev/staging/prod. State stored separately per workspace.',
        expectedOutput: 'Multiple workspaces created, can switch between them, state isolated.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: '# Manually created EC2 instance exists: i-0abcdef123456\n\n# 1. Write Terraform config for it\nresource "aws_instance" "imported" {\n  # Will be filled after import\n}\n\n# 2. Import the resource\nterraform import aws_instance.imported i-0abcdef123456\n\n# 3. Check state\nterraform state show aws_instance.imported\n# Shows all attributes\n\n# 4. Update config to match\nresource "aws_instance" "imported" {\n  ami           = "ami-0c55b159cbfafe1f0"\n  instance_type = "t2.micro"\n  # ... other attributes from state\n}\n\n# 5. Verify no changes\nterraform plan\n# No changes. Infrastructure is up-to-date.',
        explanation: 'Import adds existing resources to Terraform state. Write config first (can be minimal). Run import with resource ID. Update config to match actual resource. Brings manual resources under Terraform management.',
        expectedOutput: 'Existing AWS resource imported into Terraform, now managed by code.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: '# Move resource to new name\nterraform state mv aws_instance.old aws_instance.new\n\n# Move resource into module\nterraform state mv aws_instance.web module.web.aws_instance.main\n\n# Remove resource from state (doesn\'t destroy in AWS)\nterraform state rm aws_instance.temp\n\n# List all resources in state\nterraform state list\n\n# Show specific resource\nterraform state show aws_instance.example\n\n# Pull current state\nterraform state pull > terraform.tfstate.backup',
        explanation: 'State commands manipulate state file safely. mv renames resources without recreating. rm removes from Terraform (resource stays in cloud). list/show inspect state. pull backs up state.',
        expectedOutput: 'Understanding: Safe state manipulation for refactoring without destroying resources.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: '# Reference existing VPC not managed by this Terraform\ndata "aws_vpc" "existing" {\n  filter {\n    name   = "tag:Name"\n    values = ["my-existing-vpc"]\n  }\n}\n\n# Use in resource\nresource "aws_subnet" "app" {\n  vpc_id     = data.aws_vpc.existing.id\n  cidr_block = "10.0.1.0/24"\n}\n\n# Reference AMI\ndata "aws_ami" "ubuntu" {\n  most_recent = true\n  owners      = ["099720109477"]  # Canonical\n\n  filter {\n    name   = "name"\n    values = ["ubuntu/images/hvm-ssd/ubuntu-focal-20.04-amd64-server-*"]\n  }\n}\n\nresource "aws_instance" "web" {\n  ami = data.aws_ami.ubuntu.id\n}',
        explanation: 'Data sources read existing infrastructure not managed by Terraform. Query by filters, tags, IDs. Use results in resources. Enables integration with existing infrastructure.',
        expectedOutput: 'Data sources reference existing VPC and latest Ubuntu AMI.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: '# .github/workflows/terraform.yml\nname: Terraform\n\non:\n  pull_request:\n    paths: [\'infrastructure/**\']\n  push:\n    branches: [main]\n\njobs:\n  terraform:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      \n      - name: Setup Terraform\n        uses: hashicorp/setup-terraform@v3\n        with:\n          terraform_version: 1.6.0\n      \n      - name: Terraform Init\n        run: terraform init\n        env:\n          AWS_ACCESS_KEY_ID: ${{ secrets.AWS_ACCESS_KEY_ID }}\n          AWS_SECRET_ACCESS_KEY: ${{ secrets.AWS_SECRET_ACCESS_KEY }}\n      \n      - name: Terraform Plan\n        run: terraform plan -no-color\n        continue-on-error: true\n      \n      - name: Terraform Apply\n        if: github.ref == \'refs/heads/main\'\n        run: terraform apply -auto-approve\n        env:\n          AWS_ACCESS_KEY_ID: ${{ secrets.AWS_ACCESS_KEY_ID }}\n          AWS_SECRET_ACCESS_KEY: ${{ secrets.AWS_SECRET_ACCESS_KEY }}',
        explanation: 'Terraform in CI/CD: Plan on PRs (review changes), Apply on merge to main (automate deployment). Credentials from secrets. -auto-approve for automation. continue-on-error shows plan even if fails.',
        expectedOutput: 'GitHub Actions workflow automates Terraform plan on PRs, apply on main.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'variable "enable_monitoring" {\n  description = "Enable CloudWatch monitoring"\n  type        = bool\n  default     = false\n}\n\nresource "aws_cloudwatch_metric_alarm" "cpu" {\n  count = var.enable_monitoring ? 1 : 0\n  \n  alarm_name          = "high-cpu"\n  comparison_operator = "GreaterThanThreshold"\n  # ... other alarm config\n}\n\n# Access with [0] if exists\noutput "alarm_arn" {\n  value = var.enable_monitoring ? aws_cloudwatch_metric_alarm.cpu[0].arn : null\n}',
        explanation: 'count = 0 skips resource creation. count = 1 creates one. Use ternary (condition ? true : false) for conditional. Access with [index]. Enables feature flags.',
        expectedOutput: 'Conditional resource creation based on variable value.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'variable "users" {\n  description = "IAM users to create"\n  type        = set(string)\n  default     = ["alice", "bob", "charlie"]\n}\n\nresource "aws_iam_user" "users" {\n  for_each = var.users\n  name     = each.key\n\n  tags = {\n    Department = "Engineering"\n  }\n}\n\n# Access specific user\noutput "alice_arn" {\n  value = aws_iam_user.users["alice"].arn\n}\n\n# All user ARNs\noutput "all_user_arns" {\n  value = { for user in var.users : user => aws_iam_user.users[user].arn }\n}',
        explanation: 'for_each creates resource for each item in set/map. each.key is item. More flexible than count (can add/remove items). Access with ["key"]. Terraform tracks by key, not index.',
        expectedOutput: 'Multiple IAM users created from set, accessible by name.',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation: 'Production Terraform requires: remote state, modular code, environment isolation, security practices, CI/CD automation, code quality tools, and comprehensive documentation. Follow these for reliable infrastructure management.',
        expectedOutput: 'Understanding: Production Terraform best practices for reliability and maintainability.',
        validationCriteria: [
    ...
    expectedOutcome: 'You can create reusable modules, use workspaces for environments, import and manipulate state safely, integrate Terraform in CI/CD, use count/for_each for dynamic resources, and apply production best practices.'
  },

  walk: {
    introduction: 'Practice advanced Terraform through scenario-based exercises.',
    exercises: [
      {
        exerciseNumber: 1,
      ...
      },
      {
        ...
        solution: 'module "database" {\n  source = "./modules/database"\n\n  db_name        = "myapp"\n  instance_class = "db.t2.small"\n}\n\noutput "db_endpoint" {\n  value = module.database.endpoint\n}',
        explanation: 'Use module with source path, provide variables, access outputs with module.name.output.'
      },
      {
        exerciseNumber: 3,
        ...
        solution: 'Commands:\n\n# Create workspaces\nterraform workspace new dev\nterraform workspace new prod\n\n# Switch\nterraform workspace select dev\n\n# Show current\nterraform workspace show\n\n# Use in code\nresource "aws_instance" "web" {\n  instance_type = terraform.workspace == "prod" ? "t2.large" : "t2.micro"\n}',
        explanation: 'Workspace commands: new, select, show. Use terraform.workspace in code for environment-specific logic.'
      },
      {
        exerciseNumber: 4,
        ...
        explanation: 'Import workflow: write config → import with ID → inspect state → update config → verify plan.'
      },
      {
        exerciseNumber: 5,
        ...
        explanation: 'CI/CD: Plan on PRs for review, Apply on main for automation. Pin version, secure credentials.'
      }
    ],
    hints: [
  ...
  runGuided: {
    objective: 'Build production-ready Terraform setup with reusable modules, multi-environment support, CI/CD integration, and comprehensive documentation',
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck: 'Start with network module (VPC, subnets). Then compute (EC2, security groups). Then database (RDS). Each should take inputs, create resources, expose outputs. Use in dev first, then copy to staging/prod with different variables.'
      },
      {
        ...
        hintIfStuck: 'Use separate directories (env/dev/, env/staging/, env/prod/) with separate tfvars files. Each references same modules. Configure separate S3 state paths. Deploy dev first, then staging, then prod.'
      },
      {
        ...
        hintIfStuck: 'Create .github/workflows/terraform.yml. Add jobs for fmt, validate, tfsec, plan, apply. Plan on PR, apply on main with conditional (workspace or directory-based). Store AWS credentials in GitHub secrets.'
      }
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective: 'Build enterprise-grade Terraform infrastructure with modular architecture, advanced state management, complete CI/CD automation, security hardening, and production operational readiness',
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 30,
        passingThreshold: 'Comprehensive module library with clear abstractions. DRY principles applied. Modules are composable and versioned. Architecture supports scalability and maintainability. Separation of concerns is clear.'
      },
      {
        criterion: 'Operational Readiness',
        weight: 25,
        passingThreshold: 'Complete CI/CD automation with appropriate gates. Multi-environment deployment works flawlessly. State management is bulletproof with backups. Drift detection configured. Monitoring provides visibility. Team can operate independently.'
      },
      {
        criterion: 'Security and Compliance',
        weight: 25,
        passingThreshold: 'Security scanning integrated in CI/CD. No secrets in code or state. Encryption everywhere. Least-privilege IAM. Compliance controls (MFA, audit logging). Security best practices documented and enforced.'
      },
      {
        criterion: 'Quality and Documentation',
        weight: 20,
        passingThreshold: 'Code quality tools enforced. Comprehensive testing. Auto-generated documentation. Architecture diagrams clear. Runbooks enable team operations. Troubleshooting guides resolve common issues. Knowledge is transferable.'
      }
    ]
  },
