...
export const week9Lesson3InfrastructureSecurity: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description: 'Implement infrastructure security controls including IAM, compliance as code, security auditing, and automated compliance verification.',
    learningObjectives: [
  ...
  crawl: {
    introduction: 'Learn infrastructure security and compliance step-by-step. You will design IAM policies, implement compliance checks, and audit security posture.',
    expectedOutcome: 'Complete understanding of infrastructure security: IAM least privilege, policy as code with OPA, IaC scanning with Checkov, AWS Config compliance, Security Hub monitoring, and automated auditing',
    steps: [
      {
        stepNumber: 1,
        instruction: 'Understand the principle of least privilege',
        command: 'echo "Least Privilege IAM:\n1. Grant minimum permissions needed\n2. Use specific resources (no wildcards *)\n3. Add conditions to restrict access\n4. Regular access reviews\n5. Temporary credentials only"',
        explanation: 'Least privilege minimizes damage if credentials are compromised. Start with no access, add only what\'s needed.',
        ...
        commonMistakes: ['Granting AdministratorAccess', 'Using wildcard (*) resources']
      },
      {
        stepNumber: 2,
        instruction: 'Create IAM policy with specific permissions',
        command: 'aws iam create-policy --policy-name S3ReadOnlySpecific --policy-document \'{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Action":["s3:GetObject","s3:ListBucket"],"Resource":["arn:aws:s3:::my-app-data","arn:aws:s3:::my-app-data/*"],"Condition":{"IpAddress":{"aws:SourceIp":"10.0.0.0/16"}}}]}\'',
        explanation: 'Policy grants only GetObject and ListBucket (not PutObject or DeleteObject), scoped to specific bucket, restricted to VPC IP range.',
      ...
      {
        stepNumber: 3,
        instruction: 'Set up AWS IAM Identity Center (SSO) for centralized access',
        command: 'aws sso-admin list-instances --query "Instances[0].InstanceArn" --output text',
        explanation: 'IAM Identity Center provides SSO with MFA, centralized access management, temporary credentials. Replaces long-lived IAM user keys.',
      ...
      },
      {
        ...
        command: 'aws cloudtrail create-trail --name security-audit --s3-bucket-name my-cloudtrail-logs --is-multi-region-trail --enable-log-file-validation',
        explanation: 'CloudTrail records all API calls for audit, forensics, compliance. Log file validation prevents tampering.',
      ...
      },
      {
        ...
        command: 'docker run --rm --net host --pid host --cap-add audit_control -v /etc:/node/etc:ro -v /var:/node/var:ro aquasec/kube-bench:latest',
        explanation: 'kube-bench checks Kubernetes against CIS benchmark security controls. Identifies misconfigurations.',
      ...
      },
      {
        ...
        command: 'kubectl apply -f https://raw.githubusercontent.com/open-policy-agent/gatekeeper/master/deploy/gatekeeper.yaml',
        explanation: 'OPA Gatekeeper enforces policies as admission controller. Blocks non-compliant resources before creation.',
        ...
        commonMistakes: ['Not testing policies', 'Blocking all resources accidentally']
      },
      {
        stepNumber: 7,
        ...
        explanation: 'ConstraintTemplate defines policy logic in Rego. Constraint applies policy to specific resources. Blocks pods without memory limits.',
      ...
      },
      {
        ...
        explanation: 'Checkov scans IaC for misconfigurations: public S3 buckets, unencrypted EBS, overly permissive security groups.',
        ...
        commonMistakes: ['Not scanning before terraform apply', 'Ignoring medium-severity issues']
      },
      {
        stepNumber: 9,
        instruction: 'Scan Terraform with Conftest for policy compliance',
        command: 'conftest test terraform.tfplan --policy policy/',
        explanation: 'Conftest uses OPA to enforce custom policies on IaC. Example: require encryption, deny public access, enforce tagging.',
        ...
        commonMistakes: ['No policies defined', 'Policies not version controlled']
      },
      {
        stepNumber: 10,
        instruction: 'Enable AWS Security Hub for centralized security findings',
        command: 'aws securityhub enable-security-hub --enable-default-standards',
        explanation: 'Security Hub aggregates findings from GuardDuty, Inspector, Config, Macie. Provides compliance dashboards.',
        ...
        commonMistakes: ['Not enabling GuardDuty first', 'Ignoring critical findings']
      },
      {
        stepNumber: 11,
        instruction: 'Configure AWS Config rules for compliance monitoring',
        command: 'aws configservice put-config-rule --config-rule \'{"ConfigRuleName":"s3-bucket-public-read-prohibited","Source":{"Owner":"AWS","SourceIdentifier":"S3_BUCKET_PUBLIC_READ_PROHIBITED"}}\'',
        explanation: 'AWS Config continuously monitors resources for compliance. Rules auto-remediate or alert on violations.',
      ...
      },
      {
        ...
        command: 'echo "Compliance Dashboard:\n- CIS Benchmark score: 95%\n- Config rules passing: 87/90\n- Security Hub critical findings: 2\n- Last audit: 2025-12-10\n- Next review: 2025-12-17"',
        explanation: 'Compliance dashboard provides visibility into security posture. Track trends, demonstrate compliance to auditors.',
  ...
  walk: {
    introduction: 'Apply infrastructure security and compliance through hands-on exercises.',
    exercises: [
      {
        exerciseNumber: 1,
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:s3:::my-bucket/__ADMIN__/*"
        ...
        solution: 'Least privilege policy: read-only (GetObject, ListBucket), scoped to specific bucket and uploads/* prefix, IP-restricted to VPC, explicit deny for admin/* paths. Never use s3:* or wildcard resources.',
        explanation: 'IAM least privilege minimizes blast radius if credentials compromised'
      },
      {
        exerciseNumber: 2,
        scenario: 'Create OPA Gatekeeper policy requiring pod security standards.',
  ...
  match:
    kinds:
      - apiGroups: [""]
        ...
        solution: 'OPA Gatekeeper policy enforces Pod Security Standards: runAsNonRoot=true, allowPrivilegeEscalation=false. ConstraintTemplate defines Rego logic, Constraint applies to pods. Non-compliant pods are rejected.',
...
checkov -d . --framework terraform --__COMPACT__ --quiet --check __CRITICAL__,HIGH`,
        ...
        solution: 'Checkov scans IaC for security misconfigurations: unencrypted resources, public access, missing logging. Integrate into CI/CD to prevent insecure infrastructure. Use --skip-check for accepted risks with justification.',
        explanation: 'Infrastructure scanning catches security issues before deployment'
      },
      {
        exerciseNumber: 4,
...
aws securityhub create-insight --name "Public S3 Buckets" --filters '{"Type":[{"Value":"Software and Configuration Checks/AWS Security Best Practices","Comparison":"EQUALS"}],"ResourceType":[{"Value":"__AWS_S3_BUCKET__","Comparison":"EQUALS"}]}'`,
        ...
        solution: 'Security Hub aggregates findings from GuardDuty, Inspector, Config, Macie. Enable compliance standards (CIS, PCI-DSS). Filter by severity to prioritize remediation. Create custom insights for tracking specific issues.',
        explanation: 'Centralized security monitoring provides comprehensive visibility'
      },
      {
        exerciseNumber: 5,
...
aws ec2 copy-snapshot --source-snapshot-id snap-abc123 --encrypted`,
        ...
        solution: 'AWS Config rules continuously evaluate resources for compliance. Create rules for encryption, public access, tagging. Query non-compliant resources. Set up auto-remediation with SSM or Lambda for automatic fixes.',
        explanation: 'Automated compliance monitoring ensures continuous security posture'
      }
    ],
    hints: [
  ...
  runGuided: {
    objective: 'Build comprehensive infrastructure security and compliance platform with IAM, policy enforcement, and automated auditing',
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck: 'Enable AWS IAM Identity Center. Create permission sets with specific policies. Require MFA. Delete IAM user access keys. Use IRSA for pods, instance profiles for EC2.'
      },
      {
        checkpoint: 'Policy as code enforcing compliance',
        description: 'Automated policy enforcement blocks non-compliant resources',
        validationCriteria: [
        ...
        hintIfStuck: 'Install Gatekeeper. Create ConstraintTemplates for pod security, resource requirements, allowed registries. Add Checkov to GitHub Actions. Require policies to pass before terraform apply.'
      },
      {
        ...
        hintIfStuck: 'Enable AWS Config with managed rules (encryption, public access). Run kube-bench for Kubernetes. Enable Security Hub with CIS standard. Create CloudWatch dashboard. Set up SNS alerts for critical findings.'
      }
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective: 'Build production-ready infrastructure security platform with comprehensive IAM, automated compliance enforcement, continuous monitoring, and incident response',
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 25,
        passingThreshold: 'SSO with MFA mandatory. All policies use least privilege (specific resources, minimal actions, conditions). No long-lived keys. Roles for service authentication. Access reviews documented.'
      },
      {
        criterion: 'Policy Enforcement',
        weight: 30,
        passingThreshold: 'OPA/Kyverno policies deployed enforcing security standards. IaC scanned in CI/CD (Checkov/Conftest). Non-compliant resources blocked. Policies tested and version controlled.'
      },
      {
        criterion: 'Compliance Monitoring',
        weight: 25,
        passingThreshold: 'Config rules monitoring compliance. CIS benchmarks tracked. Security Hub aggregating findings. Dashboards show current posture. Alerts on violations. Automated remediation for common issues.'
      },
      {
        criterion: 'Security Operations',
        weight: 20,
        passingThreshold: 'CloudTrail logging all actions. GuardDuty detecting threats. Logs centralized with tamper-proofing. Incident runbooks tested. Forensics capabilities ready. Compliance documented.'
      }
    ]
  },
