...
export const week10Lesson2CloudNativeStorage: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description: 'Deploy stateful applications with StatefulSets, manage persistent storage, and operate databases using Kubernetes operators.',
    learningObjectives: [
  ...
  crawl: {
    introduction: 'Learn stateful applications and persistent storage step-by-step. You will deploy StatefulSets, configure storage, and run databases.',
    expectedOutcome: 'Complete understanding of StatefulSets, PersistentVolumes, StorageClasses, database operators, and stateful application management in Kubernetes',
    steps: [
      {
        stepNumber: 1,
        instruction: 'Understand StatefulSet vs Deployment differences',
        command: 'echo "StatefulSet vs Deployment:\n1. Stable pod names (pod-0, pod-1) vs random names\n2. Ordered deployment/scaling vs parallel\n3. Persistent identity across restarts\n4. Stable network hostnames\n5. Per-pod PersistentVolumes\n6. Use for: databases, queues, caches"',
        explanation: 'StatefulSets guarantee pod identity and order. Each pod gets stable hostname, persistent storage. Critical for databases requiring stable network identity.',
        ...
        commonMistakes: ['Using StatefulSet when Deployment sufficient', 'Not understanding ordering']
      },
      {
        stepNumber: 2,
        instruction: 'Create StorageClass for dynamic provisioning',
        command: 'kubectl apply -f - <<EOF\napiVersion: storage.k8s.io/v1\nkind: StorageClass\nmetadata:\n  name: fast-ssd\nprovisioner: kubernetes.io/aws-ebs\nparameters:\n  type: gp3\n  iops: "3000"\n  throughput: "125"\nreclaimPolicy: Retain\nvolumeBindingMode: WaitForFirstConsumer\nallowVolumeExpansion: true\nEOF',
        explanation: 'StorageClass defines how PersistentVolumes are dynamically created. gp3 provides fast SSD storage. WaitForFirstConsumer delays provisioning until pod scheduled (topology-aware).',
        ...
        commonMistakes: ['Wrong provisioner', 'Delete reclaim policy losing data']
      },
      {
        stepNumber: 3,
        instruction: 'Deploy StatefulSet with PersistentVolumeClaims',
        command: 'kubectl apply -f - <<EOF\napiVersion: v1\nkind: Service\nmetadata:\n  name: mysql\nspec:\n  clusterIP: None  # Headless service\n  selector:\n    app: mysql\n  ports:\n  - port: 3306\n---\napiVersion: apps/v1\nkind: StatefulSet\nmetadata:\n  name: mysql\nspec:\n  serviceName: mysql\n  replicas: 3\n  selector:\n    matchLabels:\n      app: mysql\n  template:\n    metadata:\n      labels:\n        app: mysql\n    spec:\n      containers:\n      - name: mysql\n        image: mysql:8.0\n        ports:\n        - containerPort: 3306\n        volumeMounts:\n        - name: data\n          mountPath: /var/lib/mysql\n        env:\n        - name: MYSQL_ROOT_PASSWORD\n          valueFrom:\n            secretKeyRef:\n              name: mysql-secret\n              key: password\n  volumeClaimTemplates:\n  - metadata:\n      name: data\n    spec:\n      accessModes: [ReadWriteOnce]\n      storageClassName: fast-ssd\n      resources:\n        requests:\n          storage: 10Gi\nEOF',
        explanation: 'StatefulSet creates 3 MySQL pods: mysql-0, mysql-1, mysql-2. Each gets persistent 10Gi volume. Headless service provides stable DNS: mysql-0.mysql, mysql-1.mysql.',
      ...
      },
      {
        ...
        command: 'kubectl get pods -l app=mysql && kubectl get pvc && kubectl exec mysql-0 -- hostname && kubectl delete pod mysql-0 && sleep 30 && kubectl exec mysql-0 -- hostname',
        explanation: 'Each pod gets stable name and hostname. Deleting mysql-0 recreates it with same name, same PVC, same data. Identity persists across restarts.',
        ...
        commonMistakes: ['Expecting random pod names', 'Not waiting for recreation']
      },
      {
        stepNumber: 5,
        instruction: 'Install CloudNativePG operator for PostgreSQL',
        command: 'kubectl apply -f https://raw.githubusercontent.com/cloudnative-pg/cloudnative-pg/release-1.21/releases/cnpg-1.21.0.yaml',
        explanation: 'CloudNativePG operator manages PostgreSQL clusters on Kubernetes. Handles replication, failover, backups, monitoring without manual intervention.',
      ...
      },
      {
        ...
        explanation: 'Operator creates 3-node PostgreSQL cluster with automatic primary election, streaming replication, Prometheus monitoring. Handles failover automatically.',
      ...
      },
      {
        ...
        command: 'kubectl get cluster pg-cluster -o jsonpath=\'{.status.writeService}\' && kubectl exec -it pg-cluster-1 -- psql -U myapp myapp -c "CREATE TABLE test (id serial PRIMARY KEY, data text);"',
        explanation: 'Operator creates read-write service (pg-cluster-rw) for primary, read-only service (pg-cluster-ro) for replicas. Write to primary, read from any.',
        ...
        commonMistakes: ['Connecting to read-only service', 'Wrong credentials']
      },
      {
        stepNumber: 8,
        instruction: 'Configure automated backups',
        command: 'kubectl apply -f - <<EOF\napiVersion: v1\nkind: Secret\nmetadata:\n  name: backup-secret\nstringData:\n  ACCESS_KEY_ID: AKIA...\n  ACCESS_SECRET_KEY: ...\n---\napiVersion: postgresql.cnpg.io/v1\nkind: Cluster\nmetadata:\n  name: pg-cluster\nspec:\n  instances: 3\n  storage:\n    size: 20Gi\n  backup:\n    barmanObjectStore:\n      destinationPath: s3://my-backups/pg-cluster\n      s3Credentials:\n        accessKeyId:\n          name: backup-secret\n          key: ACCESS_KEY_ID\n        secretAccessKey:\n          name: backup-secret\n          key: ACCESS_SECRET_KEY\n    retentionPolicy: "30d"\nEOF',
        explanation: 'Operator uses Barman for continuous archiving to S3. Takes base backups, archives WAL files. Retention policy keeps 30 days of backups.',
      ...
      },
      {
        ...
        command: 'kubectl get cluster pg-cluster -o jsonpath=\'{.status.currentPrimary}\' && kubectl delete pod pg-cluster-1 && sleep 30 && kubectl get cluster pg-cluster -o jsonpath=\'{.status.currentPrimary}\'',
        explanation: 'Operator detects primary failure, promotes replica to primary, updates services. Applications reconnect automatically. Failover typically < 30 seconds.',
        ...
        commonMistakes: ['Not monitoring failover', 'Application not handling reconnect']
      },
      {
        stepNumber: 10,
        ...
        explanation: 'MongoDB operator manages replica sets, sharded clusters, users, backups. Declarative MongoDB management on Kubernetes.',
        expectedOutput: 'Operator installed',
        validationCriteria: [
          'Operator pod running',
          'CRDs available',
      ...
      },
      {
        ...
        explanation: 'Operator creates 3-member MongoDB replica set with SCRAM authentication. Automatic primary election, data synchronization, connection strings updated automatically.',
      ...
      },
      {
        ...
        command: 'kubectl scale statefulset mysql --replicas=5 && kubectl get pods -l app=mysql -w',
        explanation: 'StatefulSet scales by creating mysql-3, then mysql-4. Waits for each pod to be Running before next. Scaling down deletes in reverse order (4, then 3).',
        ...
        commonMistakes: ['Expecting parallel scaling', 'Not waiting for readiness']
      }
    ]
  },

  walk: {
    introduction: 'Apply stateful applications and storage through hands-on exercises.',
    exercises: [
      {
        exerciseNumber: 1,
  ...
  - metadata:
    ...
    spec:
      ...
      resources:
        ...
        solution: 'StatefulSet creates redis-0, redis-1, redis-2 with stable names. Each gets 5Gi PersistentVolume. volumeClaimTemplates create PVC per pod. ReadWriteOnce allows single node attachment.',
        explanation: 'StatefulSets provide stable identity and persistent storage for stateful apps'
      },
      {
        exerciseNumber: 2,
...
allowVolumeExpansion: __TRUE__`,
        ...
        solution: 'StorageClass dynamically provisions gp3 EBS volumes. WaitForFirstConsumer delays provisioning until pod scheduled (zone-aware). Retain policy keeps volumes after PVC deletion. Encrypted for security.',
        explanation: 'StorageClasses enable dynamic provisioning with cloud-specific options'
      },
      {
        exerciseNumber: 3,
...
spec:
  ...
  monitoring:
    enablePodMonitor: __TRUE__`,
        ...
        solution: 'CloudNativePG operator manages 3-instance PostgreSQL cluster. Automatic replication, failover, Prometheus monitoring. Unsupervised updates allow automatic rolling upgrades.',
        explanation: 'Operators automate complex stateful application management'
      },
      {
        exerciseNumber: 4,
...
spec:
  ...
  backup:
    ...
    retentionPolicy: "__30D__"`,
        ...
        solution: 'Barman backs up PostgreSQL to S3 with WAL archiving. Gzip compression saves space. 30-day retention keeps month of backups. Enables point-in-time recovery.',
        explanation: 'Automated backups critical for stateful application disaster recovery'
      },
      {
        exerciseNumber: 5,
...
spec:
  ...
  statefulSet:
    spec:
      volumeClaimTemplates:
        ...
        solution: 'MongoDB operator creates 3-member replica set with SCRAM authentication. Automatic primary election, data replication. Each member gets 10Gi persistent storage.',
        explanation: 'Database operators simplify complex distributed database management'
      }
    ],
    hints: [
  ...
  runGuided: {
    objective: 'Build production stateful application platform with StatefulSets, persistent storage, database operators, and backup automation',
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck: 'Create StorageClass first. Deploy StatefulSet with serviceName and volumeClaimTemplates. Create headless service (clusterIP: None). Verify stable pod names and PVCs.'
      },
      {
        ...
        hintIfStuck: 'Install operator CRDs. Deploy Cluster resource with 3 instances. Verify replication status. Test failover by deleting primary pod. Configure S3 backups with Barman.'
      },
      {
        ...
        hintIfStuck: 'Configure barmanObjectStore in PostgreSQL Cluster spec. Test backup by checking S3 bucket. Restore to new cluster to verify. Monitor backup age with Prometheus alerts.'
      }
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective: 'Build production-grade stateful application platform with comprehensive storage management, database operators, automated backups, disaster recovery, and complete monitoring',
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 25,
        passingThreshold: 'StatefulSets with stable pod identities. PersistentVolumes bound correctly. Headless services configured. Scaling works properly. Data persists across restarts. Resource limits set appropriately.'
      },
      {
        criterion: 'Database Management',
        weight: 30,
        passingThreshold: 'Operators managing PostgreSQL/MongoDB. Multi-replica clusters with replication. Automatic failover tested. Connection pooling configured. Read replicas for scaling. Monitoring active. Performance tuned.'
      },
      {
        criterion: 'Backup & Recovery',
        weight: 25,
        passingThreshold: 'Automated backups to S3. WAL archiving for PostgreSQL. Retention policies enforced. Point-in-time recovery tested. Restore procedures documented. Backup monitoring with alerts. Encryption enabled.'
      },
      {
        criterion: 'Storage & Performance',
        weight: 20,
        passingThreshold: 'Multiple StorageClasses for different tiers. Dynamic provisioning working. Volumes encrypted. Performance appropriate for workload. Storage monitoring active. Expansion tested. No data loss scenarios.'
      }
    ]
  },
