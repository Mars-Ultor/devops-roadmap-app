...
export const week3Lesson2AWSServices: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description: 'Learn essential AWS services for compute, storage, networking, and databases.',
    learningObjectives: [
  ...
  crawl: {
    introduction: 'Learn AWS core services step-by-step: EC2, S3, VPC, RDS, and when to use each.',
    steps: [
      {
        stepNumber: 1,
        instruction: 'EC2 (Elastic Compute Cloud): Virtual servers in the cloud',
        command: 'AWS Console → EC2 → Launch Instance → Choose AMI (Amazon Machine Image) → Select instance type → Configure → Launch',
        explanation: 'EC2 = rent virtual machines. Choose OS (Amazon Linux, Ubuntu, Windows), CPU/RAM size (t2.micro, m5.large), storage. Pay per hour. Start/stop anytime.',
        expectedOutput: 'Virtual server running in AWS. SSH access via key pair. Full control over OS and applications.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 't2.micro: 1 vCPU, 1GB RAM (free tier, testing)\nt3.medium: 2 vCPU, 4GB RAM (small apps)\nm5.large: 2 vCPU, 8GB RAM (production web servers)\nc5.xlarge: 4 vCPU, 8GB RAM (compute-intensive)\nr5.large: 2 vCPU, 16GB RAM (memory-intensive)',
        explanation: 'Instance families: T (burstable, low cost), M (general purpose), C (compute-optimized), R (memory-optimized). Numbers = generation (higher = newer). Size: nano < micro < small < medium < large < xlarge.',
        expectedOutput: 'Understanding which instance type fits your workload based on CPU/memory needs.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'AWS Console → S3 → Create bucket → Upload files → Set permissions → Access via HTTPS URL',
        explanation: 'S3 = unlimited file storage. Store any file type. Pay per GB stored + requests. Highly durable (11 9s). Use for backups, static websites, data lakes, logs.',
        expectedOutput: 'S3 bucket created, files uploaded, accessible via URL or API.',
        validationCriteria: [
      ...
      {
        stepNumber: 4,
        instruction: 'S3 Storage Classes: Optimize costs based on access patterns',
        command: 'S3 Standard: Frequent access, millisecond latency\nS3 Intelligent-Tiering: Auto-moves between tiers\nS3 Glacier: Archive, retrieval takes minutes/hours\nS3 Glacier Deep Archive: Lowest cost, 12-hour retrieval',
        explanation: 'Trade access speed for cost. Frequently accessed = Standard. Rarely accessed = Glacier. Intelligent-Tiering auto-optimizes. Lifecycle rules move objects automatically.',
        expectedOutput: 'Understanding: Choose storage class based on how often you access data.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'AWS Console → VPC → Create VPC → CIDR block (10.0.0.0/16) → Create subnets (public, private) → Configure route tables → Internet Gateway',
        explanation: 'VPC = your own private network in AWS. Define IP ranges (CIDR), create subnets, control routing. Public subnet has internet access. Private subnet isolated. Security groups = firewall.',
        expectedOutput: 'VPC created with public and private subnets, internet gateway attached, route tables configured.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Inbound rules: Allow HTTP (port 80), HTTPS (443), SSH (22) from specific IPs\nOutbound rules: Allow all by default (can restrict)\nStateful: Return traffic automatically allowed',
        explanation: 'Security groups control traffic to/from instances. Whitelist approach (deny all, allow specific). Specify protocol (TCP/UDP), port, source IP. Changes apply immediately.',
        expectedOutput: 'Understanding how to configure security groups for web servers, databases, SSH access.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'AWS Console → RDS → Create database → Choose engine (PostgreSQL, MySQL, MariaDB) → Instance class → Storage → Automated backups',
        explanation: 'RDS = managed relational databases. AWS handles patches, backups, failover. You just use the database. Supports PostgreSQL, MySQL, MariaDB, Oracle, SQL Server. Multi-AZ for high availability.',
        expectedOutput: 'RDS database running with automated backups, accessible from EC2 instances.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'AWS Console → DynamoDB → Create table → Partition key → Optional sort key → Provisioned or On-demand capacity',
        explanation: 'DynamoDB = fully managed NoSQL. Key-value and document store. Millisecond latency. Auto-scaling. Pay per request or reserved capacity. No servers to manage.',
        expectedOutput: 'DynamoDB table created, can store/retrieve items via API or console.',
        validationCriteria: [
      ...
      {
        stepNumber: 9,
        instruction: 'IAM (Identity and Access Management): Control access to AWS',
        command: 'Users: People (email/password or access keys)\nGroups: Collections of users with same permissions\nRoles: For services (EC2, Lambda) to access other AWS resources\nPolicies: JSON documents defining permissions',
        explanation: 'IAM controls who can do what in AWS. Users for people. Roles for services. Policies define permissions (allow/deny actions on resources). Principle of least privilege.',
        expectedOutput: 'Understanding IAM users, groups, roles, and policies for secure AWS access.',
        validationCriteria: [
      ...
      {
        stepNumber: 10,
        instruction: 'ELB (Elastic Load Balancer): Distribute traffic across servers',
        command: 'Application Load Balancer (ALB): HTTP/HTTPS, Layer 7\nNetwork Load Balancer (NLB): TCP/UDP, Layer 4, ultra-low latency\nClassic Load Balancer: Legacy',
        explanation: 'Load balancers distribute traffic across multiple EC2 instances. Health checks detect failures. Auto-scaling adds/removes instances. ALB for web apps (path-based routing). NLB for high performance.',
        expectedOutput: 'Understanding when to use ALB vs NLB, and how load balancing improves availability.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Metrics: CPU, disk, network for EC2, RDS, etc.\nLogs: Centralized log storage and search\nAlarms: Trigger actions when thresholds exceeded\nDashboards: Visualize metrics',
        explanation: 'CloudWatch monitors AWS resources. Collects metrics automatically. Set alarms for high CPU, failed health checks. Store application logs. Create dashboards for visibility.',
        expectedOutput: 'CloudWatch monitoring EC2 instances, alarms configured, logs centralized.',
        validationCriteria: [
      ...
      },
      {
        ...
        command: 'Compute: EC2 (VMs), Lambda (serverless functions), ECS/EKS (containers)\nStorage: S3 (objects), EBS (block storage for EC2), EFS (file storage)\nDatabase: RDS (relational), DynamoDB (NoSQL), ElastiCache (Redis/Memcached)\nNetworking: VPC, Route53 (DNS), CloudFront (CDN)',
        explanation: 'AWS has 200+ services. Core: EC2, S3, RDS, VPC. Serverless: Lambda, DynamoDB. Containers: ECS, EKS. Choose based on workload requirements, not hype.',
        expectedOutput: 'Ability to select appropriate AWS services for common scenarios.',
        validationCriteria: [
    ...
    expectedOutcome: 'You understand AWS core services: EC2 for compute, S3 for storage, VPC for networking, RDS/DynamoDB for databases, IAM for security, ELB for load balancing, CloudWatch for monitoring. You can choose appropriate services for different workloads.'
  },

  walk: {
    introduction: 'Practice AWS service selection through scenario-based exercises.',
    exercises: [
      {
        exerciseNumber: 1,
        ...
        explanation: 'Standard 3-tier web app: Load balancer in public subnet, app servers in private subnet with auto-scaling, database in private subnet, static assets in S3.'
      },
      {
        exerciseNumber: 2,
        scenario: 'Choose instance type for different workloads.',
        template: 'Development/testing: __T2_MICRO__ (free tier, low cost)\nProduction web server: __M5_LARGE__ (general purpose)\nData processing: __C5_XLARGE__ (compute-optimized)\nIn-memory cache: __R5_LARGE__ (memory-optimized)',
        blanks: [
          {
            id: 'T2_MICRO',
            label: 'T2_MICRO',
      ...
      },
      {
        ...
        solution: 'E-commerce product catalog: RDS PostgreSQL (relational, ACID)\nSession storage: ElastiCache Redis (in-memory, fast)\nUser activity tracking: DynamoDB (high-scale NoSQL)\nData warehouse: Redshift (analytics)\nFull-text search: Elasticsearch (search engine)',
        explanation: 'Choose database based on data model and access patterns: RDS for relational, DynamoDB for NoSQL, ElastiCache for caching, Redshift for analytics.'
      }
    ],
    hints: [
  ...
  runGuided: {
    objective: 'Design complete AWS architecture for a production web application with high availability',
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck: 'Use draw.io or Lucidchart. Show VPC boundary, subnets in 2+ AZs, ALB distributing to EC2, EC2 connecting to RDS, S3 for static files.'
      },
      {
        ...
        hintIfStuck: 'Ask: What if an AZ fails? What if EC2 instance crashes? What if database goes down? Architecture should handle each scenario.'
      },
      {
        ...
        hintIfStuck: 'Layer security: Network (VPC, security groups), Identity (IAM), Data (encryption). Follow least privilege principle.'
      }
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective: 'Build complete AWS infrastructure documentation including architecture, cost estimate, deployment guide, and operational runbook',
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 30,
        passingThreshold: 'High availability through multi-AZ. Proper network segmentation. Appropriate service selection. Security best practices followed. Scalable design.'
      },
      {
        criterion: 'Completeness',
        weight: 25,
        passingThreshold: 'All required components documented. Cost estimate thorough. Deployment guide complete. Operational procedures included. Nothing major missing.'
      },
      {
        criterion: 'Production Readiness',
        weight: 25,
        passingThreshold: 'Monitoring configured. Backups automated. Disaster recovery planned. Security controls in place. Ready to handle real traffic.'
      },
      {
        criterion: 'Cost Optimization',
        weight: 20,
        passingThreshold: 'Right-sized instances. Reserved instance strategy. S3 lifecycle policies. Auto-scaling to match demand. Cost monitoring configured.'
      }
    ]
  },
