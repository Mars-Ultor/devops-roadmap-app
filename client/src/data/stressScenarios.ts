...
export const STRESS_SCENARIOS: StressScenario[] = [
  // LOW STRESS - Introduction to pressure
  {
    ...
    description:
      "Deploy a Docker container within tight deadline. Customer demo in 30 minutes.",
  ...
  // MEDIUM STRESS - Multiple simultaneous issues
  {
    ...
    description:
      "Two services need deployment simultaneously. Different teams waiting.",
  ...
  // HIGH STRESS - Production incident
  {
    ...
    description:
      "Service down! 5,000 users affected. CEO is asking for updates.",
  ...
  // EXTREME STRESS - Everything at once
  {
    ...
    description:
      "Multiple services failing during peak traffic. All hands on deck!",
  ...
  // MEDIUM STRESS - Resource constraints
  {
    ...
    description:
      "Deploy to production with limited permissions and no documentation.",
    stressLevel: "medium" as StressLevel,
    conditions: [
      {
        id: "rc-1",
  ...
  // HIGH STRESS - Interruption handling
  {
    ...
    description:
      "Deploy new feature while handling incoming alerts and questions.",
    stressLevel: "high" as StressLevel,
    conditions: [
      {
        id: "int-1",
...
export function getScenarioByStressLevel(
  level: StressLevel | string,
): StressScenario[] {
  return STRESS_SCENARIOS.filter(
    (s) => s.stressLevel === (level as StressLevel),
  );
}
...
export function calculatePerformanceRating(
  ...
  if (completionRate === 1 && timeRatio < 0.8 && errorsCount === 0)
    return "excellent";
  if (completionRate === 1 && timeRatio < 1.0 && errorsCount <= 1)
    return "good";
