...
export const week1Lesson3BashBasics: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description:
      "Master Bash scripting essentials: variables, pipes, redirection, loops, and automation for DevOps workflows.",
    learningObjectives: [
  ...
  crawl: {
    introduction:
      "Learn Bash scripting step-by-step. Execute each command to understand variables, pipes, redirection, loops, and script creation.",
    steps: [
      {
        stepNumber: 1,
        ...
        explanation:
          "Variables store data. Use = to assign (no spaces!). Access with $VARIABLE_NAME. Double quotes allow variable expansion.",
      ...
      },
      {
        ...
        command:
          'echo "Home directory: $HOME" && echo "Current user: $USER" && echo "Shell: $SHELL"',
        explanation:
          "Environment variables are system-wide settings. $HOME is home directory, $USER is username, $SHELL is current shell. Always available.",
        expectedOutput:
      ...
      },
      {
        ...
        command:
          'export API_KEY="secret123" && bash -c \'echo "API Key in subprocess: $API_KEY"\'',
        explanation:
          "export makes a variable available to child processes. Without export, subprocesses can't see the variable. Critical for scripts that call other scripts.",
      ...
      },
      {
        ...
        command:
          'echo -e "apple\\nbanana\\ncherry\\napricot" | grep "a" | sort',
        explanation:
          'Pipes (|) send output of one command as input to next. This creates "banana, apple, apricot" list, filters lines with "a", then sorts alphabetically.',
      ...
      },
      {
        ...
        explanation:
          "> redirects stdout to file (overwrites). >> appends. $(date) is command substitution - runs date command and inserts result.",
      ...
      },
      {
        ...
        command:
          'ls /nonexistent 2> error.log && echo "Stdout works" > output.log; cat error.log output.log',
        explanation:
          "File descriptor 1 = stdout, 2 = stderr. 2> redirects errors separately. Useful for logging errors vs normal output. ; runs commands sequentially.",
        expectedOutput:
      ...
      },
      {
        ...
        command:
          'CURRENT_DATE=$(date +%Y-%m-%d) && echo "Backup file: backup-$CURRENT_DATE.tar.gz"',
        explanation:
          "$(...) runs command and substitutes output. Alternative: backticks `...` (old style). Useful for dynamic file names, timestamps, etc.",
      ...
      },
      {
        ...
        explanation:
          "for loop iterates over list. Syntax: for VAR in LIST; do COMMANDS; done. VAR takes each value in LIST sequentially.",
        expectedOutput:
          "Processing item 1\nProcessing item 2\nProcessing item 3\nProcessing item 4\nProcessing item 5",
        validationCriteria: [
      ...
      },
      {
        ...
        command:
          'mkdir test_files && touch test_files/{file1.txt,file2.txt,file3.txt} && for file in test_files/*.txt; do echo "Found: $file"; done',
        explanation:
          "Globbing pattern *.txt expands to all .txt files. Loop processes each file. Common DevOps pattern: process logs, configs, etc.",
        expectedOutput:
          "Found: test_files/file1.txt\nFound: test_files/file2.txt\nFound: test_files/file3.txt",
        validationCriteria: [
      ...
      },
      {
        ...
        command:
          'if [ -f "build.log" ]; then echo "Build log exists"; else echo "No build log found"; fi',
        explanation:
          "if tests condition. -f checks if file exists and is regular file. Must have spaces around brackets! fi ends if statement.",
        expectedOutput:
          'Build log exists (or "No build log found" if file doesn\'t exist)',
        validationCriteria: [
      ...
      },
      {
        ...
        explanation:
          "#!/bin/bash is shebang - tells system to use Bash. chmod +x makes file executable. ./ runs script in current directory.",
        expectedOutput:
      ...
      },
      {
        ...
        command:
          'ls /tmp > /dev/null && echo "Command succeeded (exit code: $?)" || echo "Command failed (exit code: $?)"',
        explanation:
          "Every command returns exit code: 0 = success, non-zero = failure. $? contains last exit code. && runs if previous succeeded, || runs if failed.",
    ...
    expectedOutcome:
      "You can write Bash commands using variables, pipes, redirection, loops, conditionals, and scripts. You understand command substitution, exit codes, and basic automation patterns for DevOps tasks.",
  },

  walk: {
    introduction:
      "Apply Bash knowledge with fill-in-the-blank exercises. Build practical automation snippets.",
    exercises: [
      {
        exerciseNumber: 1,
        scenario:
          "Create a variable with today's date and use it in a backup filename",
        template:
          '___ASSIGN_VAR___=$(date +%Y%m%d)\necho "Creating backup: app-backup-$___VAR_NAME___.tar.gz"',
        blanks: [
          {
            id: "ASSIGN_VAR",
            label: "ASSIGN_VAR",
        ...
        explanation:
          "Command substitution $(...) captures date output. Variable used in filename creates unique backups (e.g., app-backup-20251210.tar.gz).",
      },
      {
        exerciseNumber: 2,
        ...
        explanation:
          "cat *.log concatenates all log files. Pipe to wc -l (word count lines) gives total line count. Useful for log analysis.",
      },
      {
        exerciseNumber: 3,
        scenario: "Redirect both stdout and stderr to the same log file",
        template:
          "./deploy.sh ___REDIRECT_STDOUT___ deploy.log ___REDIRECT_STDERR___",
        blanks: [
          {
            id: "REDIRECT_STDOUT",
        ...
        explanation:
          "> redirects stdout to file. 2>&1 redirects stderr (fd 2) to wherever stdout (fd 1) is going. Captures all output in one log.",
      },
      {
        exerciseNumber: 4,
        scenario: "Loop through all .sh files and make them executable",
        template:
          'for ___LOOP_VAR___ in *.sh; do\n  ___MAKE_EXECUTABLE___ "$___LOOP_VAR___"\ndone',
        blanks: [
          {
            id: "LOOP_VAR",
            label: "LOOP_VAR",
        ...
        explanation:
          'Loop iterates over all .sh files. chmod +x adds execute permission. Quotes around "$script" handle filenames with spaces.',
      },
      {
        exerciseNumber: 5,
        scenario: "Check if a file exists before processing it",
        template:
          'if [ ___FILE_EXISTS_TEST___ "config.yml" ]; then\n  echo "Config found, processing..."\n  cat config.yml\nelse\n  echo "Error: config.yml not found"\n  ___EXIT_WITH_ERROR___\nfi',
        blanks: [
          {
            id: "FILE_EXISTS_TEST",
        ...
        explanation:
          "-f tests if file exists and is regular file. exit 1 terminates script with error status. Essential for robust scripts.",
      },
    ],
    hints: [
  ...
  runGuided: {
    objective:
      "Create a backup script that archives directories, adds timestamps, and handles errors",
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        description:
          "Script takes directory path as argument and validates it exists",
        validationCriteria: [
        ...
        hintIfStuck:
          'if [ $# -eq 0 ]; then echo "Usage: $0 <directory>"; exit 1; fi. Then test [ -d "$1" ] to validate directory.',
      },
      {
        ...
        hintIfStuck:
          'TIMESTAMP=$(date +%Y%m%d_%H%M%S); tar -czf "backup_$TIMESTAMP.tar.gz" "$1"',
      },
      {
        ...
        hintIfStuck:
          'After tar command: if [ $? -eq 0 ]; then echo "Backup successful: $FILENAME"; else echo "Backup failed!"; exit 1; fi',
      },
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective:
      "Create a comprehensive automation script that organizes downloaded files by type, logs actions, and sends notifications",
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 35,
        passingThreshold:
          "Script correctly categorizes and moves files. All file types handled. No files lost or corrupted.",
      },
      {
        criterion: "Error Handling",
        weight: 25,
        passingThreshold:
          "Validates input directory exists. Checks move operations succeeded. Handles edge cases (spaces in names, special characters). Exits gracefully on errors.",
      },
      {
        criterion: "Logging & Reporting",
        weight: 20,
        passingThreshold:
          "All actions logged with timestamps. Summary report shows counts per category. Log format is clear and parseable.",
      },
      {
        criterion: "Code Quality",
        weight: 20,
        passingThreshold:
          "Uses functions for organization. Variables properly quoted. Comments explain logic. Follows Bash best practices (set -e, meaningful exit codes).",
      },
    ],
  },
