...
export const BATTLE_DRILLS: BattleDrill[] = [
  {
    ...
    description:
      "Deploy a containerized web application from source code to production-ready state",
...
export function getBattleDrillsByCategory(
  category: BattleDrill["category"],
): BattleDrill[] {
  return BATTLE_DRILLS.filter((drill) => drill.category === category);
}

// Get drills by difficulty
export function getBattleDrillsByDifficulty(
  difficulty: BattleDrill["difficulty"],
): BattleDrill[] {
...
export function getRandomBattleDrill(excludeIds: string[] = []): BattleDrill {
  const available = BATTLE_DRILLS.filter(
    (drill) => !excludeIds.includes(drill.id),
  );
...
export function getDrillsForWeek(week: number): BattleDrill[] {
  if (week <= 4) {
    ...
    return BATTLE_DRILLS.filter(
      (d) => d.difficulty === "basic" || d.difficulty === "intermediate",
    );
  } else {
