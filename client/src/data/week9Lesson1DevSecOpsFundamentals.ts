...
export const week9Lesson1DevSecOpsFundamentals: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description: 'Master security principles for DevOps. Learn shift-left security, threat modeling, and how to integrate security into every stage of the software development lifecycle.',
    learningObjectives: [
  ...
  crawl: {
    introduction: 'Learn DevSecOps principles step-by-step. You will understand shift-left security, conduct threat modeling, and integrate security into your development workflow.',
    expectedOutcome: 'Complete understanding of DevSecOps, shift-left security, OWASP Top 10, STRIDE threat modeling, and automated security scanning with SAST/DAST/SCA tools',
    steps: [
      {
        stepNumber: 1,
        instruction: 'Understand the three core DevSecOps principles',
        command: 'echo "DevSecOps Principles:\n1. Shift Left: Find vulnerabilities early\n2. Automate Security: Security as code\n3. Continuous Monitoring: Always vigilant"',
        explanation: 'DevSecOps integrates security into every phase of development, not just at the end. Shift-left means testing security early when fixes are cheaper.',
        expectedOutput: 'DevSecOps Principles:\n1. Shift Left: Find vulnerabilities early\n2. Automate Security: Security as code\n3. Continuous Monitoring: Always vigilant',
        validationCriteria: [
          'Understand shift-left concept',
          'Know why early detection matters',
          'Recognize automation importance'
        ],
        commonMistakes: ['Thinking security is only for production', 'Relying on manual security reviews only']
      },
      {
        stepNumber: 2,
        instruction: 'Identify the OWASP Top 10 security risks',
        command: 'curl -s https://owasp.org/www-project-top-ten/ | grep -E "(Broken|Injection|Cryptographic|Security)"',
        explanation: 'OWASP Top 10 lists the most critical web application security risks: Injection, Broken Auth, Sensitive Data Exposure, XXE, Broken Access Control, Security Misconfiguration, XSS, Insecure Deserialization, Using Components with Known Vulnerabilities, Insufficient Logging.',
      ...
      },
      {
        ...
        commonMistakes: ['Skipping threat modeling', 'Not considering all STRIDE categories']
      },
      {
        stepNumber: 4,
        instruction: 'Set up pre-commit hooks for secret scanning',
        command: 'pip install detect-secrets && detect-secrets scan --baseline .secrets.baseline',
        explanation: 'detect-secrets prevents committing API keys, passwords, tokens. Pre-commit hooks block commits with secrets.',
        ...
        commonMistakes: ['Committing .env files', 'Hardcoding credentials in code']
      },
      {
        stepNumber: 5,
        instruction: 'Configure dependency vulnerability scanning',
        command: 'npm audit --json > audit-report.json && cat audit-report.json | jq ".vulnerabilities | length"',
        explanation: 'npm audit checks for known vulnerabilities in dependencies. Fix high/critical issues immediately.',
        ...
        commonMistakes: ['Ignoring low-severity issues that add up', 'Not updating dependencies regularly']
      },
      {
        stepNumber: 6,
        instruction: 'Add security linting with ESLint security plugin',
        command: 'npm install eslint-plugin-security --save-dev && echo "{\\"plugins\\": [\\"security\\"]}" > .eslintrc.json',
        explanation: 'ESLint security plugin detects unsafe code patterns like eval(), regex DoS, buffer overflows.',
      ...
      },
      {
        ...
        command: 'docker run --rm -v $(pwd):/src returntocorp/semgrep:latest --config=auto /src',
        explanation: 'Semgrep performs static code analysis to find security vulnerabilities without running the code.',
        ...
        commonMistakes: ['Only running on production code', 'Not fixing findings']
      },
      {
        stepNumber: 8,
        instruction: 'Configure container image scanning',
        command: 'docker scan myapp:latest --severity high',
        explanation: 'Docker scan (powered by Snyk) checks container images for vulnerabilities in OS packages and application dependencies.',
        ...
        commonMistakes: ['Using latest base images without pinning', 'Ignoring OS-level vulnerabilities']
      },
      {
        stepNumber: 9,
        instruction: 'Set up security headers in web application',
        command: 'curl -I https://your-app.com | grep -E "(X-Frame-Options|Content-Security-Policy|Strict-Transport-Security)"',
        explanation: 'Security headers protect against XSS, clickjacking, MITM attacks. Essential headers: X-Frame-Options, CSP, HSTS, X-Content-Type-Options.',
      ...
      },
      {
        ...
        command: 'aws iam get-role --role-name MyAppRole | jq ".Role.AssumeRolePolicyDocument"',
        explanation: 'Least privilege: grant only permissions needed, nothing more. Use IAM roles, not root credentials.',
        ...
        commonMistakes: ['Using root credentials', 'Granting AdministratorAccess']
      },
      {
        stepNumber: 11,
        instruction: 'Enable audit logging for security events',
        command: 'kubectl create -f - <<EOF\napiVersion: v1\nkind: ConfigMap\nmetadata:\n  name: audit-policy\ndata:\n  policy.yaml: |\n    apiVersion: audit.k8s.io/v1\n    kind: Policy\n    rules:\n    - level: Metadata\nEOF',
        explanation: 'Audit logs record who did what, when. Essential for security investigations and compliance.',
        ...
        commonMistakes: ['Not logging authentication failures', 'Insufficient retention']
      },
      {
        stepNumber: 12,
        instruction: 'Create security incident response plan',
        command: 'echo "Incident Response Plan:\n1. Detect (alerts, monitoring)\n2. Contain (isolate affected systems)\n3. Eradicate (remove threat)\n4. Recover (restore services)\n5. Post-mortem (learn and improve)"',
        explanation: 'Having a plan before an incident saves time. Document: who to contact, how to isolate systems, communication plan.',
  ...
  runGuided: {
    objective: 'Build comprehensive DevSecOps pipeline with security integrated at every stage',
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck: 'Install detect-secrets, ESLint security plugin, Snyk CLI. Create pre-commit hook that blocks secrets. Add VS Code extensions for security linting.'
      },
      {
        ...
        hintIfStuck: 'GitHub Actions: Add jobs for npm audit, Semgrep, TruffleHog, Trivy. Use continue-on-error: false to fail pipeline. Store results as artifacts.'
      },
      {
        ...
        hintIfStuck: 'Install Kyverno. Create policies: require non-root containers, deny privileged pods, require resource limits, enforce image signatures. Test with kubectl apply.'
      }
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective: 'Build production-ready DevSecOps platform with comprehensive security controls, automated scanning, and incident response capabilities',
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 25,
        passingThreshold: 'Developers have security tools integrated in workflow. Pre-commit hooks block secrets and dangerous code. IDE shows security issues in real-time. Local scanning available. Security issues found before code review.'
      },
      {
        criterion: 'Automated Security Pipeline',
        weight: 30,
        passingThreshold: 'Comprehensive scanning: SAST, DAST, SCA, secrets, containers all integrated. Pipeline fails on high/critical issues. Results dashboarded. Scan time <5 minutes. False positive rate managed.'
      },
      {
        criterion: 'Security Policy Enforcement',
        weight: 25,
        passingThreshold: 'Policy as code deployed (OPA/Kyverno). Pod Security Standards enforced. Network segmentation via policies. Non-compliant resources automatically rejected. Policies version controlled and tested.'
      },
      {
        criterion: 'Operational Security',
        weight: 20,
        passingThreshold: 'Least privilege access enforced. Secrets managed externally. Audit logging comprehensive. Incident response tested. Security metrics tracked. Compliance documented.'
      }
    ]
  },
