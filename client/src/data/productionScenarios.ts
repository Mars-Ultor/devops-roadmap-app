...
export const PRODUCTION_SCENARIOS: ProductionScenario[] = [
  // BEGINNER: Docker deployment failure
  {
    ...
    description: 'Your Node.js API container fails to start after deployment. The previous version is still running.',
    businessImpact: 'New features cannot be deployed. Development team blocked.',
    affectedUsers: 0,
    ...
    symptoms: [
      {
        type: 'log',
        severity: 'error',
        description: 'Error starting userland proxy: listen tcp4 0.0.0.0:3000: bind: address already in use',
        source: 'docker'
      },
      {
        type: 'alert',
    ...
    investigationSteps: [
      {
        ...
        hints: ['Use docker ps to see running containers', 'Look for containers using port 3000'],
        timeEstimate: 30
      },
      {
        id: 'check-port-usage',
    ...
    rootCause: {
      ...
      description: 'Previous version of the container was not stopped before deploying the new version',
      category: 'deployment-failure',
      impact: 'medium'
    },
    ...
    prerequisiteKnowledge: ['Docker basics', 'Port binding', 'Container lifecycle'],
    learningObjectives: [
  ...
  // INTERMEDIATE: Database connection pool exhaustion
  {
    ...
    description: 'API response times have increased from 200ms to 5+ seconds. Users reporting timeouts.',
    businessImpact: 'Customer experience severely degraded. Potential revenue loss.',
    affectedUsers: 5000,
    revenueImpact: 500,
    ...
    investigationSteps: [
      {
        ...
        expectedFindings: ['Connection count at maximum (100)', 'Many idle connections'],
        hints: [
      ...
      },
      {
        ...
        expectedFindings: ['Connection pool exhausted errors', 'No connection release logs'],
        hints: [
    ...
    rootCause: {
      ...
      description: 'Recent code deployment introduced database queries that don\'t release connections back to the pool',
      category: 'performance-degradation',
      impact: 'critical'
    },
    ...
    resolutionSteps: [
      {
        ...
        description: 'Restart API servers to clear connection pool (temporary fix)',
      ...
      },
      {
        ...
        validation: 'Monitor connection pool for 30 minutes - should stay under 30%',
        canRollback: true,
        estimatedTime: 300
      }
    ],
  ...
  // ADVANCED: Kubernetes pod crash loop
  {
    ...
    description: 'Payment processing service pods are crash looping. All payment transactions failing.',
    ...
    symptoms: [
      ...
      },
      {
        type: 'user-report',
        severity: 'critical',
        description: 'Customers cannot complete purchases. Shopping carts abandoned.',
        source: 'support'
      }
    ],
    ...
    investigationSteps: [
      {
        ...
        command: 'kubectl get pods -n production | grep payment-service; kubectl describe pod <pod-name> -n production',
        expectedFindings: ['CrashLoopBackOff state', 'OOMKilled exit code', 'Memory limit: 512Mi'],
        hints: [
      ...
      },
      {
        ...
        expectedFindings: ['Memory usage climbing', 'Large data processing job', 'No cleanup of objects'],
        hints: [
      ...
      },
      {
        ...
        expectedFindings: ['Memory usage consistently near limit', 'Trending upward before crash'],
        hints: [
      ...
      },
      {
        ...
        expectedFindings: ['New version deployed 2 hours ago', 'Added batch processing feature'],
        hints: [
    ...
    rootCause: {
      ...
      description: 'New batch processing feature requires more memory than allocated. Memory limit (512Mi) insufficient for current workload.',
    ...
    resolutionSteps: [
      {
        ...
        command: 'kubectl set resources deployment payment-service --limits=memory=2Gi -n production',
      ...
      },
      {
        ...
        validation: 'Code review confirms streaming approach instead of loading all data',
        canRollback: false,
        estimatedTime: 1800
      },
      {
        ...
        command: 'kubectl set resources deployment payment-service --limits=memory=1Gi --requests=memory=768Mi',
...
export function getScenariosByDifficulty(difficulty: string): ProductionScenario[] {
