...
export const week2Lesson2WhyVersionControl: LeveledLessonContent = {
  ...
  baseLesson: {
    ...
    description:
      "Understand the critical problems version control solves and why every developer needs it.",
    learningObjectives: [
  ...
  crawl: {
    introduction:
      "Understand version control by experiencing the problems it solves. Learn through scenarios and mental models.",
    steps: [
      {
        stepNumber: 1,
        instruction:
          "The problem: Final.docx, Final_v2.docx, Final_ACTUALLY_FINAL.docx",
        command:
          "Imagine managing code like this: app.js, app_backup.js, app_working.js, app_before_refactor.js, app_oct15.js",
        explanation:
          "Without version control, developers create file copies for every change. Quickly becomes unmaintainable. Which is the real version? Which has the bug fix?",
        expectedOutput:
          "Mental model: Folder chaos with dozens of similar files, confusion about which is current.",
        validationCriteria: [
      ...
      },
      {
        stepNumber: 2,
        instruction:
          "The collaboration nightmare: Multiple developers, one codebase",
        command:
          "Developer A: Edits app.js → emails to team. Developer B: Meanwhile edits app.js → emails to team. Now what?",
        explanation:
          'Without version control, team members\' changes conflict. Who has the "real" version? How to merge? Manual merge = hours of tedious work + high error risk.',
        expectedOutput:
          "Mental model: Two developers making simultaneous changes, no way to automatically merge, lost work.",
        validationCriteria: [
      ...
      },
      {
        ...
        command:
          "All code in central repository (Git repo). Everyone clones repo, makes changes, pushes back. System tracks everything.",
        explanation:
          'Version control = single source of truth. Repository contains entire history. Every change tracked: who, what, when, why. No more "which file is current?"',
        expectedOutput:
          "Mental model: Central repository with complete history, developers pulling/pushing changes.",
        validationCriteria: [
      ...
      },
      {
        ...
        command:
          "Made a mistake? Broke the build? Just revert to last working version from 1 hour ago.",
        explanation:
          "Every commit = snapshot in time. Can compare any two versions. Can roll back to any point. Like undo for your entire project, forever.",
        expectedOutput:
          "Mental model: Timeline of commits, ability to jump to any point in history.",
        validationCriteria: [
      ...
      },
      {
        ...
        command:
          "Main branch = production code. Feature branches = isolated work. Merge when ready.",
        explanation:
          "Branches let multiple developers work simultaneously without interfering. Each feature gets its own branch. Merge to main when complete and tested.",
        expectedOutput:
          "Mental model: Main branch with multiple feature branches, merging back when ready.",
        validationCriteria: [
      ...
      },
      {
        ...
        command:
          "git blame shows who wrote each line. git log shows all changes. Every commit has author and message.",
        explanation:
          "Version control provides accountability. Not for blame, but for understanding: why was this changed? Who can explain this code? Critical for team context.",
        expectedOutput:
          "Mental model: Every line of code annotated with author, date, reason.",
        validationCriteria: [
      ...
      },
      {
        ...
        command:
          "Developer creates Pull Request → team reviews code → approves or requests changes → merge.",
        explanation:
          "Version control enables structured code review. Pull Requests = formal review process. Catch bugs, share knowledge, maintain quality. Can't merge without approval.",
        expectedOutput:
          "Mental model: Pull Request workflow with review, discussion, approval before merge.",
        validationCriteria: [
      ...
      },
      {
        ...
        command:
          "Commit pushed → CI server runs all tests → notifies if tests fail → prevents merge if broken.",
        explanation:
          "Version control integrates with CI/CD. Every commit triggers automated tests. Broken code can't be merged. Fast feedback = catch bugs early.",
        expectedOutput:
          "Mental model: Commit → automatic test → green (merge) or red (fix).",
        validationCriteria: [
      ...
      },
      {
        ...
        command:
          "Laptop dies? Clone repo on new machine. Server crashes? Restore from any clone. Complete history preserved.",
        explanation:
          "Every clone is full backup with complete history. Distributed = no single point of failure. Disaster recovery is built-in, not extra effort.",
        expectedOutput:
          "Mental model: Distributed repository, multiple complete copies, inherent backup.",
        validationCriteria: [
      ...
      },
      {
        stepNumber: 10,
        instruction:
          "Open source: Version control enables global collaboration",
        command:
          "Fork repo → make changes → submit Pull Request → maintainers review → merged into official project.",
        explanation:
          "Version control powers open source. Anyone can contribute without direct access. Fork = your copy. Pull Request = proposed change. Maintainers control what's accepted.",
        expectedOutput:
          "Mental model: Fork/PR workflow enabling global contribution.",
        validationCriteria: [
    ...
    expectedOutcome:
      "You understand why version control is essential: prevents file chaos, enables collaboration, provides time travel, ensures accountability, enables code review, integrates with CI/CD, provides disaster recovery, and powers open source.",
  },

  walk: {
    introduction:
      "Apply version control understanding through scenario analysis.",
    exercises: [
      {
        exerciseNumber: 1,
        scenario:
          "A developer accidentally deletes important code. How does version control help?",
        template:
        ...
        explanation:
          "Version control = time machine. Every commit is preserved. Deleted code can be restored from history.",
      },
      {
        exerciseNumber: 2,
        scenario:
          "Two developers edit the same file simultaneously. How does version control handle this?",
        template:
          "Scenario: __CONCURRENT_EDITS__\nVersion control detects __CONFLICT__\nDevelopers must __MANUAL_RESOLUTION__\nResult: __MERGED_CODE__ with both changes",
        blanks: [
          {
            id: "CONCURRENT_EDITS",
        ...
        explanation:
          "Version control detects conflicts automatically. Developers resolve manually. Much better than emailing files back and forth.",
      },
      {
        exerciseNumber: 3,
        scenario:
          "A bug is discovered in production. How to find when it was introduced?",
        template:
          "Use __BISECT_TOOL__ to __BINARY_SEARCH__ through commits\nTest each commit: __WORKS_OR_BROKEN__\nIdentify __EXACT_COMMIT__ that introduced bug\nReview __COMMIT_MESSAGE__ and __DIFF__ to understand why",
        blanks: [
          {
            id: "BISECT_TOOL",
            label: "BISECT_TOOL",
        ...
        explanation:
          "Git bisect automates finding bug introduction. Binary search is efficient. Commit history makes debugging faster.",
      },
      {
        exerciseNumber: 4,
        scenario:
          "New developer joins team. How does version control help onboarding?",
        template:
          "New developer __CLONES__ repository to get all code\n__COMMIT_HISTORY__ shows how project evolved\n__README__ and __DOCS__ versioned alongside code\nCan __CHECKOUT_TAGS__ to see specific releases\nPull Requests show __CODE_REVIEW__ discussions",
        blanks: [
          {
            id: "CLONES",
            label: "CLONES",
      ...
      },
      {
        ...
        solution:
          "Pull Request workflow: developer proposes changes\nCode review by team members\nAutomated tests run via CI/CD\nApproval required before merge\nResult: higher quality code in main branch",
        explanation:
          "Version control enables quality gates. Pull Requests + reviews + CI = prevent bad code from reaching production.",
      },
    ],
    hints: [
  ...
  runGuided: {
    objective:
      "Document a real-world scenario where version control would have prevented a disaster",
    conceptualGuidance: [
    ...
    checkpoints: [
      {
        ...
        hintIfStuck:
          "Examples: lost code due to crashed laptop, conflicting changes from 2 devs, accidental deletion of production database config, unable to roll back bad deploy",
      },
      {
        checkpoint: "Version control solution proposed",
        description:
          "How version control would have prevented or minimized damage",
        validationCriteria: [
        ...
        hintIfStuck:
          "Think through: What would have been in Git? How would commits help? Could have rolled back? Would branches have isolated risk?",
      },
      {
        ...
        hintIfStuck:
          "Examples: Commit every hour, require 2 approvals for main branch, run tests on every PR, never force-push to main, tag all releases",
      },
    ],
    resourcesAllowed: [
  ...
  runIndependent: {
    objective:
      'Create a comprehensive "Version Control Best Practices Guide" for a development team, demonstrating deep understanding',
    successCriteria: [
    ...
    evaluationRubric: [
      {
        ...
        weight: 30,
        passingThreshold:
          "Covers commit hygiene, branching, PR workflow, review, CI/CD, security. Not just Git commands, but team processes.",
      },
      {
        criterion: "Practicality",
        weight: 25,
        passingThreshold:
          "Practices are actionable and realistic. Team can implement immediately. Not theoretical perfection but pragmatic improvement.",
      },
      {
        criterion: "Understanding",
        weight: 25,
        passingThreshold:
          "Demonstrates why version control matters. Rationale shows deep understanding of problems being solved. Anticipates team objections.",
      },
      {
        criterion: "Communication",
        weight: 20,
        passingThreshold:
          "Guide is clear, well-organized, scannable. Examples are helpful. Diagrams clarify complex workflows. New developers can understand.",
      },
    ],
  },
