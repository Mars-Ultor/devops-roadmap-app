...
export class ProgressiveConstraintsManager {
  private static CONSTRAINTS: Record<'crawl' | 'walk' | 'run', ConstraintLevel> = {
    crawl: {
      phase: 'crawl',
      maxHints: Infinity,
      maxResets: Infinity,
  ...
  static getConstraintStatus(
    ...
    return {
      level,
      hintsRemaining: level.maxHints === Infinity ? Infinity : Math.max(0, level.maxHints - hintsUsed),
      resetsRemaining: level.maxResets === Infinity ? Infinity : Math.max(0, level.maxResets - resetsUsed),
      hintsUsed,
      resetsUsed
    };
  }
   ...
   */
  static getWarningMessage(weekNumber: number, hintsUsed: number, resetsUsed: number): string | null {
    const level = this.getConstraintLevel(weekNumber);
   ...
   */
  static initializeCopyPasteBlocking(weekNumber: number, elementSelector: string): () => void {
